...
Wnioskowanie typów (*type inference*) streszcza się po polsku najczęściej zdaniem „kompilator sam się domyśli typu” — i to zdanie myli w dwóch miejscach naraz. Po pierwsze, kompilator niczego nie zgaduje: rozwiązuje układ ograniczeń zebranych z **całego ciała funkcji**. Dlatego `let x = 10;` i `let y = 20;` to dwie nieodróżnialne linijki, z których jedna daje `u32`, a druga `i8` — rozstrzygają `takes_u32(x)` i `takes_i8(y)` **poniżej**. Po drugie, nie ma tu żadnego domyślania się odłożonego na czas działania programu: odpowiedź musi zapaść przed uruchomieniem i jest własnością programu, a nie konkretnego przebiegu.

Kierunek jest tym, co najbardziej zaskakuje osoby idące tu od `var` w Javie czy C# albo od deklaracji śródliniowej w ABAP-ie. `DATA(lv_x) = 10.` czyta **prawą stronę tej jednej instrukcji** i nic w dalszej części metody już tego nie ruszy; Rust czyta do przodu, więc `let mut names = Vec::new();` nie ma jeszcze typu elementu, dopóki nie pojawi się `names.push("Ada")`. Linijka z `let` nie jest miejscem, w którym typ jest znany — jest miejscem, w którym pada pytanie.

Drugie nieporozumienie warte rozbrojenia to zrównanie `let x = 10;` z pythonowym `x = 10`. Wartość dynamiczna niesie znacznik typu i kosztuje tyle samo, cokolwiek trzyma; tutaj `x` zajmuje 4 bajty, a `y` jeden, i obie te liczby są ustalone w czasie kompilacji. Ta strona sprawdza mocniejszą wersję tezy, zamiast ją głosić: dwa programy, jeden z `let x = 10;`, drugi z `let x: u32 = 10;`, kompilują się do **bajt w bajt identycznych** plików wykonywalnych, a `cmp` milczy. Adnotacja typu jest wejściem dla kompilatora, a nie czymś, co go przeżywa.

Na koniec trzy rzeczy praktyczne. Gdy nic w funkcji nie rozstrzyga, Rust nie dobiera szerokości do wartości — `1` to `i32`, a `1.0` to `f64`, i „mała liczba” nie jest powodem, żeby wybrać `u8`. Zanim typ się ustali, w komunikatach występuje pod nazwą zastępczą `{integer}` albo `{float}`; to nie są typy, które da się napisać, tylko kompilator mówiący „liczba o jeszcze nierozstrzygniętej szerokości”, i stąd błąd „can't compare `{float}` with `{integer}`” bierze się nie z braku konwersji, lecz z tego, że dwa różne typy zapasowe (*fallback*) nigdy nie okażą się sobie równe. Samo wnioskowanie kończy się natomiast na **sygnaturze**: każdy parametr i każdy typ zwracany wypisuje się zawsze — w ciele kompilator rozwiązuje, na granicy ty deklarujesz. A szukając pomocy, podawaj **kod błędu** (`E0282`, `E0284`), bo polskich tekstów o `{integer}` praktycznie nie ma.

**Szukaj po polsku:** wnioskowanie typów · adnotacja typu · typ zapasowy · `rust type inference` · `rust E0282 type annotations needed` · `rust integer fallback i32`
