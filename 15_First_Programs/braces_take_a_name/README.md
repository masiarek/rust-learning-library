...
Od Rusta 1.58 (styczeń 2022) w łańcuchu formatującym (*format string*) można napisać wprost `println!("n is {n}")`, a makro samo znajdzie zmienną o tej nazwie. Dla polskiego czytelnika ma to jeden praktyczny skutek: większość materiałów po polsku powstała wcześniej i zna wyłącznie starszy zapis `println!("{}", n)`. Obie formy są poprawne, żadna nie jest przestarzała, więc nie trzeba niczego przepisywać — trzeba tylko wiedzieć, że `{n}` w cudzym kodzie nie jest literówką ani nową składnią, o której się nie doczytało.

Kluczowa zasada brzmi: w klamrach stoi **identyfikator**, i nic poza nim. `println!` to **makro** (stąd wykrzyknik), które czyta ten łańcuch w czasie kompilacji, więc nie ma tam żadnego obliczania wyrażeń — parser zatrzymuje się na pierwszym znaku, który nie może być częścią nazwy. Stąd `{n + 1}` daje `expected }, found +`, `{scores.len()}` daje `found .`, a `{scores[0]}` — `found [`. Żaden z tych komunikatów nie jest błędem typów: makro nie zdołało nawet **doczytać** łańcucha do końca, więc mówi o interpunkcji, a nie o tym, co chciałeś zrobić. To jest cała różnica względem f-stringów z Pythona, które wyglądają identycznie i przyjmują dowolne wyrażenie.

Jeden przypadek dostał własny komunikat, bo popełniany jest wyjątkowo często: `{self.voter}` wewnątrz metody kończy się na `error: invalid format string: field access isn't supported`, z gotową podpowiedzią, żeby użyć argumentu pozycyjnego. Uwaga — ten dobry komunikat jest wąski: działa dla prostego `nazwa.pole`, ale już `{ballot.scores.len()}` wraca do zdawkowego `expected }, found .`. Domyślnym rozwiązaniem jest `let` linijkę wyżej, a potem zwykłe przechwycenie nazwy (*capture*); argumenty pozycyjne (`{}` plus lista) czytają się dobrze przy jednej dziurze, a przy czterech psują się przy pierwszej wstawce w środek, bo numeracja przesuwa się po cichu.

Po dwukropku zaczyna się osobny mały język — specyfikacja formatu — i tam nazwy też są przyjmowane, ale ze znakiem `$` na końcu: `{name:>8}` to osiem kolumn na sztywno, a `{name:>width$}` tyle, ile mówi zmienna `width`. Dla nas ważny jest jeden szczegół, którego angielska strona nie musi rozstrzygać: **szerokość liczona jest w znakach, nie w bajtach**, więc „Łódź” (4 znaki, ale 7 bajtów w UTF-8) wyrówna się w `{:>8}` dokładnie tak samo jak „Lodz”. Polskie ogonki nie rozjadą się w tabelce — choć to nadal nie są kolumny ekranu, więc emoji i pismo wschodnioazjatyckie potrafią zwieść. Ostatnia rzecz, na którą warto uważać przy tłumaczeniach interfejsu: sam łańcuch formatujący **musi być literałem**, nie zmienną, bo klamry są czytane w czasie kompilacji. Tablica przetłumaczonych komunikatów, składana w czasie działania programu, nie jest zadaniem dla `format!` — to robota dla biblioteki.

**Szukaj po polsku:** łańcuch formatujący · makra formatujące w Ruście · `rust format args capture 1.58` · `rust invalid format string field access isn't supported` · `rust format width counts chars not bytes` · `rust format argument must be a string literal`
