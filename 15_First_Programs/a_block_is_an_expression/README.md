...
Klamry robią w Ruście dwie rzeczy naraz i o tym jest cała ta lekcja. Pierwsza jest znajoma z każdego języka: `{ }` otwiera **zasięg**, a nazwa zadeklarowana w środku przestaje istnieć na zamykającej klamrze — nie jest przesłonięta, tylko jej nie ma (`error[E0425]: cannot find value ... in this scope`). Druga jest nowa: blok **ma wartość**, a tą wartością jest jego ostatnia linijka napisana **bez średnika**. Tour of Rust nazywa to „blokiem wyrażeniowym” i warto zapamiętać ten termin, bo z niego wynika cała reszta składni — ciało funkcji bez `return`, `if` po prawej stronie `let`, `match` użyty jako wartość.

Polskie kursy programowania wbijają rozróżnienie **instrukcja** (*statement*) i **wyrażenie** (*expression*) od pierwszych zajęć, a tutaj to rozróżnienie nagle zaczyna pracować: w Ruście prawie wszystko jest wyrażeniem, a średnik jest przełącznikiem, który zamienia wyrażenie w instrukcję. Instrukcja nie ma wartości, więc wartością bloku staje się `()`, czyli typ jednostkowy (*unit type*). `{ tally() }` to `6`, a `{ tally(); }` to `()` — ten sam blok, różnica jednego znaku, dwa różne typy. Średnik nie jest tu interpunkcją kończącą linijkę, tylko operatorem, który wyrzuca wartość.

Dlatego najczęstszy błąd pierwszego tygodnia wygląda tak: `fn double(n: i32) -> i32 { n * 2; }` daje `error[E0308]: mismatched types`, a daszki `^^^` wskazują **typ zwracany**, nie średnik. Kompilator mówi „obiecałeś `i32`, a to ciało produkuje `()`”, natomiast przyczyna leży cztery linijki niżej, w podpowiedzi `help: remove this semicolon to return this value`. Odruch, żeby „naprawić” to zmianą sygnatury, usuwa komunikat i zostawia bezużyteczną funkcję — poprawia się ciało, nie nagłówek. W drugą stronę: `return n * 2;` w ostatniej linii skompiluje się bez problemu, ale `clippy::needless_return` (włączony domyślnie) poprosi o skasowanie `return`. Tutaj `return` zostaje do **wcześniejszego** wyjścia ze środka funkcji; na końcu jest szumem — i to jest przyzwyczajenie, które trzeba świadomie oduczyć, przychodząc z C, Javy czy Pythona.

Po co się tego używa w praktyce? Najczęściej po to, żeby **zamknąć `mut` wewnątrz bloku**: `let ballots = { let mut v = Vec::new(); ... v };` oddaje na zewnątrz zwykłe, niemutowalne wiązanie, więc żadna linijka poniżej nie dopisze już nic do wektora. Dalej — żeby wyznaczyć przesłanianiu (*shadowing*) wyraźny koniec i żeby zakończyć pożyczenie (*borrow*), przy czym tu uwaga: od 2018 roku pożyczenie kończy się w miejscu ostatniego użycia, więc większość takich bloków w starszych polskich poradnikach jest już zbędna (skasuj i sprawdź — kompilator powie, czy blok był potrzebny). Z tej samej zasady wynika jeszcze jedno: `if` użyty jako wartość **musi** mieć `else`, bo brakująca gałąź to ścieżka bez wartości — `error[E0317]`, z notką „`if` expressions without `else` evaluate to `()`”.

**Szukaj po polsku:** blok wyrażeniowy · wyrażenie a instrukcja · `rust tail expression` · `rust E0308 mismatched types semicolon` · `rust clippy needless_return` · `rust E0317 if may be missing an else clause`
