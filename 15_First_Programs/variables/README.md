...
Polszczyzna trochę tu przeszkadza: „zmienna” znaczy dosłownie *taka, która się zmienia*, więc zmienna, której zmienić nie wolno, brzmi jak pomyłka. A dokładnie to robi `let` — wiąże nazwę z wartością i **domyślnie jest to wiązanie (*binding*) niemutowalne**. Druga linijka jest tu całą lekcją: `x = 20;` po `let x: i32 = 10;` to `E0384`, *cannot assign twice to immutable variable*. Warto przeczytać, co kompilator proponuje, bo proponuje jedno słowo, a nie inną nazwę: *consider making this binding mutable*, z `mut` dorysowanym pod spodem. Nie ulegaj przy tym pokusie tłumaczenia `let x = 10;` jako „stałej” — stała to `const X: i32 = 10;`, wymaga typu i wartości znanej w czasie kompilacji, podczas gdy `let` nazywa zwykłą wartość obliczoną w czasie działania i żyjącą w swoim zasięgu.

`mut` daje prawo zapisu **nazwie**, a nie wartości — to rozróżnienie, na którym stoi cała strona. Ten sam łańcuch znaków (`String`) potrafi być niemutowalny pod jedną nazwą i zapisywalny pod następną (`let owned = String::from("Ada");` a potem `let mut moved = owned;`), choć bajty na stercie są dokładnie te same. Obietnica jest przy tym pilnowana w obie strony: `mut`, do którego nikt nigdy nie pisze, to ostrzeżenie `unused_mut` — *variable does not need to be mutable*. Dlatego nie dopisuj `mut` „na wszelki wypadek”, jak w językach, gdzie nic to nie kosztuje. Zysk jest w czytaniu cudzego kodu: skoro `mut` zawsze znaczy *tu coś się zmienia*, wystarczy przejrzeć funkcję pod kątem tego jednego słowa, żeby wiedzieć, które nazwy są ruchome. Dla przychodzącego z ABAP-a to odruch odwrotny do wyuczonego — tam wszystko z `DATA` jest zapisywalne na zawsze i szuka się `CONSTANTS`; tutaj szuka się tych trzech `mut`.

Ostatnia pułapka jest składniowa i po polsku szczególnie łatwo w nią wpaść, bo oba zapisy nazwalibyśmy „przypisaniem”. `let spaces = "   ";` a potem `let spaces = spaces.len();` kompiluje się bez `mut` i bez błędu — bo nic nie zostało przypisane. Powstała **druga zmienna**, która przejęła nazwę, i wolno jej mieć inny typ (tutaj `&str` staje się `usize` o wartości 3). To jest przesłanianie (*shadowing*), zupełnie inny mechanizm niż `mut`, a całą różnicę widać w dwóch pytaniach z tabeli powyżej: `x = 20;` wymaga `mut` i typu zmienić nie może, a `let x = 20;` nie wymaga niczego i typ zmienić może.

**Szukaj po polsku:** zmienne mutowalne w Ruscie · przesłanianie zmiennych · wiązanie nazwy z wartością · `rust E0384 cannot assign twice` · `rust let vs const` · `rust unused_mut warning`
