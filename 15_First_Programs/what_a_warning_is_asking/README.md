...
Najważniejsze zdanie tej strony jest gramatyczne: komunikat `rustc` zaczyna się od **warunku** — *„if this is intentional, prefix it with an underscore”* — więc to jest pytanie, a nie polecenie. Kompilator nie zgaduje, tylko pyta, czy naprawdę chodziło ci o to, co napisałeś. Po polsku łatwo przeczytać „ostrzeżenie” jako coś do wyciszenia — zwłaszcza gdy nawyki przynosi się od `pylint` albo od `SLIN`, czyli od osobnego narzędzia, które trzeba pamiętać uruchomić. Tutaj `lint` siedzi w kompilatorze i jest domyślnie włączony, więc nie ma ani osobnego kroku, ani transakcji do zapamiętania. Warto też przestać przewijać wzrokiem ostatnią linijkę komunikatu — `note: #[warn(unused_variables)] (part of #[warn(unused)]) on by default` podaje **nazwę** linta, jego **grupę** i **poziom**, czyli wszystko, czego potrzeba, żeby później zmienić zdanie.

Czterem ostrzeżeniom z przykładowego programu odpowiadają cztery różne poprawki i podkreślnik jest właściwą odpowiedzią **dokładnie na jedno** z nich:

- `winner` — policzony i nigdy niewypisany. To była prawdziwa usterka: program liczył całe głosowanie i nie ogłaszał wyniku. Poprawka to **użyć wartości**, a nie ją ukryć.
- `abstentions` — ten program do zliczania głosów świadomie nie raportuje głosów wstrzymujących się. `_abstentions` mówi „celowo” w miejscu, w którym następny czytający to zobaczy.
- `audit` — wartość, której sensem jest samo istnienie. `_audit`, i **nigdy** samo `_`.
- `old_margin` — funkcja, której już nikt nie woła. Tutaj poprawką jest **usunięcie**, nie `#[allow(dead_code)]`: `allow` zachowa ją na zawsze i zostawi następnej osobie zagadkę, czy to jeszcze do czegoś służy. System kontroli wersji i tak ją pamięta.

Stąd bierze się ostrzeżenie przed `cargo fix`: narzędzie stosuje podpowiedzi kompilatora, a podpowiedź przy `winner` była błędna — dopisałaby podkreślnik i zamiotła usterkę. Druga rzecz, której polskie materiały prawie nie rozróżniają, to **dwa podkreślniki, które nie znaczą tego samego**. `let _nazwa = wartość` to zwykłe wiązanie: wartość żyje do końca zasięgu, a podkreślnik zmienia tylko zachowanie linta. `let _ = wartość` nie wiąże niczego — `_` jest **wzorcem** (*pattern*), a nie nazwą, więc wartość nie ma gdzie mieszkać i zostaje natychmiast wypuszczona (*drop*), w tej samej linijce. Wydruk na dole strony pokazuje to kolejnością: w wariancie (a) `DROP` pada po ostatniej linijce bloku, w (b) przed nią. Jeden znak różnicy i zupełnie inny moment zniszczenia wartości.

To zamienia kwestię stylu w klasę błędów. Wszystko, czego zadaniem jest trwać — `MutexGuard`, blokada pliku, uchwyt transakcji, wpis audytowy — przypisane do gołego `_` zostaje zwolnione, **zanim** zacznie się kod, który miało chronić. Program się kompiluje, ostrzeżenie znika, a strażnik nie strzeże. Reguła jest krótka: jeśli wartość ma `Drop`, na którym ci zależy, musi dostać nazwę — `_coś`, nigdy `_`. Dla przychodzących z Pythona to jest właśnie odpowiednik `with lock:`, czyli `let _guard = …`; a dla przychodzących z ABAP-a rzecz naprawdę nowa, bo tam obiekty sprząta odśmiecacz w momencie, którego nikt nie obiecuje, więc zdania „ta wartość ma żyć do końca tego bloku” po prostu nie da się wypowiedzieć.

Na koniec poziomy i pułapka. Każdy `lint` stoi na jednym z czterech poziomów — `allow`, `warn`, `deny`, `forbid` — ustawianych na elemencie, na bloku, na całym `crate` (`#![deny(unused)]`) albo w ogóle spoza kodu, przez `RUSTFLAGS="-D warnings"` na CI. Pułapka jest w tym, że program z czterema ostrzeżeniami **zakończył się kodem 0**: nic nie zawiedzie, dopóki ktoś tego nie postanowi. Dlatego `-D warnings` warto włączyć wcześnie — w projekcie z czterdziestoma stałymi ostrzeżeniami czterdzieste pierwsze jest niewidoczne, a to właśnie ono znalazło usterkę. Wartością tego mechanizmu nie jest pojedynczy komunikat, tylko to, że licznik normalnie stoi na zerze. I jedno rozróżnienie na koniec, przydatne przy szukaniu pomocy: kontroler pożyczeń (*borrow checker*) i niezgodność typów to błędy bezwarunkowe, nie linty — **jeśli da się coś `#[allow]`ować, to był lint**; a `clippy` zaczyna tam, gdzie `rustc` kończy, i uruchamia się osobno.

**Szukaj po polsku:** ostrzeżenia kompilatora w Ruscie · nieużywana zmienna · podkreślnik zamiast nazwy · `rust unused_variables underscore` · `rust let _ vs let _name drop` · `rust deny warnings CI`
