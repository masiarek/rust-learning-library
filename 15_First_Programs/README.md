...
The first six pages are not Rust *features* at all. They are the loop you will be inside for every other page in the library: write it, run it, read the message, print something to find out what happened. The pages are short and the traps are the interesting half — a `///` that is not a comment, a `_name` and a `_` that mean different things, an extra `)` reported on a line it is not on, an extra semicolon that is `E0308`, and `{n + 1}` in a format string, which does not do what a Python f-string does.
...
| [Comments that compile](comments_that_compile/README.md) | 101 → 201 | Six forms, and only two of them are comments — `///` is `#[doc = "..."]`, a misplaced one is a *warning* rather than an error, and the examples inside are compiled and run as tests |
...
| [Where a bracket error points](where_a_bracket_error_points/README.md) | 101 → 201 | One `)` too many is reported at a later `}` — the `-->` line is where rustc ran out of brackets to pair, the typo sits under a `-` label, and until it is fixed no other error in the file is printed |
...
| [The never type `!`](the_never_type/README.md) | 201 | The type with **zero** values, so an expression that has it fits anywhere — why `panic!()` sits in a `match` arm beside a `u32`, why a semicolon cannot turn it into `()`, and why `Result<(), !>` still will not compile |
...
Ta sekcja nie uczy jeszcze języka — uczy **pętli**, w której siedzi się potem przy każdej następnej stronie tej biblioteki: napisz plik, uruchom go, przeczytaj, co odpowiedział kompilator, wypisz coś, żeby sprawdzić, co się właściwie stało. Pierwsze sześć stron dotyczy `rustc`, ostrzeżeń, błędów nawiasów, komentarzy, wypisywania na ekran i tej składni, której wszystkie późniejsze strony używają bez wyjaśniania — samego języka jeszcze w nich nie ma. Warto od razu przyjąć do wiadomości jedno: `rustc` mówi po angielsku i nie ma przełącznika, który by to zmienił, więc czytanie jego komunikatów jest tu **umiejętnością do wyćwiczenia**, a nie przeszkodą do obejścia. Kod błędu (`E0308`) bywa przy tym najlepszym hasłem do wyszukiwarki, jakie dostaje się za darmo.
