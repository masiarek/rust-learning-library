...
Nazwa jest tu najmniej pomocną częścią: *Model Context Protocol* brzmi jak pojęcie z uczenia maszynowego, a jest formatem wiadomości — JSON-RPC 2.0, jedna wiadomość na jedną linię tekstu. Serwer MCP to zwykły program, który czyta linijkę żądania ze stdin i wypisuje linijkę odpowiedzi na stdout; nie ma w tym sieci ani żadnej trzeciej strony. Znasz już protokół o tym kształcie, nawet jeśli nigdy nie słyszałeś o MCP: `rust-analyzer` to program, który edytor uruchamia i z którym rozmawia po JSON-RPC przez potok — zamień „jakiego typu jest ten symbol” na „wywołaj tę funkcję” i masz MCP. Szukając materiałów, licz się z tym, że specyfikacja istnieje wyłącznie po angielsku, a po polsku trafisz raczej na teksty o agentach niż o protokole; nazwy z protokołu (`tools/list`, `tools/call`, `isError`, `_meta`) zostaw nietknięte nawet we własnych notatkach.

Okienko „Allow running MCP?” z wpisem `rustrover:read_file` nazywa dwie rzeczy oddzielone dwukropkiem: `rustrover` to **serwer** (jeden program uruchomiony przez edytor), a `read_file` to jedno **narzędzie** (*tool*) tego serwera, czyli pojedyncza funkcja z opisanym schematem argumentów. Prefiks jest konieczny, bo nazwy narzędzi są unikalne tylko wewnątrz jednego serwera. Cztery przyciski to cztery zgody różnej wielkości i warto zatrzymać się przy gwiazdce: `rustrover:*` obejmuje także narzędzia, których ten serwer **jeszcze nie ma** — może je dodać i ogłosić przez `notifications/tools/list_changed`. Specyfikacja mówi przy tym wprost, że opis narzędzia nie jest dowodem: dopóki nie ufasz serwerowi, adnotacje narzędzia są niezaufane. Gwiazdka jest więc oceną **serwera**, a nie tego jednego narzędzia, które ci właśnie pokazano — a samo pytanie nie jest kaprysem edytora, tylko wymogiem specyfikacji, bo wywołanie narzędzia to wykonanie dowolnego kodu i człowiek ma mieć możliwość odmowy.

Jedno rozróżnienie ginie w tłumaczeniu, bo po polsku obie rzeczy nazwałoby się „błędem”. `isError: true` to **normalny wynik** niosący komunikat, który model ma przeczytać i spróbować jeszcze raz — zły format daty, wartość poza zakresem, chwilowo niedostępne API. Natomiast `error` z kodem JSON-RPC znaczy, że samo żądanie nie miało sensu: nieznane narzędzie, źle zbudowane wywołanie, i żadne przeredagowanie argumentów nie pomoże. Kryterium jest jedno — **kto to może naprawić**. Zgłoszenie kanałem `error` wywołania poprawnego, które się nie powiodło, odbiera modelowi szansę na samodzielną korektę.

Na koniec jedyna pułapka specyficzna dla Rusta i jedna dla wieku materiałów. Na transporcie stdio **stdout jest łączem**: `println!` — pierwsza rzecz, po którą sięga każdy — wstawia do strumienia linijkę, która nie jest wiadomością JSON-RPC, i parser klienta zatrzymuje się na niej. Do gadania z człowiekiem służy `eprintln!`, bo stderr należy do ciebie, a klient ma nie czytać wypisanego tam tekstu jako oznaki awarii; równie skutecznie i o wiele ciszej psuje protokół biblioteka logująca ustawiona na stdout. A jeśli czytasz starszy artykuł: od rewizji `2026-07-28` protokół jest **bezstanowy** — każde żądanie niesie własną wersję i możliwości w bloku `_meta`, podczas gdy wcześniejsze rewizje zaczynały się od `initialize`. Opisy sesji i „handshake'u” są po prostu starsze niż protokół, który opisują.

**Szukaj po polsku:** protokół MCP · serwer MCP w Ruście · `MCP stdio transport stdout must not` · `MCP isError vs JSON-RPC error` · `rmcp rust sdk server`
