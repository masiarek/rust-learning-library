...
„Rust wolno się kompiluje” to najczęstszy zarzut wobec tego języka — także na polskich forach — i najczęściej odpowiada się na niego listą flag bez modelu, czyli czymś bezużytecznym. Ta strona zaczyna od modelu, i on jest krótki: budowanie to nie jedna czynność, tylko **cztery fazy** — analiza składniowa i rozwijanie makr, sprawdzanie typów i pożyczeń (*type & borrow check*), generowanie kodu (*codegen*) oraz konsolidacja (*linking*, w starszych polskich podręcznikach „konsolidator”, choć szukać trzeba pod słowem `linker`) — do czego dochodzą jeszcze informacje debugowania. Każde znane usprawnienie skraca **dokładnie jedną** z tych faz. Dlatego pytanie nie brzmi „jak przyspieszyć Rusta”, tylko „za którą fazę ja płacę”, a odpowiedź jest inna w każdym projekcie. Sprawdza się ją poleceniem `cargo build --timings`, które zapisuje wykres całego budowania do `target/cargo-timings/`. Pokrętło sięgające fazy, na którą i tak nie tracisz czasu, nie zrobi nic — i to jest najczęstszy powód, dla którego ktoś stosuje wszystkie porady i nie widzi różnicy.

Trzy pokrętła z tej strony, każde z jednym zastrzeżeniem, o którym łatwo zapomnieć:

- **`debug = "line-tables-only"`** w `[profile.dev]` — stabilne, bez nocnego kompilatora, i od tego się zaczyna. Traci się możliwość podglądania zmiennych lokalnych w `lldb`/`gdb`; **nie** traci się linijki z komunikatu paniki. `panicked at src/main.rs:5:9` bierze się z `#[track_caller]`, jest wkompilowane w samo wywołanie i wygląda tak samo przy każdym ustawieniu `debug`. Rozmywa się dopiero ślad stosu pod spodem.
- **`-Z threads=8`** — zrównoleglenie *przedniej* części kompilatora, czyli tej, która sprawdza typy. Wymaga wersji nightly i wciąż bywa niedeterministyczne (diagnostyka potrafi wyjść w innej kolejności), więc na własnej maszynie tak, w CI nie.
- **`codegen-backend = "cranelift"`** — inny generator kodu: LLVM produkuje szybki kod, Cranelift produkuje kod szybko. Wyłącznie pod `[profile.dev]`. Wpisany do profilu wydaniowego daje wolniejszy program i przekreśla cały sens zabiegu.

Najważniejsza część jest jednak arytmetyczna i dotyczy tego, jak wolno o takich zyskach **mówić**. Po pierwsze, **prawo Amdahla** — faza, do której nic nie sięgnęło, nie skurczyła się, więc jej udział w budowaniu rośnie (tu z 4% do 8%) i to ona wyznacza sufit dalszych optymalizacji. Po drugie, rzecz mniej oczywista, pokazana w zadaniu na końcu strony: oszczędność **nie jest cechą samego usprawnienia**, tylko usprawnienia i wszystkiego, co już zastosowano wcześniej. Ta sama zamiana konsolidatora „oszczędza” 6,60 s zastosowana jako pierwsza i 3,96 s zastosowana jako ostatnia, a końcowy czas budowania jest identyczny. Wniosek praktyczny: mierzenie dwóch zmian osobno i dodawanie wyników zawyża je, a zdanie „mold dał nam siedem sekund” jest zdaniem o kolejności, nie o `mold`.

Stąd też bierze się jedyna strona w tej bibliotece bez własnego zweryfikowanego wyniku. Czas budowania zależy od liczby rdzeni, pamięci, dysku, wersji kompilatora i tego, co akurat robił laptop — czyli jest liczbą, której nikt nie odtworzy. Warto z tego wyciągnąć nawyk na własne raporty: **„nasze budowanie przyspieszyło o 47%” to fakt o jednej maszynie, a „informacje debugowania i konsolidacja to 29% naszego budowania” to fakt o projekcie** — i tylko drugie zdanie mówi komukolwiek, co robić dalej.

**Szukaj po polsku:** prawo Amdahla · czas kompilacji Rusta · `cargo build --timings` · `rust debug line-tables-only` · `rust cranelift codegen backend` · `mold linker rust`
