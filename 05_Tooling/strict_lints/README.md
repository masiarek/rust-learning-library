...
- [Production template](../../34_Templates/production/README.md) — this panic set at `deny`, minus `arithmetic_side_effects`, in a whole project to copy; the [training template](../../34_Templates/training/README.md) has the same lints at `warn`
...
Słowo *lint* nie ma dobrego polskiego odpowiednika i nie potrzebuje go: nazwy lintów, poziomy `allow` / `warn` / `deny` oraz cały blok `[lints.clippy]` trafiają wprost do `Cargo.toml` i do atrybutów `#[allow(...)]`, więc w przetłumaczonej postaci byłyby bezużyteczne. Warto za to zauważyć rozróżnienie, na którym stoi ta strona: trzy części tej konfiguracji robią trzy różne rzeczy. `pedantic` to linty skończone i stylistyczne — uczą idiomu, którego oczekuje biblioteka standardowa. `nursery` to linty **w budowie**, hałaśliwsze, niż sugeruje nazwa: na dwulinijkowej, całkowicie poprawnej funkcji potrafi wypaść `this could be a const fn`, a przy poziomie `deny` jest to już błąd kompilacji kodu bez żadnej wady. Natomiast zestaw zakazujący paniki to linty z grupy `restriction`, czyli coś zupełnie innego niż wykrywanie błędów — one zabraniają rzeczy legalnych i idiomatycznych. Clippy nie twierdzi, że `unwrap` jest błędem; to **ty** deklarujesz, że ten program nie ma prawa spanikować. To jest polityka projektu, a nie poprawka.

Dwie pułapki, każda innego rodzaju. `priority = -1` nie jest ozdobnikiem: grupa i pojedynczy należący do niej lint są ze sobą w konflikcie z definicji, a ujemny priorytet sprawia, że `pedantic` i `nursery` stosują się **jako pierwsze**, dzięki czemu wpis poniżej może je nadpisać — bez tego Cargo odrzuca manifest, zamiast zgadywać, więc ta awaria przynajmniej krzyczy. Druga nie krzyczy wcale i to o niej nie mówi się na slajdach: `arithmetic_side_effects` strzela na `n + 1` między dwiema liczbami całkowitymi. To nie jest lint od przypadków brzegowych — przyjęcie go oznacza pisanie `checked_add`, `saturating_add` albo `wrapping_add` w całym projekcie i zmienia sposób czytania kodu bardziej niż cała reszta bloku razem wzięta. Tę jedną linijkę przyjmij świadomie albo ją pomiń.

Codzienną używalność załatwia `clippy.toml`: przy `allow-unwrap-in-tests` ten sam `unwrap()` w module `#[cfg(test)]` nie wywołuje ani jednego ostrzeżenia, więc prototypować dalej można normalnie. A jeśli nie masz dziś ochoty spierać się z całym blokiem, jest wersja na 80%: `cargo clippy -- -D clippy::pedantic -D clippy::nursery` — dwie grupy „uczące Rusta”, bez polityki zakazu paniki, i wyraźnie tańsza połowa do przyjęcia.

**Szukaj po polsku:** linty w Ruście · konfiguracja clippy · `clippy restriction lints` · `clippy lint group priority -1` · `clippy arithmetic_side_effects`
