...
Polskie poradniki zaczynają zwykle od zdania „zainstaluj VS Code i rozszerzenie rust-analyzer”, przez co łatwo wyjść z założenia, że to **edytor** zna Rusta. Nie zna. Cała inteligencja siedzi w osobnym programie — `rust-analyzer`, utrzymywanym przez sam projekt Rust — który czyta twój crate i odpowiada na pytania przez protokół serwera języka (*Language Server Protocol*). Prawdopodobnie masz go już na dysku, bo dostarcza go `rustup`: sprawdź `rustup which rust-analyzer`. VS Code, Zed, Helix, Neovim i Emacs zadają temu samemu programowi te same pytania i tylko inaczej rysują odpowiedzi — więc wybierasz **okno**, a nie ilość wiedzy o języku. Jeden edytor na tej liście do tego klubu nie należy: RustRover ma własny analizator napisany w Kotlinie, starszy od `rust-analyzera` i nieskrępowany LSP, przez co bywa **drugą, niezależną opinią** — czasem zauważy coś, czego tamten nie widzi, i odwrotnie. Reszta jest wspólna dla wszystkich: `cargo build`, `clippy`, komunikaty błędów i `rustfmt` są identyczne wszędzie. Żaden edytor nie sprawia, że Rust **kompiluje się** łatwiej; różnią się tylko tym, jak wcześnie powiedzą ci to, co `rustc` i tak powie.

Jedna funkcja naprawdę warta wyboru edytora na czas nauki to **podpowiedzi typów w kodzie** (*inlay hints*) — szary napis po `let`, nazywający typ, którego nie napisałeś. Łatwo wziąć go za odczytanie literału z prawej strony i to jest właśnie nieporozumienie, które ta strona rozbraja: podpowiedź jest **wnioskiem z całego ciała funkcji**, łącznie z liniami, których jeszcze nie przeczytałeś. Na tej stronie `let seats = 3;` to `i32`, a `let votes = 3;` — ten sam literał na tak samo wyglądającej linii — to `u64`, bo dwie linijki niżej dodaje się go do `u64`. Wydruk kodu tego nie powie; edytor powie. Dlatego rada „każdy edytor tekstu wystarczy, to przecież zwykły tekst” jest zła dla początkującego i całkiem rozsądna dla kogoś, kto ma wnioskowanie typów już w głowie.

Dwie pułapki z tej strony wyglądają identycznie — jak zepsuty edytor — i obie warto znać, zanim stracisz na nie wieczór:

- **Ruchoma podłoga wersji przy własnej konfiguracji.** Zmierzone tutaj: na Neovimie 0.11.5 dodatek `lang.rust` z LazyVim instaluje się bez jednego błędu, kompiluje 29 parserów — i nigdy nie uruchamia serwera języka, bo `rustaceanvim` wymaga wersji 0.12. Jedyny ślad to jedna linijka w dwustu liniach wydruku instalacji. Podświetlanie składni działa dalej (tree-sitter nie interesuje się wersją), więc edytor wygląda na zdrowego, nie dając żadnych typów, żadnych diagnostyk i żadnej wskazówki dlaczego. Tak może zawieść tylko droga „składam sobie sam”: żaden gotowy edytor z tej listy nie pozwala zestawić kombinacji, której nikt nie wydał.
- **`rustup` z Homebrew.** Jeśli instalowałeś Rusta formułą `brew`, a nie skryptem `rustup-init`, to standardowa rada „wskaż IDE katalog `~/.cargo/bin`” nie zadziała, bo tam prawie nic nie ma. Nakładki leżą w `/usr/local/opt/rustup/bin`, a prawdziwy plik binarny wskazuje `rustup which rust-analyzer`.

Werdykt jest po polsku równie prosty: **na czas nauki wybierz edytor, który nie zadaje pytań** — RustRover albo VS Code, z Zedem tuż za nimi. W Polsce narzędzia JetBrains są na tyle rozpowszechnione, że dla wielu osób RustRover będzie najkrótszą drogą, a jego darmowa licencja niekomercyjna wymaga tylko oświadczenia, że nie zarabiasz na tym, co w nim piszesz. LazyVim jest znakomitą odpowiedzią, ale na **inne** pytanie — „jak dołożyć Rusta do edytora, w którym już myślę”. Jeżeli komendy vima nie siedzą ci jeszcze w palcach, koszt trafia w zły budżet: godziny idą na edytor dokładnie wtedy, gdy najtrudniejsze są własność i czasy życia.

**Szukaj po polsku:** podpowiedzi typów · wnioskowanie typów w Ruscie · `rust-analyzer inlay hints` · `rustaceanvim requires neovim 0.12` · `rustup which rust-analyzer`
