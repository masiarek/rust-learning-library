...
Przypięcie toolchaina (*pinning*) to zamiana zbiegu okoliczności w obietnicę: laptop, który nagrywa klucze odpowiedzi, i CI, które je sprawdza, chodziły na tym samym `rustc` przypadkiem, a nie dlatego, że jakiś plik tego wymagał. Zanim jednak sięgniesz po cięższe narzędzia, warto zobaczyć, że **rzeczywista ekspozycja tej biblioteki jest mała i konkretna** — a to jest liczba, do której trzeba porównywać koszt każdego lekarstwa. Runner zapisuje wyłącznie `stdout` programu; komunikaty kompilatora nigdy nie trafiają do klucza, więc nowy `rustc`, który dołoży lint albo przeredaguje ostrzeżenie, niczego tu nie ruszy. Zostają trzy dziury i tylko jedna jest groźna: **proza cytująca komunikat kompilatora** wprost w treści strony, bo tej nie sprawdza nic i psuje się po cichu. Próg edycji (`--edition 2024`) pęka natychmiast i głośno, a niestabilne z założenia wyjścia (`{:?}`, formatowanie liczb zmiennoprzecinkowych) są rzadkie i pokażą się jako zagadkowy diff.

Samo lekarstwo to pięć linijek `rust-toolchain.toml` w korzeniu repozytorium, a decydują o nim dwa szczegóły, na których łatwo się przejechać. Po pierwsze, **to funkcja rustupa, nie Rusta**: wywołanie przechwytuje `shim` leżący na `PATH` — ten „`rustc`” to kilka kilobajtów, które czytają plik i uruchamiają prawdziwy kompilator. Wywołaj prawdziwe binarium ścieżką bezwzględną, a plik zostanie zignorowany w całości; przy rustupie z Homebrew shimy nie leżą zresztą w `~/.cargo/bin`, co warto wiedzieć, zanim zaczniesz debugować „nieszczące” przypięcie. Po drugie, **`channel = "stable"` nie przypina niczego** — śledzi kanał, więc dwie maszyny w odstępie miesiąca dostaną dwa kompilatory, a ty masz plik, który wygląda jak gwarancja i nią nie jest. Wpisz numer.

Cena też jest realna i wynika z rzeczy nieoczywistej: **rustup indeksuje toolchainy po nazwie, nie po rozwiązanej wersji**, więc przypięte `1.97.1` jest *osobną instalacją* obok `stable`, nawet gdy oba wskazują dokładnie ten sam kompilator. Stąd `profile = "minimal"` z jawnie wypisanymi komponentami: 657 MB zamiast 1,5 GB przy profilu domyślnym, a na CI, które niczego nie cache'uje między przebiegami, ta różnica pobiera się przy każdym zadaniu. Reszta kosztu to obowiązek, i ten obowiązek jest tu właśnie zaletą — toolchain rusza się dopiero wtedy, gdy ktoś zmieni tę jedną linijkę, a taka zmiana jest commitem z diffem, w którym nowy kompilator przechodzi po wszystkich kluczach odpowiedzi, zanim zobaczy je ktokolwiek inny. Nieprzypięty toolchain robi dokładnie to samo między twoimi commitami, tyle że nikt nie patrzy.

**Szukaj po polsku:** przypinanie wersji kompilatora · odtwarzalność buildów · `rust-toolchain.toml` · `rustup toolchain override` · `rustup profile minimal`
