...
Całą tę stronę da się streścić jednym zdaniem, które warto zapamiętać po polsku: **szablon konfigurację kopiuje, workspace ją współdzieli.** Słowa `workspace` nie tłumaczymy — to dosłownie klucz `[workspace]` w `Cargo.toml`, a szukanie „przestrzeni roboczej Cargo” po polsku i tak nic nie zwróci. Rzecz w tym, że przy czterdziestu folderach z ćwiczeniami odruchem jest napisanie skryptu, a wszystkie cztery pliki, które ten skrypt miałby kopiować, są już współdzielone z korzenia drzewa: `rust-toolchain.toml` obowiązuje w dół, bo rustup **idzie w górę** katalogów, szukając go; `members = ["exercises/*"]` to glob, więc nowy katalog staje się członkiem w chwili, gdy powstaje; `[workspace.lints.clippy]` dziedziczy każdy członek, który powie `workspace = true`; a — i to jest ten element, który najłatwiej przeoczyć i który ostatecznie unieważnia pomysł skryptu — **tę jedną linijkę zgody dopisuje samo `cargo new`**, gdy tylko zobaczy w korzeniu tabelę lintów. Rytuał zakładania ćwiczenia sprowadza się więc do `cargo new` i niczego więcej.

Z zależnościami działa ten sam mechanizm, ale z jedną celową asymetrią, o którą łatwo się potknąć: **linty dziedziczy się hurtem, zależności per crate.** `[workspace.dependencies]` w korzeniu to rejestr wersji, a nie automatyczne `use` — każdy członek nadal musi napisać `anyhow = { workspace = true }`, bo pakiet powinien deklarować to, czego faktycznie używa. Zysk jest taki, że wersja mieszka w jednym miejscu i żadne dwa ćwiczenia nie rozjadą się po cichu na dwie wersje tego samego crate’a. Przy nauce trzymaj tę listę krótko — sięgnięcie po `itertools`, zanim poczujesz, dlaczego sam `Iterator` nie wystarczał, uczy crate’a zamiast języka, a każda kolejna zależność to jeszcze jedna rzecz między tobą a komunikatem błędu.

Koszty są trzy i wszystkie drobne: `cargo run` wymaga teraz `-p` (`cargo run -p ex01_hello`) i to jedyny, który naprawdę czuć na co dzień; jest jeden `Cargo.lock`, więc wszyscy członkowie zgadzają się co do wersji `serde`; a pojedyncze ćwiczenie przestaje być przenośne — skopiuj folder poza drzewo, a straci toolchain, linty i będzie się odwoływać do workspace’u, którego tam nie ma. W zamian jest jeden wspólny `target/` zamiast czterdziestu. I dlatego `cargo generate` nie jest tu konkurencją: on rozwiązuje problem **treści** (dziewiętnaście plików szkieletu Leptosa, których nie napiszesz z pamięci), a nie **konfiguracji**. Gdybyś wygenerował czterdzieści projektów z szablonu, miałbyś czterdzieści kopii tych samych czterech plików — i po pierwszej zmianie zdania co do jednego linta zostałyby skamieliną tego, w co wierzyłeś w marcu.

**Szukaj po polsku:** przestrzeń robocza Cargo · `cargo workspace members` · `workspace.lints` · `cargo new workspace lints` · `cargo run -p`
