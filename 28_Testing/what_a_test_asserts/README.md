...
- **Java / C#.** JUnit's `assertEquals(expected, actual)` — note the argument order is the **opposite** of `assert_eq!(actual, expected)` by convention, which matters only for reading the failure output. std has nothing like AssertJ or FluentAssertions. The nearest crate is [`googletest` ↗](https://docs.rs/googletest), a matcher library modelled on GoogleTest for C++, which the Advanced Rust testing course in [the reading list](../resources/README.md) starts with.
