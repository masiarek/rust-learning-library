...
- [Comparing two numbers of different types](../../19_Numbers/comparing_two_number_types/README.md) — `-1i32 as u64 < 1u64` is `false`: why a comparison wants `From`, not `as`
...
Polskie materiały — i cała tradycja C oraz Javy — nazywają `as` **rzutowaniem** i traktują je jak zwykłą konwersję typów, stąd odruch, żeby napisać `n as u8` wszędzie tam, gdzie typ się nie zgadza. Kłopot w tym, że `as` **zawsze się udaje**: nie zwraca `Result`, nie panikuje, nie ostrzega, a mimo to traci dane na cztery sposoby. Zawężenie zostawia młodsze bity (`300u32 as u8` to `44`), zmiana znaku czyta te same bity na drugą stronę (`-1i32 as u32` to `4294967295`), liczba zmiennoprzecinkowa jest obcinana w stronę zera i **nasycana** na krańcu zakresu (`1e10f64 as i32` to `2147483647`, a `f64::NAN as i32` to `0`), w drugą zaś stronę zaokrągla się do najbliższej reprezentowalnej wartości (`16_777_217i64 as f32` to `16777216`). Żadne z tych zachowań nie jest błędem Rusta — wszystkie są zdefiniowane celowo, przy czym nasycanie jest zdefiniowane dopiero od Rusta 1.45.

Tu czeka pułapka na kogoś, kto zapamiętał z polskich kursów zdanie „w trybie debug Rust panikuje przy przepełnieniu”. To prawda o arytmetyce, ale nie o `as`: `300u32 as u8` jest **zdefiniowanym obcięciem**, a nie przepełnieniem arytmetycznym, więc nie panikuje w żadnym profilu kompilacji. Dla odmiany `250u8 + 10` panikuje w trybie debug i zawija się w release — to jedyne miejsce, w którym oba profile podają inną odpowiedź. Trzy nazwane metody wybierają zachowanie jawnie: `saturating_add(10)` daje `255`, `wrapping_add(10)` daje `4`, a `checked_add(10)` zwraca `None` — i tylko ta ostatnia oddaje decyzję wywołującemu.

Dwa błędy z tej strony warto zapamiętać razem, bo oba wyglądają na coś innego, niż są. Pierwszy: `(voted / eligible) as f64 * 100.0` przy `4` i `6` daje `0.0`, ponieważ dzielenie całkowite wykonało się **przed** rzutowaniem — `as` nie naprawi wartości, która już przepadła, a `f64::from(voted) / f64::from(eligible)` daje `66.7`. Drugi ma ostrzejszą krawędź: warunek `position < scores.len() as i32` dla `position = -1` przepuszcza, bo `-1 < 3` jest prawdą, a `position as usize` to wtedy `18446744073709551615` i indeksowanie panikuje. `usize::try_from(-1)` mówi to wprost — `Err(TryFromIntError(NegOverflow))` — a `.get(i)` po prostu zwraca `None`.

Reguła praktyczna jest krótka: **jeśli nie umiesz powiedzieć na głos, co `as` robi z wartością spoza zakresu, chodziło ci o `try_from`.** Przy rozszerzaniu pisz `u64::from(n)`, bo taki zapis przestanie się kompilować w dniu, w którym ktoś zmieni typ `n` z `u32` na `u64`; `n as f64` skompiluje się dalej i po cichu zacznie gubić precyzję powyżej 2⁵³. Zostaje uczciwy wyjątek: konwersje z założenia stratne, przy których `as` jest jedynym narzędziem — wtedy dopisuje się komentarz mówiący, że utrata danych jest zamierzona. A jeśli wolisz, żeby ktoś tego pilnował za ciebie, włącz w clippy regułę `cast_possible_truncation` i jej krewne — każde `as` zamienia się wtedy w ostrzeżenie, na które trzeba odpowiedzieć.

**Szukaj po polsku:** rzutowanie typów w Ruscie · konwersja typów · przepełnienie arytmetyczne · `rust as cast truncation` · `rust checked_add wrapping_add` · `rust clippy cast_possible_truncation`
