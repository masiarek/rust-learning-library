...
Obserwowalność (*observability*) i monitorowanie (*monitoring*) to po polsku dwa różne słowa — i całe szczęście, bo różnica z drugiego akapitu tej strony polega dokładnie na tym. Monitorowanie odpowiada na pytanie zadane **z góry**: jaki jest odsetek błędów, jak długa jest kolejka, ile zżera procesor. Obserwowalność ma pozwolić odpowiedzieć na pytanie zadane **dopiero w trakcie awarii**, o jedno żądanie na milion. Polskie materiały często zlewają oba pojęcia w jeden „monitoring”, więc szukając wyjaśnienia tej różnicy, od razu wpisuj angielskie `observability vs monitoring`.

Druga rzecz, specyficzna dla tego rozdziału i warta uwagi, zanim zaczniesz czytać kolejne strony: **tutaj nic nie kompiluje się tak jak w reszcie tej biblioteki.** Każdy sprawdzany przykład w tym repozytorium to jeden plik `.rs` budowany przez `rustc` bez żadnych zależności, a obserwowalność w Ruscie jest historią o crate'ach — `tracing`, `opentelemetry`, eksporter, a pod tym wszystkim zwykle jeszcze środowisko asynchroniczne. Dlatego każda strona dzieli się tu na dwie połowy. **Mechanizm** — span jako struktura z identyfikatorem rodzica, nagłówek `traceparent` jako 55 bajtów, które trzeba sparsować i umieć odrzucić, kardynalność jako iloczyn, który da się policzyć — jest modelowany w bibliotece standardowej i sprawdzany jak każdy inny przykład. **API**, czyli wywołanie `tracing`, które naprawdę napiszesz, siedzi w bloku `text` opisanym wprost jako nieskompilowane, z odsyłaczem do dokumentacji crate'a. Zamodelowane drzewo spanów uczy drzewa i nie jest żadnym dowodem na temat `tracing` — to rozróżnienie warto zapamiętać, bo strony go nie ukrywają.

Słownictwo, które lepiej ustalić od razu, bo wraca potem na każdej stronie: `trace` to **ślad** (całe żądanie przez wszystkie usługi), `span` w polskiej praktyce zostaje **spanem** — tłumaczenia w rodzaju „przedziału” nikt nie używa, a w Jaegerze czy Grafanie i tak zobaczysz „span”; `log` to po prostu log, `metric` to **metryka**. Angielskie nazwy są tu operatywne podwójnie: mówi nimi nie tylko `rustc`, ale i interfejsy narzędzi, w których te dane potem oglądasz.

**Szukaj po polsku:** obserwowalność systemów · ślad rozproszony · metryki i logi · `rust tracing crate` · `observability vs monitoring` · `opentelemetry rust`
