...
**Split off the header.** Write `fn take_header<'a>(input: &mut &'a [u8], n: usize) -> Option<&'a [u8]>`: it returns the first `n` bytes and moves the caller's slice past them, or, when there are fewer than `n`, returns `None` and leaves the caller's slice as it was. Then write its twin over `&mut &'a mut [u8]`, returning `Option<&'a mut [u8]>`, take two headers off one buffer, and write into both while both are held. Print the twin's slice length after a `None`: one natural way to write it compiles and leaves the caller with an empty slice.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:repointing_a_slice_kata -->
*[`repointing_a_slice_kata.rs`](examples/repointing_a_slice_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:repointing_a_slice_kata -->
*Verified output of [`repointing_a_slice_kata.rs`](examples/repointing_a_slice_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. take_header on &mut &[u8]
   take 2, take 2                 -> Some([CA, FE]), Some([00, 02])
   rest                           -> [68, 69]
   take 9                         -> None, rest still [68, 69]

2. take_header_mut on &mut &mut [u8]
   take 2, take 2, write both     -> view left 2 long
   take 9                         -> None, view still 2 long
   buf                            -> [CA, FE, 00, 02, 68, 69]

3. The early `?` after mem::take
   take 9                         -> None, view now 0 long
```
<!-- /output -->
