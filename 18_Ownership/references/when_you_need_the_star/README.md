...
**Fewest stars.** Each marked line is written with no `*` and no borrowing `&`. Add the fewest that make it compile — none is a possible answer — change nothing else on the line, and name the rule from [the whole table](#the-whole-table) that decides it. With `flags` `[true, false, false]`, `readings` `[4, -2, 9]` and `limit` `7`, what does each line produce?
...
fn report(flags: &[bool], readings: &[i32], limit: &i32, lamp: &mut bool) {
    ...
    if first { println!("taken"); }                            // (a)
    let over = peak > limit;                                   // (b)
    let high = readings.iter().filter(|n| n > limit).count();  // (c)
    let dips = readings.iter().any(|n| n.is_negative());       // (d)
    lamp = first | dips;                                       // (e)
}

// in main: flags and readings are Vecs, limit an i32, lamp a bool
report(flags, readings, limit, lamp);                          // (f)
```

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:when_you_need_the_star_kata -->
*[`when_you_need_the_star_kata.rs`](examples/when_you_need_the_star_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:when_you_need_the_star_kata -->
*Verified output of [`when_you_need_the_star_kata.rs`](examples/when_you_need_the_star_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
(a) if *first                      -> taken
    rule: a condition wants `bool`
(b) peak > limit                   -> true
    rule: same depth, &i32 > &i32
(c) filter(|n| *n > limit).count() -> 1
    rule: filter adds a `&`; then same depth
(d) any(|n| n.is_negative())       -> true
    rule: the dot
(e) *lamp = first | dips           -> *lamp = true
    rule: `*lamp` is the place; `|` has an impl for &bool
(f) report(&flags, &readings, &limit, &mut lamp)
    rule: deref coercion, &Vec<bool> -> &[bool] and &Vec<i32> -> &[i32]

stars in report: 3 (a, c, e); lamp is now true
```
<!-- /output -->
