...
(There is a sixth, and it is missing here because it is not a scope event at all: an **assignment** frees whatever the location was holding, so a value can die on a line with no brace in sight — [Assignment drops the old value](../assignment_is_a_drop/README.md).)
