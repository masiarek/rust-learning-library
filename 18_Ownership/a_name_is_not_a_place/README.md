...
And it is not a `Copy` trick — a mistake this repo has [a whole page about](../../17_Option_and_Result/shadowing_and_unwrap/README.md), because tutorials routinely credit shadowing for what `Copy` is quietly doing. The same shape works on a `String`, which is not `Copy` at all:
...
- [Shadowing and `unwrap`](../../17_Option_and_Result/shadowing_and_unwrap/README.md) — the `Copy` confusion this page's `String` example is guarding against
