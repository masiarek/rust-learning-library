...
| [References: the map](references/README.md) | 101 → 301 | Every page on `&`, `&mut`, `ref`, `*` and raw pointers in reading order, plus six pages that were missing: the loan model the borrow checker enforces, when you write the `*` yourself, the `ref` keyword, `&mut &mut [T]`, six pointer types in one table, and claims from Stack Overflow and books, run |
