...
| [Printing borrows](printing_borrows/README.md) | 101 | Why `println!("{s}")` twice is not `E0382`: every formatting macro is handed `&s`, and rustc's own expansion shows the `&` — the moves come from a by-value parameter, an argument like `s + "!"`, or `dbg!` |
...
| [Returned by value](returned_by_value/README.md) | 201 | A returned value lands in room the caller reserved before the call, so its type needs one size — measured as registers and an `sret` pointer in LLVM IR, and the reason `-> str` is `E0277` and `str` is never `Clone`. Companions: [every error](returned_by_value_errors/README.md), [lints](returned_by_value_lints/README.md), [reading](returned_by_value_resources/README.md) |
| [Drawing `sret`](drawing_the_return_slot/README.md) | 201 → 301 | The hidden return pointer in three panels: the caller reserves the room and passes its address in `rdi`, the callee writes into it and hands the address back in `rax`. Written out by hand as a `&mut` and a `MaybeUninit` out-parameter, compared in assembly, and watched in lldb |
...
| [References: the map](references/README.md) | 101 → 301 | Every page on `&`, `&mut`, `ref`, `*` and raw pointers in reading order, plus six pages that were missing: the loan model the borrow checker enforces, when you write the `*` yourself, the `ref` keyword, `&mut &mut [T]`, six pointer types in one table, claims from Stack Overflow and books run, every error and lint on the path with its fix, and a kata on each new lesson |
...
| [Borrowing something forever](borrowing_forever/README.md) | 201 → 301 | `&'a mut Node<'a>` names two lifetimes with one name, and invariance stops the compiler shrinking it — so after one call the node cannot be printed, borrowed, moved or even dropped. Two names fix it; no lint notices |
| [A struct that points into itself](self_referential_structs/README.md) | 201 → 301 | Safe Rust builds one three ways (quinedot's "only" way is not the only one), and none can move, be returned, mutate or have a `Drop` — so store a range. Companions: [every error](borrowing_forever_errors/README.md), [lints](borrowing_forever_lints/README.md), [reading](borrowing_forever_resources/README.md) |
