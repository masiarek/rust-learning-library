...
- **ABAP.** Parameter passing already makes a version of this distinction visible, which makes the bridge unusually direct: on a method `VALUE(iv_x)` copies and `REFERENCE(iv_x)` does not, a bare `iv_x` means `REFERENCE(iv_x)`, and an `IMPORTING` parameter passed by reference cannot be changed in the method ([`METHODS` parameters ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapmethods_parameters.htm)). `CHANGING` says the caller will see writes — straight away when it is passed by reference, and only once the method has ended without errors when it is `VALUE(iv_x)` ([pass by value ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenpass_by_value_glosry.htm)). What ABAP cannot express is the part this page is about — *which* of several passed-by-reference parameters the returned value points into, and for how long. In practice that is handled by convention and by not keeping returned references around. Rust makes it a type, and the payment is that you sometimes have to write `'a` and `'b` where ABAP would have said nothing at all.
