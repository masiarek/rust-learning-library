...
- [`bin()` is not the bits ↗](https://masiarek.github.io/python-learning-library/01_Text_and_Bytes/bin_is_not_the_bits/index.html) — Trap 3 from Python's side: `bin(-9)` is `'-0b1001'`, a sign and a magnitude, because a Python `int` has no width to show
...
- [Listing 2.4, run](../../10_Resources/rust_in_action/non_base2/README.md) — *Rust in Action*'s base 2, 8 and 16 listing, checked, with `{:#06x}` and `from_str_radix` beside it
