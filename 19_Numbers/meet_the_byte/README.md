...
`u8` to nie „mała liczba”, tylko jeden bajt — jednostka, w której `size_of` liczy wszystko inne. W Ruscie nie istnieje typ znaczący po prostu „liczba”: szerokość siedzi w nazwie typu (`u8` — bez znaku, osiem bitów; `usize` — tyle, ile na tej maszynie ma wskaźnik) i język pyta o nią w pierwszej linijce, którą napiszesz. Pamięć adresowana jest bajtami, nie bitami, i można to sprawdzić zamiast przyjąć na wiarę: różnica adresów dwóch sąsiednich elementów tablicy `[u8; 3]` wynosi zawsze 1. Dlatego „odczytaj bit 3” nigdy nie jest pojedynczą instrukcją — pobierasz cały bajt i maskujesz. Rust nie zostawia też furtki na bajt innej szerokości: C ma na to `CHAR_BIT`, Rust nie ma niczego.

Najważniejsza konsekwencja dla polskiego czytelnika jest taka: `.len()` na łańcuchu znaków liczy **bajty**, a nie znaki, bo `String` i `&str` są zawsze w UTF-8. To nie ciekawostka o emoji — każda polska litera z diakrytykiem zajmuje w UTF-8 dwa bajty, więc `"łódź".len()` to **7**, podczas gdy `"łódź".chars().count()` to 4. Rust nie pozwala udawać, że jest inaczej: indeksowanie łańcucha zwykłą liczbą w ogóle się nie kompiluje (`E0277`), a wycinek do przesunięcia bajtowego, które wypada w środku znaku, kompiluje się i panikuje, wskazując przeciętą literę — `&"łódź"[0..1]` daje `end byte index 1 is not a char boundary; it is inside 'ł' (bytes 0..2 of string)`. Gdy chodzi o znaki, trzeba powiedzieć `.chars()`; gdy chcesz sprawdzić przed cięciem, jest `is_char_boundary`.

Z tym wiąże się pułapka nazewnicza, na którą nabiera się każdy przychodzący z C: tam `char` **jest** bajtem, a w Ruscie `char` to skalarna wartość Unicode i zajmuje **cztery** bajty. Jeśli chcesz bajt, mówisz to osobną składnią — `b'F'` to `u8` o wartości 70, `'F'` to `char`, `b"Sw"` to `&[u8; 2]`, a `"Sw"` to `&str`. Warto przy okazji zobaczyć, czego `size_of` nie robi: `&str` ma 16 bajtów, `String` 24, i w żadnej z tych liczb nie ma ani jednej litery tekstu — tekst leży na stercie, a `size_of` nigdy nie idzie za wskaźnikiem.

Przy przesunięciach polska terminologia jest wyraźniejsza od angielskiej. Przesunięcie w prawo bywa **logiczne** (dopełnia zerami od góry) albo **arytmetyczne** (dopełnia bitem znaku); w Javie odpowiadają im dwa różne operatory, `>>>` i `>>`. W Ruscie `>>>` nie ma i nie jest potrzebny, bo wybiera typ: `253u8 >> 1` to 126, a `(-3i8) >> 1` to -2. Wybór znakowości zmiennej **jest** wyborem rodzaju przesunięcia i kompilator nie może trafić źle. Tak samo `!b` na `u8` zostaje ośmiobitowe, bez rytualnego `& 0xFF`, które dopisuje się w językach o liczbach bez szerokości — tutaj maską jest typ.

Rachunek za tę szerokość płaci się przy przepełnieniu (*overflow*). `255u8 + 1` panikuje w kompilacji debug i daje 0 w release; jedno i drugie jest zachowaniem udokumentowanym, nie niezdefiniowanym, ale wniosek jest niewygodny: błąd przepełnienia potrafi przejść wszystkie twoje testy i zawinąć się dopiero na produkcji, bo obie kompilacje nie zgadzają się ze sobą. Dlatego tam, gdzie szerokość faktycznie stoi pod znakiem zapytania, nie pisze się gołego `+`, tylko mówi wprost, o co chodziło: `wrapping_add`, `checked_add`, `saturating_add`, `overflowing_add`. Zadanie na końcu strony pokazuje ostrzejszy wariant tej samej rzeczy — `1u8 << 8` panikuje w debug, a w release liczba przesunięć jest maskowana modulo 8, więc wychodzi `1u8 << 0` i głos dziewiątego kandydata po cichu ląduje na pierwszym. Na koniec: pojedynczy bajt nie ma kolejności, ma ją dopiero liczba złożona z kilku, i wtedy trzeba powiedzieć, czy pierwszy idzie bajt najstarszy, czy najmłodszy — to jest kolejność bajtów (*endianness*), więc sięgaj po `to_be_bytes` i `to_le_bytes` na każdej granicy (plik, protokół, suma kontrolna), bo `to_ne_bytes` w formacie wymiany jest błędem przenośności.

**Szukaj po polsku:** bajt · przepełnienie arytmetyczne · polskie znaki w UTF-8 · `rust str len bytes not chars` · `rust integer overflow debug release`
