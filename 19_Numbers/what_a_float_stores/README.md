...
**Exact decimal arithmetic** when the domain is money or law: the `rust_decimal` crate — [Decimals](../other_number_types/decimal_numbers/README.md) runs it beside Python's `Decimal` — or scale the denominators away. This repo has a whole page on the third option — [Scale the denominator away](../../09_Advanced/scaled_integers/README.md) shows a real proportional count where the float difference is 8.9e-16 and the consequence is that a tie stops being a tie.

## If you are coming from another language

**Python.** A Python `float` *is* an `f64` — the same hardware type, the same answers, and `Decimal(0.1)` prints all 55 digits of what was really stored. Two things change. Python has no `f32` at all (`numpy.float32` or `struct` if you need it), where Rust makes the width a choice you write down. And Python lets you `sorted()` a list of floats, put one in a `dict` key, and `==` two computed values with no complaint — the discipline is entirely yours. Rust moves that discipline into the type system. What Python has that Rust does not: `decimal.Decimal` in the standard library, where Rust sends you to a crate — [Other number types](../other_number_types/README.md) maps which one.
...
**Comparing a float with an integer**, and choosing the type a mixed comparison happens in, is [Comparing two numbers of different types](../comparing_two_number_types/README.md). *Rust in Action*'s section on the same hazards, including an `EPSILON` example whose difference is exactly zero, is checked claim by claim on [its own page](../comparing_numbers_claims_checked/README.md).
