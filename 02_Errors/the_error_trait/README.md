...
`std::error::Error` prosi o zaskakująco niewiele: dwie cechy nadrzędne (*supertraits*) — `Debug` i `Display` — oraz jedną metodę `source()`, która ma wartość domyślną. Dlatego `impl Error for QuorumNotMet {}` z **pustym ciałem** jest pełnoprawną implementacją, a nie zaślepką: wszystko, czego cecha (*trait*) wymaga, było już na miejscu. Dla kogoś przychodzącego od wyjątków bywa to zaskoczeniem, bo klasa bazowa `Exception` daje maszynerię, bez której nie ma ani `raise`, ani `except`. Tutaj maszyneria siedzi w `Result` i w `?`, a `impl Error` jest bliższe znacznikowi: mówi „ten typ ma podróżować jako awaria” i to on pozwala schować go w `Box<dyn Error>`.

Dwie cechy nadrzędne to dwie publiczności, a nie formalność. `Display` czyta użytkownik twojego programu, `Debug` czytasz ty — i warto zapamiętać, że to właśnie **`Debug`** wypisują `unwrap`, `expect` oraz środowisko uruchomieniowe przy `Err` wychodzącym z `main`. Starannie ułożone zdanie z `Display` nie trafi więc do użytkownika, jeśli sam go nie wypiszesz. Napisanie tylko jednej z dwóch nie jest skrótem, tylko `E0277` — kompilator cytuje ci z powrotem definicję `pub trait Error: Debug + Display` i pokazuje, które ograniczenie nie zostało spełnione.

`source()` jest tą jedną metodą, od której zależy jakość całej obsługi błędów. Domyślne `None` znaczy „nic tego nie spowodowało” i czasem jest prawdą — ale gdy twój błąd opakował inny, nadpisanie `source()` kosztuje cztery linijki, a ich brak sprawia, że przyczyna leży sobie w polu struktury i jest **nieosiągalna**: czytelnik dowiaduje się, *co* się stało, i nigdy, *dlaczego*. To najczęstsza usterka w ręcznie pisanych typach błędów. Z samego łańcucha wynika też reguła, która wygląda na kwestię gustu, a nią nie jest: `Display` każdej warstwy mówi wyłącznie to, co wiedziała **ta** warstwa. Gdyby komunikat zewnętrzny brzmiał „nie udało się odczytać ballots.txt: nie ma takiego pliku”, łańcuch wypisałby całe to zdanie, a zaraz potem powtórzył jego drugą połowę. I jeszcze jedno, czego biblioteka standardowa nie daje: stabilnego iteratora po przyczynach nie ma (`Error::sources()` czeka od 2018 roku), więc pętlę wypisującą łańcuch pisze się samemu — cztery linijki warte trzymania w każdym programie.

Ostatnia sprawa jest dla polskiego programisty najbardziej praktyczna, a na angielskiej stronie widać ją tylko kątem oka: `Display` to **zaszyte w kodzie zdanie**, a nie identyfikator komunikatu. W ABAP-ie tekst wyjątku pochodzi z klasy komunikatów `T100` — numer, tłumaczenia, utrzymanie poza kodem — i `MESSAGE e001(zvote) WITH lv_path` wypisze się w języku zalogowanego użytkownika. Rust nie ma w standardzie żadnego odpowiednika: żeby przetłumaczyć komunikat, potrzebujesz crate'a i własnego wyszukiwania po kluczu. Skutek praktyczny jest taki, że łańcuch błędów w polskiej aplikacji łatwo robi się dwujęzyczny — twoja warstwa po polsku nad `no such file or directory` z `io::Error` — więc lepiej zdecydować o tym świadomie, na przykład trzymając wszystkie warstwy po angielsku i tłumacząc dopiero przy wypisywaniu. Konwencji zapisu trzymaj się przy tym także po polsku (małą literą, bez kropki na końcu, bez przedrostka `Error:`): ona nie wynika z angielskiej ortografii, tylko z zagnieżdżania, bo komunikat niosący własny prefiks zamienia się przy pierwszym opakowaniu w `error: Error: Kworum nieosiągnięte.: nie ma takiego pliku`.

**Szukaj po polsku:** własny typ błędu · łańcuch przyczyn błędu · tłumaczenie komunikatów w aplikacji · `rust std::error::Error source` · `rust api guidelines error types` · `rust Error::sources unstable`
