...
Pierwsza rzecz, która myli, dotyczy tego, **gdzie** kompilator postawi znacznik: gdy zadeklarujesz `-> Result<u32, ParseIntError>` — czyli typ ostatniej rzeczy, która może zawieść, i to jest naturalny pierwszy strzał — kompilator pokaże `E0277` przy **`?`**, a nie przy wywołaniu `read_sheet`. Nie ma w tym niekonsekwencji: `read_sheet` oddało dokładnie to, co obiecało, a to `?` jest operatorem, który po drodze wykonuje konwersję przez cechę (*trait*) `From`. Komunikat mówi więc, że brakuje konwersji, której nigdy nie napisałeś — i oba dalsze rozwiązania sprowadzają się do jej dostarczenia.

Dwie drogi mają po polsku wygodne nazwy: różnicę można **wymazać** albo **nazwać**. Wymazanie to `Box<dyn Error>` — kompiluje się w chwili, gdy je napiszesz, bo każdy typ implementujący `Error` konwertuje się do niego jedną zbiorczą implementacją (*blanket impl*), i będzie się kompilować dalej, gdy dołożysz kolejne źródła awarii. Uwaga na fałszywego przyjaciela: „wymazywanie typów” polski czytelnik zna z Javy jako artefakt typów generycznych, o którym nikt nie decyduje; tutaj jest to świadomy wybór projektowy i jego cenę widać gołym okiem. `downcast_ref` odzyskuje konkretny typ, ale jest **testem typu w czasie działania** — jednym na każdego kandydata, który przyjdzie ci do głowy, a kompilator nie sprawdzi, czy skończyłeś listę.

Nazwanie to wyliczenie (*enum*), i tu pada jedyne zdanie z tej strony, które warto zapamiętać dosłownie: **jeden wariant na decyzję wywołującego, a nie jeden na typ błędu pod spodem.** To jest odruch do odwrócenia, bo hierarchie wyjątków w Javie czy w ABAP-ie buduje się zwykle według *źródła* awarii — klasa na każdy rodzaj rzeczy, która pękła. Tutaj pytanie brzmi inaczej: czy wywołujący zrobi z tym coś innego? Zacznij od zera, ponów, pomiń wiersz, przerwij — cztery odpowiedzi, cztery warianty. Dwie implementacje `From` załatwiają resztę: `?` znajduje je po typie, więc ciało `tally` nie zmienia się ani o znak, a `source()` przechowuje błąd, z którego wariant powstał, więc łańcuch przyczyn przeżywa przemianowanie.

Na koniec pułapka wersjonowania, szczególnie dotkliwa dla przychodzących od wyjątków. Dorzucenie podklasy do hierarchii wyjątków jest bezpieczne, bo wywołujący łapią po nadklasie. Dorzucenie wariantu do **publicznego** wyliczenia jest zmianą łamiącą zgodność (*breaking change*) właśnie dlatego, że wywołujący dopasowują wyczerpująco — to samo, co czyni `match` tak wygodnym wewnątrz twojego crate'a (kompilator obejdzie z tobą każde miejsce, które ma decyzję do przemyślenia), staje się kłopotem poza nim. `#[non_exhaustive]` odkupuje zachowanie znane z wyjątków, ale musi stać tam od pierwszego wydania. Reszta mieści się w jednym zdaniu: **biblioteka nazywa, program wymazuje** — a typowy projekt robi jedno i drugie, każde po swojej stronie.

**Szukaj po polsku:** własny typ błędu · hierarchia wyjątków a wyliczenie · `rust Box<dyn Error>` · `rust error enum From impl` · `rust non_exhaustive breaking change`
