...
Dla polskiego czytelnika ta półka ma jedną konkretną pozycję i warto ją znaleźć od razu: **Tour of Rust** jest jedyną książką na całej stronie przetłumaczoną na polski. Haczyk, o którym angielska wersja nie musi wspominać: przekład obejmuje rozdziały 1–5 — podstawy, kontrolę przepływu, struktury, typy generyczne oraz własność i pożyczanie — a od rozdziału 6 (tekst, cechy, inteligentne wskaźniki, moduły) polskie strony pokazują po prostu angielski oryginał. Werdykt ze strony obowiązuje bez zmian i jest tu ważniejszy niż gdziekolwiek indziej: to *wycieczka*, czyli materiał **przed** The Book, a nie zamiast niej — pokazuje funkcję i idzie dalej, bez ćwiczeń i bez tłumaczenia, dlaczego coś wygląda tak, a nie inaczej. Realistyczna ścieżka wygląda więc tak: Tour of Rust po polsku, żeby oswoić słownictwo i zobaczyć kształt języka, a potem The Book po angielsku.

Druga pozycja napisana dosłownie z myślą o tobie to **Easy Rust**: Dave MacLeod ułożył ją w celowo ograniczonym słownictwie angielskim, dla czytelników, dla których angielski nie jest pierwszym językiem — i nic innego na tej półce tego nie robi. Zastrzeżenie ze strony obowiązuje (ostatnia większa aktualizacja w 2021, część idiomów się przesunęła, płatnym kontynuatorem jest *Month of Lunches*), ale jako miejsce do wyrobienia sobie nawyku czytania technicznego angielskiego jest najtańsze, jakie tu znajdziesz. W drugą stronę ostrzeżenie: **Too Many Linked Lists**, czyli najwyżej ocenioną darmową pozycję po samej The Book, napisano bardzo idiomatycznie i żartobliwie — jest znakomita merytorycznie i jednocześnie językowo najtrudniejsza na tej stronie. Jeśli będzie się czytać ciężko, nie odbieraj tego jako miary swojego Rusta.

Tabela werdyktów bywa czytana po polsku jak ranking i to jest błąd, na który warto się uodpornić od razu. **Reference** nie znaczy „gorsza niż Read it”, tylko „do zaglądania, nie do czytania od deski do deski”; **Only if X** znaczy „świetna dla jednej osoby na dziesięć, dla reszty strata czasu”; a **Unvetted** znaczy „nikt tutaj tego nie czytał i nie udaje, że czytał” — nie „słaba”. Strona rozdziela też dwie rzeczy, które polskie zestawienia książek zwykle zlewają w jedno: **fakty** (wydawca, rok, liczba stron, cena) są sprawdzone u wydawcy w podanej dacie, a **werdykty** są opinią — i jest to napisane wprost, zamiast udawać, że jedno i drugie waży tyle samo.

Na koniec dwie pułapki, które po polsku wyglądają inaczej niż po angielsku. Po pierwsze, **data polskiego wydania nie jest datą książki**: przekład ukazuje się długo po oryginale, więc pozycja z sekcji „dobre kiedyś, dziś przestarzałe” — katalog z lat 2017–2019, sprzed `async`/`await`, NLL i dwóch edycji języka — potrafi trafić do sprzedaży ze świeżo wyglądającą datą na okładce. Sprawdzaj rok i edycję oryginału, nie polskiej wersji. Po drugie, polskie „rankingi najlepszych książek o programowaniu” mają dokładnie tę wadę, którą ta strona wytyka najczęściej linkowanemu angielskiemu zestawieniu: autor stawia na pierwszym miejscu własne książki, a listy afiliacyjne polecają po kolei wszystko. Sygnał ostrzegawczy jest ten sam w każdym języku — na uczciwej liście któraś pozycja jest postawiona *przeciwko* innej. Trzy darmowe sprawdziany ze strony działają bez zmian: czy stoi za tym wydawca, czy kod leży publicznie i daje się skompilować dzisiejszym `cargo`, i czy ktokolwiek gdziekolwiek o tej książce w ogóle rozmawia.

**Szukaj po polsku:** darmowe książki o Ruscie · Tour of Rust po polsku · Rust dla początkujących · `Easy Rust` · `too many linked lists` · `rust for rustaceans`
