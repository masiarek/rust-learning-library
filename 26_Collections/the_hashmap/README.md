...
Tę strukturę polskie materiały nazywają „tablicą mieszającą” albo „mapą asocjacyjną”, a osoby przychodzące z Pythona mówią po prostu „słownik” — w kodzie i w wyszukiwarce i tak zawsze `HashMap`. Najważniejsza rzecz na tej stronie nie ma polskiej nazwy w ogóle, bo nie ma odpowiednika ani w Pythonie, ani w ABAP-ie: to API `entry`. Pętla zliczająca wygląda tak: `*tally.entry(name).or_insert(0) += 1;` — `entry` oddaje referencję mutowalną do wartości, wstawiając najpierw wartość domyślną, jeśli klucza nie było. To **jedno** przeszukanie tablicy, podczas gdy odruch przyniesiony z innych języków, `if map.contains_key(k) { … } else { … }`, kosztuje dwa albo trzy. Kluczem może być typ spełniający `Eq + Hash`, a umowa między tymi dwiema cechami (*traits*) jest istotą rzeczy: **dwa klucze równe sobie muszą mieć ten sam skrót**. Dlatego `f64` kluczem nie bywa — `NaN != NaN` łamie `Eq`.

Dalej dwie pary, których nie wolno pomylić. Pierwsza: `get` pyta, `[]` twierdzi — `map.get(k)` zwraca `Option`, a `map[k]` przy braku klucza panikuje. `map[k]` jest przy tym **miejscem**, a nie referencją, więc `println!("{}", map[k])` przejdzie dla dowolnego `V`, ale `let v: String = map[k];` to już `E0507`, *„cannot move out of index of `HashMap<&str, String>`”*. Druga para: `insert` **nadpisuje** i oddaje poprzednią wartość jako `Option` — i to jest jedyne ostrzeżenie, jakie dostaniesz. Zignorowanie go jest najczęstszym sposobem na ciche zgubienie danych: w ćwiczeniu na tej stronie Cara zdobywa 5 + 4 + 2 = 11, a pętla napisana na `insert` zamiast na `entry` pokazuje przy niej 2 i niczego nie zgłasza.

Na koniec pułapka, która najmocniej uderza właśnie w osoby po Pythonie, bo polskie kursy Pythona od wersji 3.7 uczą wprost, że słownik **zachowuje kolejność wstawiania**. `HashMap` w Ruscie kolejności nie ma i mieć nie chce: hasher dostaje losowe ziarno przy każdym uruchomieniu procesu (obrona przed atakiem HashDoS), więc dwa uruchomienia tego samego programu wypiszą wpisy w innej kolejności. Praktyczna reguła jest krótka — cokolwiek wypisujesz albo porównujesz, najpierw posortuj, albo weź `BTreeMap`, uporządkowany po kluczu z definicji. Najgorszy przypadek tego zjawiska nie wygląda jak błąd: przy remisie `max_by_key` zwraca ten z równych wpisów, do którego iterator dotarł jako do ostatniego, więc program wyłaniający zwycięzcę z `HashMap` bez jawnego rozstrzygnięcia remisu jest niedeterministyczny — i przejdzie każdy test, jaki mu napiszesz.

**Szukaj po polsku:** tablica mieszająca · słownik w Ruscie · `rust HashMap entry or_insert` · `rust HashMap random iteration order HashDoS` · `rust BTreeMap vs HashMap`
