...
Siatkę (*grid*) da się w Ruscie trzymać na trzy sposoby, a po polsku łatwo je pomylić, bo słowo „tablica” obsługuje wszystkie trzy. `[[u8; 4]; 4]` to prawdziwa tablica dwuwymiarowa — jeden blok pamięci, długość zapisana w typie, **zero** alokacji. `vec![vec![0u8; 4]; 4]` to wektor wektorów: **pięć** alokacji, po jednej na wiersz plus wektor zewnętrzny, a przy tysiącu wierszy już 1001. `vec![0u8; 16]` z osobno trzymaną szerokością to **jedna** alokacja i indeks liczony ręcznie jako `r * width + c`. Te tysiąc dodatkowych alokacji kupuje dokładnie jedną rzecz: wiersze o różnej długości, czego żadna z pozostałych postaci w ogóle nie wyrazi. I jeszcze drobiazg, który łatwo zapisać w notatkach źle: `println!("{grid:?}")` wypisuje całą siatkę w **jednej** linii, a `{:#?}` nie jest skrótem do obrazka, bo łamie wiersz po każdym elemencie z osobna — kwadrat cyfr na ekranie zawsze pochodzi z pętli.

Pułapka, na której potyka się większość osób, siedzi w pętli zapisującej. `iter_mut()` zwraca `&mut u8`, więc `cell` jest referencją mutowalną, a nie liczbą — samo `cell = 1` przestawiłoby referencję i kończy się błędem `E0308` z komunikatem *expected `&mut u8`, found integer* oraz podpowiedzią *help: consider dereferencing here*. Zapis wymaga dereferencji: `*cell = 1`. Podchwytliwy jest tu brak symetrii — bliźniacza pętla, która tylko czyta, nie ma żadnej gwiazdki, bo `println!("{cell}")` sięga przez referencję samo, dzięki `Display`. Obie pętle wyglądają tak samo i takie nie są.

`vec![elem; n]` wymaga `Clone` i tworzy *n* niezależnych wartości — to zdanie warto zapamiętać, jeśli przychodzisz z Pythona. Tam `[[0] * 4] * 4` zapisuje **tę samą** listę cztery razy, więc `grid[0][0] = 9` widać we wszystkich wierszach naraz; polskie kursy Pythona wymieniają to jako klasyczny błąd początkującego. W Ruscie nie da się tego napisać przez przypadek: współdzielenie jednego wiersza między wierszami wymaga `Rc<RefCell<Vec<u8>>>` i widać je w typie. Bezpiecznym pythonowym odpowiednikiem jest wersja ze składaną listą, `[[0] * w for _ in range(h)]`, nigdy gwiazdka.

Płaski wektor ma za to własny koszt, o którym łatwo zapomnieć, bo nie objawia się paniką: szerokość podróżuje osobno i nikt jej nie sprawdza. `set(0, 5, 9)` na siatce o szerokości 3 wylicza indeks 5, czyli istniejącą komórkę — wiersz 1, kolumna 2 — i pisze tam po cichu. Wektor wektorów dostaje ten test za darmo, bo każdy wiersz zna własną długość. Jeśli wybierasz postać płaską, zamknij ją w strukturze trzymającej szerokość i dołóż `assert!`, w przeciwnym razie zamieniasz `Vec<Vec<T>>` na klasę błędów, które nigdzie się nie zgłaszają. Do odzyskiwania wierszy służy `chunks(width)` — oddaje wycinek na wiersz, bez kopiowania i bez alokacji.

**Szukaj po polsku:** wektor wektorów · tablica dwuwymiarowa w Ruscie · dereferencja referencji mutowalnej · `rust vec of vecs allocations` · `rust iter_mut E0308 expected &mut` · `rust flat vec grid r * width + c`
