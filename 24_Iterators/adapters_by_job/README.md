...
Ta tabela jest w gruncie rzeczy słownikiem: „zadanie, które mam” → „nazwa metody, której szukam”. I właśnie dlatego tłumaczenie tych nazw na polski szkodzi bardziej, niż pomaga — kiedy `filter` i `take_while` opiszemy jako „filtruj” i „bierz dopóki”, różnica między nimi zniknie w pół zdania, a to jest różnica w wynikach, nie w stylu. Na `[5, 3, 0, 4, 2, 1]` warunek „mniejsze niż 5” daje z `filter` całe `[3, 0, 4, 2, 1]`, bo `filter` sprawdza **każdy** element — a z `take_while` pustą listę, bo `take_while` zatrzymuje się na **pierwszym** elemencie, który warunku nie spełnia, a piątka stoi na samym początku. Na danych posortowanych to zaleta i oszczędność; na nieposortowanych dostajesz prefiks, który wygląda wiarygodnie, i to jest najgorszy rodzaj błędu.

Druga pułapka nie ma odpowiednika w Pythonie, więc łatwo w nią wejść: `flat_map` nad `Result`em **kasuje błędy**. `Result` jest iteratorem o długości 0 albo 1, więc `rows.iter().flat_map(|s| s.parse::<i32>())` kompiluje się i czyta się dobrze — tyle że cztery wiersze na wejściu dają trzy na wyjściu, w tym samym typie i o tym samym kształcie co wynik udany, i nic nigdzie nie mówi, że jeden wiersz zniknął. Jeśli „pomiń to, czego nie da się sparsować” faktycznie jest wymaganiem, to narzędzie w sam raz. Jeśli nie — zbieraj do `Result` i pozwól pierwszemu błędowi zatrzymać łańcuch.

Trzecia i czwarta pułapka to raczej zdziwienia niż błędy. `peek` bierze `&mut self`, choć niczego nie konsumuje: musi pobrać element i go potrzymać, więc iterator trzeba zadeklarować jako `let mut`, a pożyczenie trzymane przez `peek` w miejscu, gdzie wołasz też `next`, jest błędem kompilacji, a nie problemem czasu wykonania. Z kolei `windows`, `chunks` i `dedup` w ogóle nie są adapterami — dwa pierwsze to metody **wycinka** (*slice*), a `dedup` metoda `Vec`. Powód warto zapamiętać, bo tłumaczy sam mechanizm: one muszą widzieć kilka elementów naraz, a iterator, który element już oddał, nie ma jak po niego wrócić. `iter.windows(2)` kończy się jako `E0599`; ratunkiem jest wcześniejszy `collect` albo zachowanie wycinka. I jeszcze jedno: `dedup` usuwa wyłącznie **sąsiadujące** duplikaty, więc odsianie powtórzeń z całej sekwencji wymaga najpierw sortowania albo przejścia przez `HashSet`.

Na koniec `scan`, który po polsku najprościej wyjaśnić przez arkusz kalkulacyjny: to suma narastająca. `fold` zwraca tylko wynik końcowy, `scan` wypuszcza każdy stan pośredni — `[5, 8, 8, 12, 14, 15]` zamiast samego `15`. Dwa szczegóły robią tu całą robotę: stan jest `&mut`, więc zmieniasz go w miejscu, zamiast zwracać nową wartość jak w `fold`; a domknięcie (*closure*) oddaje `Option`, więc `None` **kończy iterator** — i to jest sposób na `take_while`, którego decyzja zależy od wszystkiego, co dotąd przeleciało.

**Szukaj po polsku:** adaptery iteratorów · suma narastająca · `rust filter vs take_while` · `rust flat_map result drops errors` · `rust slice windows chunks dedup`
