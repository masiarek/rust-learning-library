...
| [`zip` and `enumerate`](zip_and_enumerate/README.md) | 101 → 201 | Walking two sequences as one: the `(i, (a, b))` pattern, and two silent failures — `zip` ending at the shorter side with no warning, and `enumerate` numbering what reaches it rather than the source |
