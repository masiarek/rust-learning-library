...
W polskiej literaturze funkcyjnej ta operacja nazywa się **zwijaniem** listy, a `acc` to **akumulator** — i to dobre słowa, bo od razu mówią, że coś przechodzi przez całą sekwencję i wychodzi z drugiej strony. `fold` bierze dwa argumenty: wartość początkową i domknięcie (*closure*) o kształcie `(akumulator, element) -> nowy akumulator`. Zdanie z tej strony, które warto zapamiętać dosłownie, brzmi: `sum`, `count`, `max`, `all` i `collect` **są** zwinięciami, i to nie na zasadzie analogii — std pisze `Sum for i32` jako `iter.fold(0, |a, b| a + b)`, a `count` jako `self.fold(0, |accum, _elem| accum + 1)`. Wniosek jest jednak odwrotny do oczekiwanego: nadal pisz wersję nazwaną. Ta wiedza mówi ci, **do czego** służy `fold`, a nie że warto przepisywać `sum` ręcznie — nazwana metoda jest czytelniejsza, a część z nich jest dodatkowo zoptymalizowana.

Do czego więc służy? Do sytuacji, w której **wynik nie jest tego samego typu co elementy**. `sum` potrafi zbudować tylko liczbę; `fold` zbuduje `HashMap` z zestawieniem (`tally["Ada"]` to `9` z dwóch wierszy Ady), `String`, albo krotkę. Stąd dwie reguły praktyczne. Akumulator jest przekazywany **przez wartość i zwracany**, więc domknięcie ma kształt `|mut acc, item| { …; acc }` — nie `|acc, item|` z mutacją, po której liczysz, że coś zostanie. A krotka jako akumulator odpowiada na dwa pytania w jednym przejściu: `fold((i32::MAX, i32::MIN), |(lo, hi), s| (lo.min(*s), hi.max(*s)))` daje `(0, 5)`, podczas gdy `.min()` i osobno `.max()` przechodzą dane dwa razy.

Różnicę między `fold` a `reduce` najłatwiej po polsku ustawić na pojęciu **elementu neutralnego** ze szkolnej algebry. `reduce` bierze za wartość początkową pierwszy element sekwencji, więc akumulator musi być typu elementu, a wynik jest `Option` — na pustej sekwencji nie ma pierwszego elementu i `reduce` odmawia zmyślenia go, zwracając `None`. `fold` zwróci to, co sam podałeś. Dla sumy element neutralny istnieje i `fold(0, +)` jest uczciwe: suma niczego naprawdę wynosi zero. Dla maksimum nie istnieje — `fold(i32::MIN, max)` na pustej liście zwraca `-2147483648`, liczbę, której nigdy nie było w danych, a która wygląda jak prawdziwa odpowiedź. Dlatego `max()` w std zwraca `Option`, a nie liczbę.

Dwie rzeczy na koniec, obie widoczne w wyniku programu. `fold` nie umie powiedzieć „koniec, mam dość” — odwiedzi każdy element, więc błąd trzeba by nieść w akumulatorze aż do końca; `try_fold` przerywa i na `["5", "3", "no", "4"]` domknięcie wykona się **trzy razy, nie cztery**. To ta sama maszyneria, na której stoją `all`, `any` i `find`. I pułapka, która nie daje żadnego sygnału, bo wynik jest identyczny: `.clone()` wewnątrz `fold` prawie zawsze oznacza, że akumulator jest **odbudowywany zamiast niesiony** — sześć wektorów, żeby zrobić jeden, koszt kwadratowy zamiast liniowego. Klon zwykle bierze się stąd, że `acc` nie zostało zadeklarowane jako `mut` i wersja bez klona wygląda, jakby nie miała prawa się skompilować. Ma prawo: akumulator jest **przenoszony** do domknięcia i przenoszony z powrotem.

**Szukaj po polsku:** zwijanie listy · akumulator · element neutralny · `rust fold vs reduce` · `rust try_fold short circuit` · `rust fold accumulator clone`
