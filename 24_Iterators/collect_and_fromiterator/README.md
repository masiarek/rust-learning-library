...
Polski czasownik podpowiada tu złą intuicję. „Zebrać” brzmi jak czynność, którą wykonuje `collect` — a `collect` nie robi nic własnego: jego ciało to jedna linijka, `FromIterator::from_iter(self)`, czyli oddanie roboty **typowi, o który poprosiłeś**. Decyzja jest po lewej stronie, w adnotacji, a nie po prawej, w wywołaniu. Ten sam iterator zebrany czterokrotnie daje `Vec<&str>`, `String` (sklejony, bez separatora), `BTreeSet` (posortowany i bez powtórzonej `Ada`) oraz `VecDeque` — nie dlatego, że `collect` je rozróżnia, tylko dlatego, że za każdym razem uruchamia inną implementację `FromIterator`.

Stąd bierze się najczęstszy błąd przy tej metodzie: `E0282, type annotations needed`. Po polsku mówimy „brakuje adnotacji typu”, co jest prawdą, ale myli — kompilator nie pyta o typ **elementów** (ten potrafi wywnioskować), tylko o to, **do jakiej kolekcji** ma je włożyć. I reguła nie brzmi „opisuj każdy `collect`”, tylko „typ musi dać się ustalić”: wystarczy wiązanie (`let v: Vec<_>`), turbofish (`collect::<Vec<_>>()`), ale tak samo dobrze **pole struktury**, typ zwracany funkcji albo typ parametru — wnioskowanie biegnie wstecz, od miejsca, w którym wartość ostatecznie ląduje. Jeden wyjątek warto zapamiętać, bo jest złośliwy: parametr będący wycinkiem (*slice*), czyli `&[String]` — a więc dokładnie ten typ parametru, do którego popycha `clippy::ptr_arg`. `&Vec<String>` dochodzi do `&[String]` przez koercję dereferencji (*deref coercion*), a wnioskowanie nie odtwarza koercji wstecz: bierze `[String]` dosłownie i odrzuca jako typ bez znanego rozmiaru (`E0277`). Idiomatyczny parametr i wnioskowanie z miejsca użycia po prostu się tu nie składają — wtedy wraca się do turbofisha.

Najważniejszy pojedynczy wzorzec to `Result<Vec<T>, E>`, bo **odwraca kształt**: iterator `Result`ów staje się jednym `Result`em na całą sekwencję, więc wołający ma do sprawdzenia jedną rzecz zamiast jednej na wiersz, a na końcu linii można postawić `?`. Dwie konsekwencje, o których polskie materiały zwykle milczą: `collect` przerywa na pierwszym błędzie (*short-circuit*) — na czterech wierszach domknięcie (*closure*) odpali się dwa razy, nie cztery — i **zostaje wyłącznie pierwszy błąd**. Jeśli potrzebujesz raportu ze wszystkimi, to nie ten konsument: `partition(Result::is_ok)` albo `Vec<Result<T, E>>` zachowa komplet, przy czym ten drugi kształt jest o jedną adnotację obok, na tym samym łańcuchu, i daje `[Ok(5), Err(..), Ok(0), Err(..)]` — nie ma tam czego przerywać. Tak samo zachowuje się `Option`, a w wersji skrajnej `Result<(), E>`: „czy wszystko się udało”, bez żadnej wartości w środku. To zarazem powód, dla którego przypadkowe `let _: () = it.collect();` kompiluje się i po cichu nie robi nic.

Na koniec dwie rzeczy, które dzieją się bezgłośnie. Zbieranie par do mapy **nie jest scalaniem**: `[("Ada", 5), ("Ben", 3), ("Ada", 4)]` daje `{"Ada": 4, "Ben": 3}`, bo późniejszy klucz nadpisuje wcześniejszy i nic o tym nie mówi — w Javie `Collectors.toMap` rzuca w tej sytuacji wyjątkiem, Rust wybrał ciszę, a gdy duplikaty mają się sumować, właściwym narzędziem jest `fold` z `entry().or_insert()`. Druga: wybór kolekcji jest **deklaracją o twoim typie** — `HashSet` wymaga `Eq + Hash`, `BTreeSet` wymaga `Ord` — a błąd o niespełnionej cesze (*trait*) pokaże się w linii z `collect`, nie przy definicji struktury. Dezorientuje to dokładnie raz.

**Szukaj po polsku:** wnioskowanie typów w Ruscie · adnotacja typu przy `collect` · `rust E0282 type annotations needed` · `rust collect into Result short circuit` · `rust FromIterator impl for my type`
