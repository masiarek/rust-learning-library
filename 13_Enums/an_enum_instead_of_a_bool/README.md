...
**The call that cannot be checked.** Write `clean(s, lowercase: bool, trim: bool)`, then call it with the two arguments swapped. Say what tells you it is wrong — the answer is nothing.

Now replace both `bool`s with two-variant enums and swap the arguments again. Say what changed, what it cost in bytes, and — the other half — name three places a `bool` is still the right choice.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:an_enum_instead_of_a_bool_kata -->
*[`an_enum_instead_of_a_bool_kata.rs`](examples/an_enum_instead_of_a_bool_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:an_enum_instead_of_a_bool_kata -->
*Verified output of [`an_enum_instead_of_a_bool_kata.rs`](examples/an_enum_instead_of_a_bool_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
THE BOOL VERSION, CALLED TWO WAYS
  clean_bools(input, true, false)  "  hello world  "
  clean_bools(input, false, true)  "Hello World"
  Both compile. One of them is the call the author meant and the
  other is the arguments swapped, and nothing anywhere can tell
  you which -- the types are identical, so the compiler has no
  question to ask.

AT THE CALL SITE, WHICH IS WHERE IT IS READ
  clean_bools(s, true, false)      <- true what? false what?
  clean(s, Casing::Lower, Trimming::Keep)
  The second one is readable without opening the function. That
  is most of the value, and it costs nothing.

THE ENUM VERSION, WITH THE ARGUMENTS SWAPPED
  clean(input, Trimming::Trim, Casing::Lower)
  does not compile: expected `Casing`, found `Trimming`. The
  backwards call became a build error rather than a wrong answer
  in production.
  clean(input, Casing::Lower,    Trimming::Trim) = "hello world"
  clean(input, Casing::Preserve, Trimming::Keep) = "  Hello World  "

AND IT IS FREE
  size_of::<bool>()      1 byte
  size_of::<Casing>()    1 byte
  Two named variants cost exactly what two unnamed states cost.
  There is no runtime price for the check, because the check
  happened at compile time.

WHEN A bool IS STILL RIGHT
  When the name at the call site already says everything:
  `v.is_empty()`, `s.contains(c)`, `if found { ... }`. The rule
  is about PARAMETERS -- a bool you pass in is two unnamed states
  arriving somewhere the name is gone.
```
<!-- /output -->
...
Pułapka wartości logicznej (*boolean trap*) wygląda w miejscu wywołania tak: `print_page(true, false)` — i nie ma w tej linijce niczego, co pozwoliłoby rozstrzygnąć, czy chodzi o wydruk dwustronny w czerni, czy jednostronny w kolorze. Obie kolejności się kompilują, a tylko jedna jest zadaniem, o które chodziło. Dla czytelnika przychodzącego z Pythona albo z ABAP-a najważniejsza jest tu jedna informacja o samym Ruście: **nie ma w nim argumentów nazwanych**. Nie napiszesz `both_sides=True` jak w Pythonie ani `both_sides = abap_true`, czego ABAP przy dwóch parametrach wręcz wymaga — jedynym miejscem, w którym może zamieszkać nazwa, jest **typ**. Dwa wyliczenia zamiast dwóch `bool`i nie są więc ozdobnikiem stylistycznym, tylko jedynym dostępnym mechanizmem nazywania argumentów w wywołaniu; przy okazji zamieniają odwróconą kolejność w `E0308` z gotową poprawką (`help: swap these arguments`). Komentarz `/* both_sides= */` robi to samo zadanie, stoi dokładnie tam, gdzie powinien stać typ, i nie sprawdza go nikt.

Struktura jednopolowa (*newtype*) opakowująca `bool` kupuje tę samą odmowę — zamienione argumenty to znowu `E0308` — i nie kupuje nic ponadto. `DoubleSided(true)` wciąż wymaga od czytelnika wiedzy, w którą stronę biegnie `true`, a `!flag` wciąż po cichu odwraca sens. W wyliczeniu nie ma żadnego `true`, które dałoby się przestawić. Reguła Drysdale'a jest dobra i warta zapamiętania w tej postaci: `newtype`, jeśli sens **zawsze** pozostanie dwuwartościowy; wyliczenie, jeśli może pojawić się trzecia możliwość. Ta trzecia możliwość jest w jednej kolumnie darmowa — dochodzi wariant i każde `match` przestaje się kompilować, dopóki go nie obsłużysz — a w drugiej oznacza przepisywanie, bo `bool` ma dwie wartości i nie da się z tym negocjować.

Ten sam błąd piętro niżej ma postać komentarza nad polem: „`fg_color` musi być (0, 0, 0), jeśli `monochrome` jest prawdą”. Taki komentarz opisuje stany, na które typ pozwala, a których program nie ma prawa wyprodukować — czyli jest specyfikacją bez kontroli. Wciągnij flagę i pole, którym ona rządzi, do jednego wyliczenia (`Monochrome` albo `Foreground(RgbColor)`), a opisywać nie ma już czego: sześć stanów robi się czterema, a te dwa sprzeczne z komentarzem przestają być zapisywalne. Przy prawdziwym `RgbColor` ta sama arytmetyka daje 33 554 432 stany, z czego 16 777 215 przeczy komentarzowi, wobec 16 777 217 stanów, w których skłamać się nie da. Drysdale pisze o tym *make invalid states inexpressible*, a szerzej znane sformułowanie tej samej zasady brzmi *make invalid states unrepresentable* — po polsku najbliżej „stanów nieprawidłowych nie da się w ogóle zapisać” — i wychodzi przy tym **mniej**, a nie więcej pamięci: na tej platformie struktura z `bool`em i `Shade` zajmuje 2 bajty, ta z wyliczeniem 1, bo trzywartościowy `Shade` zużywa 3 z 256 układów bitów swojego bajtu i znacznik ma się gdzie schować.

Na koniec rzecz, którą trzeba wiedzieć, zanim się w tej sprawie policzy na narzędzia: **clippy tego nie złapie**. Dwa linty nazywają dokładnie ten refaktor — `fn_params_excessive_bools` i `struct_excessive_bools` — ale oba siedzą w grupie `pedantic`, domyślnie wyłączonej, i odzywają się dopiero **powyżej trzech** wartości logicznych. `print_page(bool, bool)` jest pod progiem, a `DisplayProps` ma jednego `bool`a, więc trafna podpowiedź „consider refactoring bools into two-variant enums” nigdy nie trafi do osoby, która najbardziej jej potrzebuje. To jest więc nawyk projektowy, a nie coś, co wyłapie narzędzie — dokładnie tak samo jak przy literówce, która staje się wiązaniem.

**Szukaj po polsku:** pułapka wartości logicznej · argumenty nazwane · wzorzec newtype · `rust boolean trap enum` · `rust E0308 swap these arguments` · `rust make invalid states unrepresentable`
