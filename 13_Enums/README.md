...
| [Variants that carry data](variants_that_carry_data/README.md) | 201 | Product versus sum, counted rather than asserted; what the payload costs, measured — the niche that makes `Option<Box<T>>` the same eight bytes as the pointer, and the rare 104-byte variant every 4-byte one pays for until it is boxed |
