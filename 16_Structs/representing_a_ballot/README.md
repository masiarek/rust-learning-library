...
```bash
rustc --edition 2024 16_Structs/representing_a_ballot/examples/representing_a_ballot.rs -o /tmp/rb && /tmp/rb
```
...
Zanim padnie choćby jeden typ, warto zadać pytanie matematyczne — ono tłumaczy, dlaczego trzy rodziny kart potrzebują trzech różnych kształtów w Ruscie, a nie trzech odmian jednego:

- **karta punktowa** (*score / STAR*) to **funkcja** z kandydata w wartość 0..=5, więc każdy kandydat ma swoją przegródkę, nawet jeśli wyborca jej nie wypełnił — stąd `[Score; N]`;
- **karta aprobująca** (*approval*) to **podzbiór** kandydatów — maska bitowa (*bitset*) albo `HashSet<CandidateId>`;
- **karta rankingowa** (*ranked*) to **porządek**, zwykle częściowy — `Vec<CandidateId>`, w którym danymi jest sama kolejność, a przy remisach w rankingu `Vec<Vec<CandidateId>>`, czyli ciąg poziomów.

Wyborca, który uszeregował dwóch z pięciu kandydatów, oddał ciąg o długości dwa, a nie pięcioelementową strukturę z dziurami — i właśnie dlatego jeden typ karty nie obsłuży po cichu dwóch rodzin naraz.

Sześć kształtów tej samej karty różni się nie tyle szybkością, ile tym, **jakie błędy da się w nich w ogóle napisać**. Tablica `[u8; 3]`, `[Stars; 3]`, struktura krotkowa `Row` i struktura nazwana `NamedBallot` zajmują **po 3 bajty** — mają identyczny układ w pamięci i różnią się wyłącznie tym, co wolno o nich powiedzieć. `Vec<u8>` to **24 bajty, zanim pojawi się jakakolwiek dana**, bo na stosie leży sam uchwyt (wskaźnik, długość, pojemność), a dane mieszkają na stercie; te 24 bajty to dokładna cena za to, że liczby kandydatów nie znasz w czasie kompilacji. W zapisie pozycyjnym czai się przy tym błąd, którego żaden typ nie zobaczy: te same trzy karty z nagłówkiem `Ada,Ben,Cara` dają zwycięstwo Bena (13 punktów), a z nagłówkiem `Ben,Ada,Cara` — zwycięstwo Ady (też 13). **Nic się nie zepsuło**: ani błąd parsowania, ani panika, ani nawet ostrzeżenie. Znaczenie kolumny 0 nigdy nie było w danych, tylko w nagłówku, którego system typów nie widział. Struktura nazwana zamyka tę dziurę z definicji (piszesz `.ada`, a nie `[0]`), ale wpisuje kandydatów do **typu**, więc wyborów wczytywanych z pliku już nią nie opiszesz — i ta sprzeczność jest właściwą treścią lekcji.

„Puste” to nie „zero”, a tę różnicę polski czytelnik zna raczej z komisji niż z Rusta: brak znaku w kratce i wpisana ocena 0 wnoszą do sumy tyle samo, ale to nie jest to samo zachowanie wyborcy. `Vec<Option<Stars>>` pamięta różnicę i pozwala powiedzieć „oceniono 2 z 3 kandydatów” — wielkość, która w prawdziwym protokole naprawdę występuje. Najlepsze, że nic nie kosztuje: `Stars` ma sześć wariantów i 250 wolnych układów bitów, więc `Option<Stars>` nadal zajmuje **jeden bajt** (ta sama nisza, *niche optimization*, co przy poprzedniej lekcji). Czy tę informację zachować, jest decyzją modelową, nie wydajnościową — liczeniu jest wszystko jedno, audytowi nie.

Mapa znosi problem kolumn, bo znaczeniem jest sam klucz, ale wprowadza inny, w kontekście wyborów szczególnie kłopotliwy: `HashMap` losuje ziarno funkcji skrótu **przy każdym uruchomieniu procesu**, więc kolejność iteracji zmienia się z przebiegu na przebieg. Procedura, która rozstrzyga remis, przeglądając mapę, dziedziczy tę nieodtwarzalność — a wynik, którego nikt nie potrafi powtórzyć, nie jest wynikiem. Dlatego domyślnym wyborem jest tutaj `BTreeMap`, iterujący zawsze w porządku kluczy, mimo gorszej złożoności; w językach, w których słownik przypadkiem zachowuje kolejność wstawiania, tę samą zależność buduje się, nigdy się o niej nie dowiadując. Docelowy kształt silnika to natomiast jedna płaska tablica: `candidates: Vec<String>` (pozycja → kto) plus `cells: Vec<u8>` w porządku wierszowym (*row-major*), czyli `voter * n + cand`. Błąd rozjechanego nagłówka zostaje wymieniony na błąd arytmetyki indeksu, a opłaca się to tylko dlatego, że to wyrażenie pisze się **raz**, w jednej metodzie, którą da się przetestować. Waga bloku (`42 × 5,4,3`) jest zaś kompresją, a nie innym głosowaniem: liczba mnoży wiersz, a nie dołącza do niego — wstawienie 42 jako czwartej liczby do tej samej tablicy byłoby dokładnie tym samym błędem kategorii co nagłówek.
