...
W karcie STAR komórka trzyma liczbę **od 0 do 5**, a `u8` — od 0 do 255. Ta różnica to nie drobiazg, tylko dokładnie to miejsce, w którym mieszka błąd: `total_raw([5, 30, 0])` zwraca 35 i żaden typ nie protestuje, bo 30 jest najzupełniej poprawnym `u8`. Newtype jest odpowiedzią — **struktura krotkowa** z jednym **prywatnym** polem i jedną sprawdzającą funkcją `Score::new`, która zwraca `Option<Score>` zamiast panikować. Walidacja przestaje wtedy być czynnością, którą wykonuje każda funkcja z osobna, a staje się czymś, co **pamięta typ**: `&[Score]` w sygnaturze jest dowodem, że ktoś już sprawdził. Cały wiersz karty przechodzi przez te same drzwi w jednym wyrażeniu — `collect()` do `Option<Vec<Score>>` zatrzymuje się na pierwszym `None`, więc nigdy nie trzymasz karty zwalidowanej do połowy: wiersz jest przyjmowany albo odrzucany w całości.
...
Pułapka, dla której warto doczytać tę stronę do końca, brzmi: **prywatność obowiązuje na poziomie modułu, nie typu**. Wewnątrz `mod ballot` wyrażenie `Score(7)` kompiluje się bez mrugnięcia okiem; dopiero poza modułem dostaniesz `error[E0423]: cannot initialize a tuple struct which contains private fields`. Newtype w jednym płaskim pliku, bez żadnego `mod`, jest więc dokumentacją, a nie ścianą — drzwi stoją, muru wokół nich nie ma. Gwarancja jest tak mocna, jak mały jest moduł: każda kolejna funkcja dopisana do `mod ballot` to następne miejsce, którym niezmiennik może wyciec.
