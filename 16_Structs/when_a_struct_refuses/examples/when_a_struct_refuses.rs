...
impl fmt::Display for Scored {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{} scored {}", self.name, self.score)
    }
}
...
impl Default for Retries {
    fn default() -> Self {
        Retries { limit: 10 } // a domain default the derive could never guess
    }
}
...
fn main() {
    ...
    let a = Player { name: "Ada".into(), score: 5 };
    let b = Player { score: 5, ..Default::default() };
    println!("   named all:            {a:?}   (name {:?}, score {})", a.name, a.score);
    println!("   ..Default::default(): {b:?}   <- and this needs Default to exist");
    println!("   note the derived Default gave name an EMPTY string, not a missing one:");
    println!("   the type's zero, which is rarely your domain's. b.name.is_empty() = {}",
             b.name.is_empty());
    ...
    println!("       {}", Scored { name: "Ada".into(), score: 5 });
    ...
    let borrowed = Borrowed { name: &s };
    let owned = Owned { name: "Ada".into() };
    println!("       &str  field: {}   Box<str> field: {}", borrowed.name, owned.name);
    ...
    println!("       Level(3) == Level(3) is {}", Level(3) == Level(3));
    ...
    println!("   derived would be Retries {{ limit: 0 }}; ours says limit = {}",
             Retries::default().limit);
    ...
    println!("   `Stack::capacity()` names no T anywhere, but it lives in impl<T>, so");
    println!("   there is nothing to infer T from. Three fixes, in order of preference:");
    println!("     Stack::<i32>::capacity() = {}   name it at the call", Stack::<i32>::capacity());
    let t = Stack { rows: vec![1, 2, 3] };
