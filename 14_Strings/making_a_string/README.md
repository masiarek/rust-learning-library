...
**Four arrivals and one pre-payment.** Make a `String::with_capacity(100)`, push ten characters into it, and assert the capacity has not moved. Parse `" 42 \n"` into an `i32` without panicking — and check whether `parse` trims for you or whether your `trim()` was doing the work. Then build one line out of an `i32`, an `f64` shown to two decimals, and a `bool`, with `format!`.

Then the same text arriving as bytes. Take a `Vec<u8>` that really is UTF-8 into a `String` with `String::from_utf8`, and one that is not — read what the error says, and what it hands back to you. Run the broken bytes through `String::from_utf8_lossy` as well, and say which of the two you would put in a log viewer and which in a ballot parser.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:arriving_as_a_string_kata -->
*[`arriving_as_a_string_kata.rs`](examples/arriving_as_a_string_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:arriving_as_a_string_kata -->
*Verified output of [`arriving_as_a_string_kata.rs`](examples/arriving_as_a_string_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. Pre-paying for the growth
   String::with_capacity(100)  len   0  capacity 100
   after 10 pushes             len  10  capacity 100
   assert_eq!(capacity, 100) held — the buffer was bought once, up front.
   (An empty String::new() would have reallocated on the way to 10.)

2. Digits out of a raw field
   read_score(" 42 \n" ) -> Ok(42)
   read_score("42"     ) -> Ok(42)
   read_score("4 2"    ) -> Err(invalid digit found in string)
   read_score(""       ) -> Err(cannot parse integer from empty string)
   The trim is doing real work — `parse` itself refuses the spaces:
   " 42 \n".parse::<i32>()        -> Err(ParseIntError { kind: InvalidDigit })
   The Result is the whole point — a bad field is data, not a crash.

3. One String out of three types
   "seat 3: 61.84% turnout, decided = true"
   {:.2} rounds for display only; the f64 itself is untouched.

4. Bytes that really are UTF-8
   String::from_utf8(7 bytes) -> Ok("★ Ada")
   No copy: from_utf8 takes the Vec<u8> and hands back the same allocation.

5. Bytes that are not
   from_utf8       -> Err: invalid utf-8 sequence of 1 bytes from index 2
                      first bad byte at index 2
                      and the Vec is handed back unharmed: 5 bytes
   from_utf8_lossy -> "Ad�a�"   <- each bad byte became U+FFFD
   5 bytes in, 5 chars out, 2 of them the replacement character.
   Lossy never fails, which is the risk: it silences the error rather than
   reporting it. Use it for display; use from_utf8 when the bytes mattered.
```
<!-- /output -->
...
- [Returned by value](../../18_Ownership/returned_by_value/README.md) — why a function that builds text returns a `String` and never a `str`

## Po polsku

Pięć zapisów, jeden wynik w bajtach — i na tym polega cała trudność, bo skoro wszystkie się kompilują i wszystkie dają to samo, kompilator nie pomoże wybrać. Wybiera **źródło**, nie gust. `to_owned()` wcale nie jest operacją na tekście: to konwersja „pożyczone → własne”, która oddaje własnego bliźniaka źródła — z `&[i32]` zrobi `Vec<i32>`, z `&Path` zrobi `PathBuf`, a `42.to_owned()` odda po prostu `i32`. Łańcuch znaków wychodzi z niej w jednym jedynym przypadku: kiedy na wejściu był wycinek łańcucha (`&str`). `to_string()` przeciwnie — dotyczy tekstu zawsze, dla każdego typu z implementacją `Display`. Po polsku ta zasada brzmi nawet wyraźniej niż po angielsku: „zamień ten łańcuch na łańcuch” nie nazywa niczego, co się faktycznie dzieje, a „przejmij go na własność” nazywa dokładnie to.

Na polskich forach to samo pytanie wraca w postaci „co jest szybsze, `to_string()` czy `to_owned()`” — i jest to pytanie martwe. Spór o wydajność wygasł w 2016 roku, kiedy std wyspecjalizowało `to_string` dla `str`, więc dla wycinka łańcucha oba zapisy robią dziś dokładnie tyle samo pracy. Zostaje czytelność, czyli reguła z akapitu wyżej. Druga rzecz, którą warto zapamiętać raz na zawsze: `ToString` implementuje się **nigdy**. W `alloc` siedzi zbiorcza implementacja (*blanket impl*) `impl<T: Display + ?Sized> ToString for T`, więc twoja byłaby drugą i kompilator odrzuci ją błędem `E0119, conflicting implementations`. Piszesz `Display` — a `to_string()`, `{}` i `format!` dostajesz w gratisie. To dobry rodzaj błędu: język mówi ci, gdzie jest robota do wykonania. Symetrycznie: `Debug` się wyprowadza przez `#[derive(Debug)]`, `Display` pisze się ręcznie.

Najdroższa pułapka tej strony nie daje żadnego ostrzeżenia. `String` też implementuje `Display`, więc `owned.to_string()` na czymś, co już jest `String`-iem, kompiluje się w milczeniu i alokuje **drugi** bufor na stercie. To odruch przyniesiony z Pythona, gdzie `str(s)` na łańcuchu jest darmowe i oddaje ten sam obiekt; tutaj jest to zwyczajne klonowanie, tylko nienazwane. Chcesz podejrzeć — `&owned` nie kosztuje nic; chcesz wartość — przenieś ją; chcesz naprawdę kopię — napisz `.clone()`, żeby następny czytelnik widział, że to było zamierzone. I dwa drobiazgi na koniec: samo `.into()` nie skompiluje się bez wskazania celu (`E0282, type annotations needed`), bo to `From` czytane od tyłu i nic w wyrażeniu nie mówi, co ma powstać; a droga powrotna, `parse()`, zwraca `Result`, ponieważ tekst jest danymi wejściowymi, a dane wejściowe kłamią.

**Szukaj po polsku:** różnica między to_string a to_owned · konwersja typów w Ruscie · `rust to_string vs to_owned` · `rust E0119 conflicting implementations ToString` · `rust implement Display not ToString`
