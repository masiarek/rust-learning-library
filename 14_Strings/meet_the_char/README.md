...
- [`char` is four bytes ↗](https://masiarek.github.io/encodings-learning-library/05_Rust/char_is_four_bytes/index.html) — this page's four lengths asked from the encoding's side: `size_of::<char>()`, `len_utf8()`, `len()` and `as u32` for one character, why the surrogate hole is Unicode's rather than Rust's, and what `chars()`, `bytes()` and `char_indices()` each yield walking the same string
- [The `string` module ↗](https://masiarek.github.io/python-learning-library/01_Text_and_Bytes/the_string_module/index.html) — what a language looks like when the ASCII predicates are *not* on the type. Python's `string` module keeps them as constants — `string.punctuation`, `string.hexdigits` — because they predate the methods that replaced them, and that page sorts them back into the ASCII chart they came from. `string.punctuation` is character-for-character `char::is_ascii_punctuation`; `string.whitespace` is **six** characters where `is_ascii_whitespace` is five, because Rust follows the WhatWG Infra definition and omits `U+000B` VERTICAL TAB — which `char::is_whitespace` does not
