...
**An interner that hands out numbers.** Intern a column of repeated names so each distinct text is stored once. Start with `fn intern(&mut self, s: &str) -> &str`, intern two names while you are still holding the first result, and read the `E0499`. Then return a `Symbol(u32)` instead, add `fn resolve(&self, id: Symbol) -> &str` to get the text back, and store each text once as an `Rc<str>` shared by the map that finds a symbol and the `Vec` that finds the text. Report how many owners each text has, and how big a `Symbol` is next to a `&str`.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:symbol_interner_kata -->
*[`symbol_interner_kata.rs`](examples/symbol_interner_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:symbol_interner_kata -->
*Verified output of [`symbol_interner_kata.rs`](examples/symbol_interner_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. Intern a column: eight values, three distinct
   symbols               [0, 1, 0, 2, 1, 0, 2, 0]
   texts stored          3
   owners of each text   2 (the map's key and the Vec's entry)

2. Handles compare as integers, and are small
   symbols[0] == symbols[2], both Ada   true
   a Symbol                             4 bytes
   a &str                               2 machine words

3. Resolve when the text is needed, as many at once as you like
   Ada, Ben, Cara
   resolve takes &self, so three shared borrows can be alive together.

4. Why intern does not hand out &str
   fn intern(&mut self, s: &str) -> &str ties the &str it returns to the
   MUTABLE borrow of the interner. Keep the first result and call intern
   again, and rustc stops with E0499: two mutable borrows at once.
   A Symbol borrows nothing, so that question never comes up.
```
<!-- /output -->
