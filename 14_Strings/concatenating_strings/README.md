...
W Javie, C#, Pythonie czy ABAP-ie `+` (albo `&&`) na napisach jest **symetryczny**: obie strony są równoprawne, nikt niczego nie traci, a wynikiem jest trzeci obiekt. W Ruscie jest inaczej i wszystko wynika z jednego wiersza w bibliotece standardowej — `impl Add<&str> for String`. To **jedyna** implementacja dodawania dla łańcuchów, więc lewy operand musi **posiadać** bufor (i zostaje skonsumowany), a prawy musi być pożyczonym wycinkiem. Stąd trzy różne kody błędu, które mówią dokładnie to samo: `E0369` dla `"a" + "b"` (żadna ze stron nie ma bufora — literał leży w pliku wykonywalnym, w pamięci tylko do odczytu, dokładnie tej wielkości co tekst), `E0308` dla `a + b` przy dwóch `String`-ach (brakuje `&` po prawej) i `E0368` dla `+=` na `&str`. Jedno zdanie do zapamiętania: **`+` rozbudowuje lewy operand, więc lewy operand musi mieć co rozbudowywać.**

Jest tu jednak drobiazg, na którym łatwo się przewrócić przy czytaniu cudzego kodu. W typowym przykładzie z podręcznika po prawej stronie stoi `&s2`, czyli `&String` — a `&String` **nie jest** `&str`. Kod się kompiluje, bo argument wywołania jest miejscem, w którym działa koercja przez `Deref` (*deref coercion*): kompilator sam zamienia `&String` na `&str`. Koercja dotyczy jednak **wartości w konkretnym miejscu wywołania**, a nie typu — dlatego wyrażenie `s1 + &s2` działa, ale próba nazwania jego typu jako `<String as Add<&String>>::Output` kończy się `E0277`, bo takiej implementacji naprawdę nie ma. Warto to zobaczyć raz, bo IDE potrafi podpowiedzieć właśnie tę długą formę i „poprawić” działający kod na niedziałający. Prawidłowa nazwa tego typu to po prostu `String`.

Praktycznie wybór jest prosty i wygląda tak: **domyślnie `format!`** — nie interesuje go, kto jest właścicielem czego, przyjmuje dowolnie wiele kawałków i czyta się go w kolejności powstającego zdania. `+` bierz tylko wtedy, gdy już trzymasz `String` po lewej i nie będzie ci potrzebny — wtedy jest najtańszy ze wszystkich, bo wynikiem jest ten sam bufor, a przy `String::with_capacity` cały łańcuch doklejeń nie alokuje ani razu. Gdy kawałków jest tyle, ile akurat wyszło z kolekcji, właściwą odpowiedzią jest `join(", ")` albo `concat()`: obie najpierw przechodzą po elementach, żeby policzyć rozmiar, i dopiero potem alokują raz.

Osobno stoi `concat!` z wykrzyknikiem — to **makro wykonywane w czasie kompilacji**, którego wynikiem jest `&'static str`, więc w czasie działania programu nie kosztuje nic. Nie da się nim jednak skleić zmiennej: przyjmuje wyłącznie literały. Mimo bliźniaczej nazwy nie jest to zamiennik `concat()` ani `format!`.

**Szukaj po polsku:** łączenie łańcuchów znaków · koercja przez Deref · przeniesienie własności lewego operandu · `rust E0369 cannot add &str to &str` · `rust String concatenation format!`
