...
| [Every `size_of`, `len` and `capacity` error, and its fix](size_len_capacity_errors/README.md) | 101 → 201 | Twenty-four refusals by symptom — `size_of` takes a type, `len()` counts bytes, `capacity()` only reads — each with rustc 1.98.0's text and a fix that compiles, and nine `help:` lines that point the wrong way |
| [Lints around `size_of`, `len` and `capacity`](size_len_capacity_lints/README.md) | 101 → 201 | Twenty-six rustc and clippy warnings, each with a silent version — `len_zero`, `size_of_ref`'s 8 where 24 was meant, `read_zero_byte_vec` — and six capacity-and-length mistakes no lint catches |
| [Drawing the owner and the view](drawing_the_owner_and_the_view/README.md) | 101 → 201 | Both halves on one drawing, checked with `as_ptr` — the view's pointer lands 7 bytes into the owner's buffer, and `E0597`, `E0505`, `E0502`, `E0599` and `E0515` are each the drawing refusing a dangling arrow |
...
| [Why a `char` is 32 bits wide](why_char_is_32_bits/README.md) | 201 → 301 | The arithmetic the primitive-types table leaves out — U+10FFFF needs 21 bits, 21 is not addressable, and the 11 left over are why `Option<char>` is still four bytes |
| [Four lengths, and which one the other system means](four_lengths/README.md) | 201 | One string, four counts — `.len()`, `.chars().count()`, `encode_utf16().count()` and the cluster count `std` refuses; which database, language and protocol means which; and the byte limit that panics mid-letter |
...
| [RFC 1212 — how `lines()` learned about `\r\n`](rfc_1212_line_endings/README.md) | 201 | Why `lines()` handles Windows text — the 2015 change, the Unicode separators the thread argued for and refused, the trailing `\r` on a file's last line, and the round trip it quietly breaks |
| [RFC 1054 — the method that renamed itself to promise less](rfc_1054_str_words/README.md) | 201 | Why `words()` is called `split_whitespace()` — std declining to define "a word", the pattern alternative that still will not compile, and the two definitions of whitespace std ships |
| [Comparing and sorting text](comparing_strings/README.md) | 201 | Why `"Zebra" < "apple"` is true — `Ord` on a string is `as_bytes().cmp()`, so the order is lexicographic by code point; plus the `ß` that lowercasing both sides gets wrong, and why a language's alphabet is a table rather than an algorithm |
| [Searching without splitting](searching_a_string/README.md) | 101 → 201 | Four questions and one trait — `contains`, `find`, `starts_with` and the split family take the same argument, `find` reports a **byte** offset, and `Pattern` is usable everywhere and nameable nowhere |
| [Replacing part of a string](replacing_in_a_string/README.md) | 101 → 201 | `replace` builds a new `String` rather than editing one — `replace_range` and `retain` are the two that really do, the chain that cannot swap two words, and the one pass that fixes it |
| [Parsing out of a string](parsing_a_string/README.md) | 101 → 201 | `.parse()` is a `FromStr` call, so the type comes from you and the failure comes back as a `Result` — the whitespace it never trims, the ASCII-only digits, and the `kind()` integers get and floats do not |
| [The format mini-language](the_format_language/README.md) | 201 | `{:>8.3}` is a language of its own — the whole grammar in nine lines, the precision that truncates a string instead of rounding it, and the `{{:>8?}}` that pads a number and ignores a `&str` |
...
| [String parameters worth copying](string_api_design/README.md) | 201 → 301 | Which signature makes the caller pay — `&str` by default, `impl AsRef<str>` for any kind of text, `impl Into<String>` when the function keeps it, `Cow` as a return — each counted in allocations |
...
[The string crates](string_crates/README.md) is the other half of the outside world — fifteen crates for the jobs `std` declines, from graphemes and normalization to legacy encodings and strings that never touch the heap, each beside the lesson here that says what `std` gives you instead. [Internationalization](internationalization/README.md) is the stub for the job `std` leaves out entirely — translated messages, plurals, locale-aware numbers and collation.
