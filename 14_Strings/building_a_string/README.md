...
**Every distinct permutation, in one buffer.** Write `permutations(s)` returning each distinct arrangement of the characters once — `"aa"` has one, and so does `""`. Build every candidate in a single `String` that grows with `push` on the way down and shrinks with `pop` on the way back, instead of allocating a new string at every level. Then predict how many `"banana"` has before you run it, and check your deduplication against n! divided by the factorial of each letter's count.

```rust
// rustc --edition 2024 --test permutations.rs -o t && ./t
...
<!-- source:permutations_kata -->
*[`permutations_kata.rs`](examples/permutations_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:permutations_kata -->
*Verified output of [`permutations_kata.rs`](examples/permutations_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. Adam's three cases
   "abc" -> ["abc", "acb", "bac", "bca", "cab", "cba"]
   "aa"  -> ["aa"]
   ""    -> [""], one arrangement of nothing rather than none

2. How many, and the repeats the skip rule removes
   input         n!  distinct  returned
   "abcd"        24        24        24
   "aabb"        24         6         6
   "banana"     720        60        60
   "aaaa"        24         1         1
   distinct = n! divided by the factorial of each letter's count, and the
   skip rule produces exactly that many without generating the rest.

3. Permute characters, not bytes
   "ée" -> ["eé", "ée"]
   the two bytes of "é" swapped: String::from_utf8 is_err = true
   A permutation of bytes can be something that is not text at all.
```
<!-- /output -->
...
Cała ta strona sprowadza się do dwóch pytań zadawanych o każdy sposób doklejania tekstu: **kto jest właścicielem lewej strony i kto alokuje**. `push_str` i `push` tylko pożyczają `&mut` i dopisują w istniejącym buforze; `format!` pożycza wszystko i **zawsze** buduje nowy `String`; a `+` jest tu wyjątkiem, bo **przenosi własność** lewego argumentu — jego bufor staje się wynikiem, dlatego łańcuch `a + ", " + &b` nie alokuje ani razu, a zmiennej `a` już potem nie ma. Asymetria bierze się stąd, że w bibliotece standardowej istnieje dokładnie jedna implementacja: `impl Add<&str> for String`. Lewa strona musi być `String` i zostaje skonsumowana, prawa jest zawsze wycinkiem — stąd `E0308` przy `a + b` i podpowiedź kompilatora *consider borrowing here*, czyli „dopisz `&`”.

Druga połowa strony to pułapka, która polskiego czytelnika dotyczy o wiele bardziej niż angielskiego: **`insert`, `remove`, `truncate`, `split_off` i `drain` przyjmują przesunięcia w bajtach, nie w znakach**. `String::from("Gdańsk")` ma sześć liter, ale siedem bajtów, a `truncate(4)` panikuje, bo bajt 4 wypada w środku `ń` (`truncate(3)` daje spokojnie `"Gda"`). To jest dokładnie ten błąd, którego **testy na samym ASCII nigdy nie wykryją**, a pierwsze polskie nazwisko w bazie wykryje natychmiast. Nawyk z ABAP-a (`lv+2(1)` na napisie UCS-2) i z Pythona (`s[:4]` po znakach) prowadzi tu prosto w panikę — legalne przesunięcia bierze się z `char_indices()`, a nie z `len()/2`.

Trzecia rzecz to nawyk do **oduczenia**, jeśli przychodzisz z Pythona. Polskie poradniki powtarzają — słusznie, ale o Pythonie — że sklejanie napisów w pętli przez `+=` jest kwadratowe i trzeba użyć `join`. W Ruscie `String` jest naprawdę mutowalny, więc `s.push_str(...)` w pętli to koszt zamortyzowany O(1) i jest to rozwiązanie właściwe, a nie wstydliwe. Ten sam błąd wraca jednak w przebraniu: `format!` **wewnątrz pętli** alokuje cały nowy `String` na każdym obrocie, żeby zaraz go dokleić i wyrzucić. Zamiast tego pisz `write!(bufor, "...")`, który używa tej samej składni formatowania, ale wpisuje się do już posiadanego bufora.

Przy `write!` warto znać dwie drobnostki, bo obie potrafią zabrać kwadrans: trzeba zaimportować `use std::fmt::Write` (bez tego kompilator mówi zagadkowo *no method named `write_fmt`*, a nie „brakuje importu”), a zwracany `Result` obowiązkowo się `unwrap`uje — istnieje tylko dlatego, że to samo makro obsługuje `io::Write`, gdzie zapis naprawdę może się nie udać. Zapis do `String` nie może.

**Szukaj po polsku:** sklejanie łańcuchów w pętli · przesunięcie bajtowe a znak · granica znaku UTF-8 · `rust push_str vs format!` · `rust String truncate is not a char boundary`
