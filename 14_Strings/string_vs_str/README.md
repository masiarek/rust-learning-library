...
## The names hide the pattern

Read `String` as **`StringBuf`** and the whole page collapses into one sentence: *a buffer that owns and grows, and a view that borrows a run of it*. The standard library already names the pattern that way everywhere else — `PathBuf` / `Path`, `OsString` / `OsStr`, `CString` / `CStr` — and `Vec<T>` / `[T]` is the same pair once more, with the buffer called `Vec` and the view called a slice. `String` / `str` is the odd one out: the oldest pair, named before the convention settled, and the one whose names suggest two kinds of *text* rather than an owner and a view. Nothing in the language changes if you do the rename in your head; what changes is that `&str` stops being a second string type and becomes what `&[T]` and `&Path` already are.

The split is not a Rust invention, either. Python has it too, at build time rather than in the type: a `str` is finished text, and the thing you *grow* is an `io.StringIO` or a list you `"".join` at the end — Java's `StringBuilder`, C#'s too. Those languages keep the buffer and the finished text apart because appending to an immutable string copies it every time; Rust keeps them apart for the same reason and then writes the difference into the type, so the compiler can see which one you are holding. [Building a `String`](../building_a_string/README.md) is the page for the growing half.
...
- [users.rust-lang.org — *String vs str, why?* ↗](https://users.rust-lang.org/t/string-vs-str-why/61334) — a forum thread, so weigh it as one: the `StringBuf` reading above is steffahn's, and the observation that garbage-collected languages keep the same split is scottmcm's
...
Jedna sztuczka, która porządkuje całą stronę: czytaj `String` jako **`StringBuf`**. Biblioteka standardowa nazywa ten sam wzorzec właśnie tak wszędzie indziej — `PathBuf` / `Path`, `OsString` / `OsStr`, `CString` / `CStr` — a para `Vec<T>` / `[T]` to on raz jeszcze, z buforem nazwanym `Vec` i widokiem nazwanym wycinkiem. Tylko `String` / `str` dostały nazwy sugerujące dwa rodzaje *tekstu* zamiast właściciela i widoku. Sam podział nie jest zresztą wynalazkiem Rusta: Python ma go w chwili budowania — `str` to gotowy tekst, a rośnie `io.StringIO` albo lista, którą na końcu się `"".join`-uje; Rust wpisuje tę różnicę w typ, żeby kompilator widział, który z dwóch trzymasz.
