...
Największa pułapka jest tu językowa: polskie materiały bardzo często opisują panikę (*panic*) jako „taki wyjątek”, a to zdanie podsuwa plan, którego w Ruscie nie da się zrealizować — „na razie zostawię `unwrap`, potem opakuję to w `try`”. Nie ma czym. `catch_unwind` istnieje po to, żeby panika nie przekroczyła granicy FFI w stronę C i żeby framework testowy mógł zaraportować nieudany test zamiast umrzeć razem z nim; użyty jako `try`/`catch` wokół zwykłej logiki zamienia głośny błąd w cichy. Panika nie jest ani obsłużonym błędem, ani wartością zwracaną — jest **przerwaną w połowie robotą z posprzątaną wokół niej pamięcią**.

I to rozróżnienie jest sednem strony: odwijanie stosu (*unwinding*) jest porządne wobec **pamięci** i niedbałe wobec **pracy**. Destruktory (`Drop`) między miejscem paniki a górą wykonują się co do jednego, w odwrotnej kolejności — RAII trzyma się w czasie paniki, więc blokady są zwalniane, pliki zamykane, a `MutexGuard` naprawdę puszcza. Tyle że `Drop` nie ma pojęcia, że funkcja była na trzecim z pięciu kroków. Po panice **zasoby są w znanym stanie, a dane w takim, w jakim zostawiła je przerwana robota**: dwie szklanki nalane, trzy zamówienia, na które nikt nawet nie spojrzał, i nalane zostają nalane — w skali produkcyjnej wstawione wiersze zostają wstawione, wysłane bajty już poleciały, a wysłanego maila nie da się cofnąć. Dwie rzeczy zmieniają nawet ten obraz: `panic = "abort"` w profilu release usuwa odwijanie w całości, więc **żaden destruktor się nie wykona** (kod po cichu liczący na `Drop` w czasie paniki liczy tak naprawdę na ustawienie builda), a panika w `Drop` w trakcie odwijania kończy się natychmiastowym `abort` całego procesu — dlatego destruktor to złe miejsce na operacje, które mogą się nie udać.

Trzy szczegóły na koniec, w polskich tekstach rzadko obecne. W wątku (*thread*) ginie tylko ten jeden wątek: `join()` oddaje ładunek paniki jako `Err`, a `Mutex`, który wątek trzymał w chwili śmierci, zostaje **zatruty** (*poisoned*) — pozostałe wątki dowiadują się o wypadku nie przez własną śmierć, lecz przez `lock()` zwracające `Err`. `Option::unwrap` jest oznaczony atrybutem `#[track_caller]`, więc w komunikacie widnieje **twoja** linijka z `unwrap`, a nie linia z `core/src/option.rs` — ale to wciąż nie jest linia, która podała `None`; ta siedzi w śladzie stosu (*backtrace*), domyślnie wyłączonym, więc bez `RUST_BACKTRACE=1` jej nie zobaczysz. I kod wyjścia: nieobsłużona panika to **101**, nie 1 — std rezerwuje tę wartość po to, żeby skrypt, CI albo nadzorca procesu odróżnił „program zawiódł i powiedział o tym” od „program się rozsypał”; warto o tym pamiętać, zanim napisze się w skrypcie zwykłe `if status != 0`.

**Szukaj po polsku:** panika w Ruscie · odwijanie stosu · zatruty mutex · `rust panic vs exception` · `rust RUST_BACKTRACE=1 backtrace` · `rust panic abort no unwinding`
