...
```bash
rustc --edition 2024 17_Option_and_Result/expect/examples/expect.rs -o /tmp/ex && /tmp/ex
```
...
`expect` to mechanicznie `unwrap` z dopisanym zdaniem, ale to zdanie pełni zupełnie inną rolę, niż podpowiada odruch: **nie jest komunikatem o błędzie**, tylko twierdzeniem o tym, dlaczego ta operacja nie mogła się nie udać. Dlatego najcenniejsze, co ta metoda daje, bywa negatywne — jeżeli takiego zdania nie potrafisz napisać, to nie jest niedogodność, tylko wynik badania: nie masz dowodu, a więc i prawa do paniki.

Polszczyzna ma tu własną pułapkę. Naturalnie pisze się „nie udało się wczytać kworum”, a to jest dokładnie angielskie *failed to…* — zdanie, które powtarza wyłącznie to, że program się zatrzymał, co słowo `panicked` w pierwszym wierszu powiedziało już samo. Zamiast tego opisz stan, który **powinien** zachodzić, i zrób test proponowany przez tę stronę: **wskaż gwaranta**. „Loader wypełnia to z wartości domyślnych”, „wyżej jest `return None` dla pustego wycinka”, „to jest literał kilka znaków w lewo” — jeżeli takiej frazy nie ma, żadne przeredagowanie nie pomoże i trzeba zmienić typ zwracany na `Result`. Stąd bierze się też najlepszy argument za tą metodą przy przeglądzie kodu: `.unwrap()` w diffie znaczy „dowodu nie przedstawiono”, `.expect("…")` znaczy „dowód jest, sprawdź go” — i tylko to drugie da się zrecenzować. Praktyczna uwaga na koniec: ten tekst czyta programista, nie użytkownik, i ląduje obok angielskiego `panicked at …` oraz `Debug` błędu, więc w zespołach mieszanych zwykle wygrywa pisanie go po angielsku, choćby dla spójności zgłoszenia w issue trackerze.

Dwie rzeczy techniczne, o które łatwo się potknąć. Na `Result` panika składa się z dwóch części — `{twoje zdanie}: {błąd wypisany przez Debug}` — więc twierdzenie i przyczyna zostają obie, a `expect` nigdy nie kosztuje cię błędu, w odróżnieniu od całej rodziny `unwrap_or`, która go po prostu wyrzuca (`expect_err` jest lustrzanym odbiciem, na wypadek gdy zaskoczeniem jest to, że się udało). Z tego samego wypisywania bierze się ograniczenie `E: Debug`, którego `unwrap_or` nie ma: bez `Debug` nie ma z czego złożyć komunikatu i dostajesz `E0277` z podpowiedzią, żeby dodać `#[derive(Debug)]`. To dobry powód, by na typach błędów dopisywać `Debug` odruchowo — i warto zapamiętać, że drukowany jest `Debug`, nigdy `Display`.

Na koniec pułapka, która wygląda całkiem niewinnie. `expect` przyjmuje `&str`, czyli zwykły argument obliczany zachłannie, więc komunikat zbudowany przez `format!` powstaje, alokuje się i ginie przy **każdym udanym** wywołaniu: w ćwiczeniu z tej strony `proof()` wykonuje się cztery razy, choć nic nie zawiodło ani razu. Zwykły literał nie kosztuje nic, więc jego używaj domyślnie, a gdy komunikat naprawdę potrzebuje wartości z czasu działania, przenieś całość na smutną gałąź — `unwrap_or_else(|| panic!("…"))`, gdzie ten sam `proof()` wykonuje się zero razy.

**Szukaj po polsku:** panika w Ruscie · obsługa błędów w Ruscie · `rust expect vs unwrap` · `rust expect panic message best practice` · `rust Result expect requires Debug`
