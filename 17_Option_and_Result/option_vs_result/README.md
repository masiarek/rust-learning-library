...
Ani `Option` (*Opcja*), ani `Result` (*Rezultat*) nie są wbudowane w język — to zwykłe wyliczenia z `prelude`, które mógłbyś napisać sam. Cała różnica siedzi w smutnym wariancie: `None` nie niesie nic, `Err(E)` niesie ładunek. Stąd bierze się jedyne pytanie, które trzeba sobie zadać przy wyborze: **czy kod wywołujący mógłby sensownie zapytać „dlaczego nie?”**. `map.get("zoe")` zwraca `None`, bo możliwa odpowiedź jest dokładnie jedna — nie ma takiego klucza. `File::open` zwraca `Err`, bo odpowiedzi jest kilka (nie ma pliku, brak uprawnień, za dużo otwartych deskryptorów) i na każdą można zareagować inaczej. Drugi test, gdy pierwszy nie rozstrzyga: **czy brak jest w ogóle błędem?** Zapytanie mapy o nieistniejący klucz nie jest awarią — zadałeś pytanie i dostałeś uczciwą odpowiedź. `Option` jest od braku, `Result` od niepowodzenia.

Dla czytelnika przychodzącego z Javy czy C# najkrótszy skrót brzmi tak: `Result` to wyjątek, który podróżuje jako zwykła wartość zwracana, zamiast rozwijać stos. Przeoczyć go przypadkiem się nie da — oba typy mają `#[must_use]`, więc kompilator ostrzeże przed zignorowanym wynikiem, a kiedy naprawdę chcesz go wyrzucić, napisz `let _ = …`, żeby następny czytelnik widział, że to była decyzja. Formą, która zawsze działa, jest `match`: kompilator odrzuci go, jeśli pominiesz ramię, i na tym opiera się całe bezpieczeństwo. Reszta — `if let`, `unwrap_or`, `unwrap_or_else` — to lukier składniowy na tym samym `match`u. Osobno zapamiętaj `let … else`: trzyma szczęśliwą ścieżkę przy lewym marginesie i to dzięki niemu obsługa błędów nie zamienia się w schodki.

Dwie rzeczy mechaniczne, na których potyka się każdy. Pierwsza to `map` kontra `and_then`. `map` przekształca wartość w środku i zostawia opakowanie, więc jeśli twoje domknięcie samo zwraca `Option`, wychodzi `Option<Option<u32>>` — prawie nigdy nie o to chodziło. `and_then` spłaszcza (to `flat_map` pod inną nazwą), więc zasada kciuka brzmi: jeśli domknięcie samo może zawieść albo wrócić puste, sięgaj po `and_then`. Druga to `?`, czyli sedno całego projektu — „daj mi szczęśliwą wartość albo natychmiast zwróć z tej funkcji smutną”. Warto znać dwa ograniczenia, zanim ugryzą: `?` na `Option` działa tylko w funkcji zwracającej `Option` (a na `Result` — tylko w zwracającej `Result`), i po stronie `Result` `?` **nie zwraca po prostu błędu** — zwraca `Err(From::from(e))`, czyli po drodze konwertuje go na typ błędu twojej funkcji. To niejawne wywołanie `From` jest najużyteczniejszą rzeczą, jaką da się o `?` wiedzieć.
