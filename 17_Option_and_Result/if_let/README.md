...
```bash
rustc --edition 2024 17_Option_and_Result/if_let/examples/if_let.rs -o /tmp/il && /tmp/il
```
...
`if let` to `match`, z którego usunięto gałąź, na której i tak nic się nie działo — owo `None => {}`, napisane wyłącznie po to, żeby zadowolić kompilator. Różnicy w czasie działania nie ma żadnej: to lukier składniowy, kompilujący się do dokładnie tego samego `match`a. W czytaniu warto od razu ustawić sobie jedną rzecz: znak `=` w `if let Some(x) = v` **nie jest przypisaniem**. Po lewej stronie stoi wzorzec, a całość czyta się „jeżeli ta wartość pasuje do tego wzorca, zwiąż nazwy i wykonaj blok”. Skoro pod spodem jest dopasowanie wzorców, działa każdy wzorzec: destrukturyzacja struktur, krotki, zagnieżdżenia — a literał wpisany we wzorzec (`score: 5`) jest **warunkiem**, nie wiązaniem.

Płaci się za to jedną rzeczą i lepiej wiedzieć o niej wcześniej niż później. `match` nad wyliczeniem jest wyczerpujący (*exhaustive*): kompilator nie zbuduje programu, dopóki nie obsłużysz wszystkich wariantów, i to jest najużyteczniejsza właściwość bezpieczeństwa w całym języku, bo wypłaca się w dniu, w którym ktoś dokłada nowy wariant. `if let` z tej kontroli rezygnuje — dla jednego wyrażenia. Ćwiczenie z tej strony pokazuje różnicę jednym wierszem: po dopisaniu wariantu `Contested` wersja z `match` przestała się kompilować i człowiek musiał zdecydować, co z nią zrobić, a wersja z `if let … else` kompilowała się dalej i zaczęła odpowiadać „still counting” na stan, który wcale nie jest liczeniem. Reguła kciuka: `if let`, gdy naprawdę chodzi o **jeden kształt, a reszta cię nie obchodzi** (wartość jest albo jej nie ma, pierwszy element, udany parse); `match`, gdy dzielisz zbiór przypadków — zwłaszcza nad wyliczeniem, które kontrolujesz ty albo twoje zależności. Jeżeli chciałbyś, żeby ktoś ci powiedział o nowym wariancie, nie pisz `if let`.

Trzy pokrewne konstrukcje odpowiadają na trzy różne „tak, ale co, jeśli…”:

- `let … else` — odwraca `if let`: zamiast wcinać pracę, po którą przyszedłeś, załatwia porażkę i zostawia wiązanie na lewym marginesie do końca funkcji. Blok `else` musi przerwać przepływ sterowania (*diverge*) — `return`, `break`, `continue`, `panic!` — i właśnie dlatego dalej kompilator może traktować wiązanie jako pewne;
- `while let` — gdy `None` jest warunkiem zakończenia pętli, a nie sytuacją wyjątkową;
- `matches!` — gdy odpowiedzią ma być zwykły `bool`. Kusząca alternatywa, czyli mutowalna flaga zadeklarowana nad `if let` i ustawiana w środku na `true`, to cztery wiersze na jedno wyrażenie logiczne.

Na koniec dwie rzeczy, które ugryzą. Wzorzec **przenosi własność**, więc `if let Some(n) = name` zabiera wartość i późniejsze użycie `name` kończy się na `E0382`. Kompilator podpowiada `Some(ref n)`, ale w praktyce pisze się `= &name` — dopasowanie do referencji sprawia, że wiązania też są referencjami; do łączenia wywołań służą `.as_ref()` i `.as_deref()`. Odruch sięgnięcia po `.clone()` jest tutaj najczęstszy i prawie zawsze zły. Druga rzecz to prawdziwa zmiana zachowania w edycji 2024: wartość tymczasowa zbudowana przez wyrażenie, po którym dopasowujesz, ginie **przed** blokiem `else`, a w edycji 2021 dopiero po nim. Nikogo to nie obchodzi, dopóki tą wartością nie jest strażnik blokady (*lock guard*) — wtedy kolejność z 2021 oznacza zakleszczenie w `else`, który próbuje wziąć tę samą blokadę. Z edycją 2024 wiąże się też druga nowość: łańcuchy `if let … && let …` (od Rusta 1.88) działają wyłącznie tam, więc jeśli w starszym kodzie widzisz schodki zagnieżdżonych `if let` z warunkiem na samym dole, to jest właśnie ten powód.

**Szukaj po polsku:** dopasowanie wzorców w Ruscie · wyliczenia w Ruscie · `rust if let vs match exhaustive` · `rust let else` · `rust let chains edition 2024`
