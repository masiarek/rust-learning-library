...
```bash
rustc --edition 2024 17_Option_and_Result/partial_functions/examples/partial_functions.rs -o /tmp/pf && /tmp/pf
```
...
Ta strona jest jedynym miejscem, gdzie `Option` dostaje uzasadnienie matematyczne, a polski czytelnik ma tu gotowe słownictwo z matematyki dyskretnej: **funkcja częściowa** (*partial*) to taka, która nie jest określona na całej dziedzinie, a **funkcja całkowita**, spotykana też jako *totalna* (*total*), ma wynik dla każdego argumentu. Dzielenie nie ma odpowiedzi dla zerowego dzielnika, pierwiastek rzeczywisty dla liczby ujemnej, `first()` dla pustej listy — to nie są przypadki brzegowe do załatania, tylko miejsca, w których działanie jest naprawdę niezdefiniowane. Język ma wtedy trzy wyjścia: **paść** (panika, wyjątek), **skłamać** wartownikiem (`-1`, `null`, `NaN`, pusty łańcuch znaków) albo **poszerzyć przeciwdziedzinę** o wartość znaczącą „brak odpowiedzi”. Rust wybiera trzecie i efekt jest dokładnie taki: funkcja częściowa zwracająca `T` staje się funkcją całkowitą zwracającą `Option<T>`. Nic nie jest już niezdefiniowane — każde wejście ma odpowiedź, tyle że częścią odpowiedzi jest `None`.

Najważniejsze zdanie tej strony jest sprostowaniem popularnego wytłumaczenia, które brzmi mniej więcej „`Option` chroni cię przed paniką przy dzieleniu przez zero”. Dla liczb zmiennoprzecinkowych to **nieprawda**: `10.0_f64 / 0.0` daje `inf`, a `0.0_f64 / 0.0` daje `NaN`, bez paniki, bez błędu i bez ostrzeżenia, bo IEEE 754 definiuje oba wyniki. Gorzej: `NaN` nie jest równy nawet samemu sobie, więc późniejsze `==` po cichu wychodzi fałszywe, a sortowanie zaczyna się dziwnie zachowywać. To wzmacnia, a nie osłabia argument za `Option`em — nie chodzi o zamianę awarii na kontrolowany błąd, tylko o to, żeby liczba, która wygląda wiarygodnie, a odpowiedzią nie jest, w ogóle nie weszła do wyników. Paniką kończy się dopiero arytmetyka całkowitoliczbowa: dosłowne `10 / 0` zatrzyma jeszcze kompilator (lint `unconditional_panic`, *„this operation will panic at runtime”*), a gdy dzielnik przychodzi w czasie wykonania, program panikuje z komunikatem `attempt to divide by zero`. `10i32.checked_div(0)` zwraca po prostu `None`.

Gdy już masz to pojęcie, `Option` w typach zwracanych biblioteki standardowej przestaje wyglądać na kaprys projektantów. Rodzina `checked_*` jest najczystszą ilustracją w całym std: to samo działanie arytmetyczne wystawione dwa razy, a jedyna różnica polega na tym, czy przypadek niezdefiniowany jest paniką, czy wartością — `checked_add` przy przepełnieniu, `checked_sub` gdy odejmowanie na typie bez znaku zeszłoby poniżej zera, `checked_div` przy zerze. Tak samo działa para `v[i]` i `v.get(i)`, tak samo `first()`, `last()` i `map.get(k)`. Za każdym razem te same dane i to samo pytanie; całą różnicą jest typ zwracany.

Na koniec dwie sprawy dyscypliny. Po pierwsze, `None` mówi „brak odpowiedzi” i nic ponadto, więc wystarcza tam, gdzie brak ma dokładnie jeden powód (klucza nie ma w mapie, liczba jest ujemna); kiedy funkcja bywa niezdefiniowana z kilku powodów, na które kod wywołujący zareagowałby inaczej, jesteś mu winien `Result`. Po drugie, zapisz w komentarzu, co twój `None` **znaczy**: `mean(&[])` zwraca `None` dlatego, że takiej średniej nie ma, a nie dlatego, że wynosi 0 — zero jest przecież oceną, którą wyborca może realnie postawić. Sklejenie tych dwóch faktów w jedną liczbę to dokładnie ten wartownik, przed którym cała strona przestrzega.

**Szukaj po polsku:** funkcja częściowa a całkowita · dzielenie przez zero w Ruscie · wartownik zamiast braku wartości · `rust checked_div checked_add overflow` · `rust NaN not equal to itself` · `rust option total function`
