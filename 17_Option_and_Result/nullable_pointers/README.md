...
```bash
rustc --edition 2024 17_Option_and_Result/nullable_pointers/examples/nullable_pointers.rs -o /tmp/np && /tmp/np
```
...
Zdanie „Rust nie ma `null`a” powtarza się w polskich materiałach tak często, że łatwo przegapić, co dokładnie zajmuje jego miejsce. `&T`, `&mut T` i `Box<T>` **zawsze** wskazują na coś prawdziwego — nie ma literału, którym dałoby się zapisać pusty wskaźnik, ani sposobu, żeby go wyprodukować w bezpiecznym kodzie. Opcjonalność zostaje więc wyprowadzona z wartości do **typu**: wskaźnik, którego może nie być, to po prostu inny typ, `Option<Box<T>>`. Skutek jest bardzo konkretny — `check_optional` nie dosięgnie swojego `i32`, dopóki nie powie, co się dzieje, kiedy go nie ma. Ten `match` **jest** sprawdzeniem nulla, a nie ceremonią wokół niego ani konwencją, o której da się zapomnieć na jednej ścieżce, której nikt nie przetestował. Skoro `&i32` i `Option<&i32>` to dwa różne typy, sygnatura funkcji mówi wprost, który z nich przyjmuje, i pomyłka po stronie wywołującego jest niemożliwa, a nie tylko źle widziana.

Naturalne pytanie brzmi: ile kosztuje to opakowanie? Nic. `Box<i32>` zajmuje 8 bajtów i `Option<Box<i32>>` też 8, bo skoro `Box` nigdy nie bywa pusty, kompilator przeznacza ten niemożliwy układ bitów na `None` — to jest optymalizacja niszy (*niche optimization*). Wyjątek jest jeden i wart zapamiętania, bo to w nim intuicja zawodzi: **surowemu wskaźnikowi** (`*const T`) wolno być pustym, więc żadna nisza nie zostaje wolna i `Option<*const i32>` puchnie do 16 bajtów na prawdziwy znacznik. Optymalizacja nie jest przysługą, którą Rust robi `Option`owi — jest konsekwencją tego, że typ w środku coś obiecał.

Prawdziwym powodem istnienia tego typu nie są jednak opcjonalne argumenty, tylko **struktury rekurencyjne**. W `next: Option<Box<Node>>` obie połowy pracują nad różnymi pytaniami: `Box` daje `Node` znany rozmiar (bez niego typ zawiera sam siebie i jest nieskończenie duży — kompilator odrzuca to od razu), a `Option` pozwala łańcuchowi się **skończyć**. Lista w C zapisuje ten koniec jako pusty `next` i ufa, że każdy, kto po niej chodzi, sprawdzi go przed użyciem; tutaj koniec listy jest wartością, o której wie system typów, więc `while let Some(n) = cursor` nie wyjdzie poza ostatni węzeł — i wciąż kosztuje 16 bajtów na węzeł, bez osobnej flagi „czy jest następny”. Dwie rzeczy do zapamiętania na co dzień: pełny `match` piszesz rzadko, bo `as_deref()` zamienia `Option<Box<T>>` w `Option<&T>` i pozwala zajrzeć do środka bez konsumowania pudełka; a `p` w ramieniu `Some` jest typu `Box<i32>`, nie `i32` — drukuje się i dereferencjuje jak liczba wyłącznie dzięki `Deref`, o czym dobrze wiedzieć, zanim przyjdzie komunikat o niezgodności typów. To wszystko jest odpowiedzią na „pomyłkę wartą miliard dolarów” Tony’ego Hoare’a: problemem nigdy nie było samo istnienie nulla, tylko wpuszczenie go do **każdego** typu.

**Szukaj po polsku:** pusty wskaźnik · struktura rekurencyjna · pomyłka warta miliard dolarów · `rust Option<Box<T>> niche optimization` · `rust E0072 recursive type has infinite size`
