...
**Shadowing** is declaring a variable whose name is already in use. The new one hides the old for the rest of the scope; the old one is [still there](../../18_Ownership/shadowing_does_not_drop/README.md), unchanged, and comes back when the scope ends.
...
```bash
rustc --edition 2024 17_Option_and_Result/shadowing_and_unwrap/examples/shadowing_and_unwrap.rs -o /tmp/sau && /tmp/sau
```
...
- [A shadow does not drop](../../18_Ownership/shadowing_does_not_drop/README.md) — the sequel: this page is about the *name*, that one is about the value it stopped naming
- [When to shadow](../../18_Ownership/when_to_shadow/README.md) — once you know what it is: the trade against `mut`, the idioms, and the three bugs that compile
...
Przesłanianie (*shadowing*) i rozpakowywanie (*unwrapping*) `Option`/`Result` to dwie niezależne sprawy: pierwsza dotyczy **nazw**, druga **wartości**. Polskie słowo akurat dobrze podpowiada, o co chodzi — przesłonić to zasłonić, a nie nadpisać: `let score = score.len();` nie zmienia starej zmiennej, tylko tworzy nową o tej samej nazwie, a stara nadal istnieje i wraca, gdy skończy się zasięg wewnętrznego bloku. Od razu warto też odróżnić przesłanianie od wiązania we wzorcu (*pattern binding*): `if let Some(number) = maybe_number` niczego nie przesłania, bo nazwa `number` jest zupełnie nowa — wprowadza ją dopasowanie wzorców, a nie żaden wcześniejszy `let`.
