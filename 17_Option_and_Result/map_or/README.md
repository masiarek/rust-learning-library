...
```bash
rustc --edition 2024 17_Option_and_Result/map_or/examples/map_or.rs -o /tmp/mo && /tmp/mo
```
...
Jeden powód, żeby w ogóle sięgać po `map_or`, jest twardy: **typ może się zmienić**. `unwrap_or` potrafi oddać wyłącznie `T`, więc wartość zapasowa musi już być tego samego typu co ta w środku; `map_or` oddaje `U`, dzięki czemu `Option<u8>` wchodzi, a `String` wychodzi — „4 stars” albo „no score”. Jeśli nic nie przekształcasz i typ zostaje ten sam, chciałeś napisać `unwrap_or`, a nie `map_or` z tożsamościowym domknięciem (*closure*).

Prawdziwa pułapka jest w kolejności argumentów, i po polsku boli dokładnie tak samo jak po angielsku, bo mówimy przecież „jeśli jest wynik, to podwój, a jeśli nie — weź sto”. `map_or(100, |q| q * 2)` odwraca ten porządek: **wartość domyślna stoi pierwsza, a wykonuje się ostatnia**, odwrotnie niż w `match`u, gdzie ramię `Some` idzie na górze. Tego odruchu nie da się wytrenować i nie trzeba — pociechą jest to, że zamiana argumentów miejscami kończy się błędem typów, a nie cichym błędem w wyniku: kosztem zaskoczenia jest jedna kompilacja, nie jeden zły raport. Na `Result` jest jeszcze ostrzej: w `map_or_else(default, f)` tym „domyślnym” argumentem jest domknięcie przyjmujące `E`, więc **ścieżka błędu jest zapisana po lewej**, przed ścieżką sukcesu. Zasada brzmi: czytaj sygnaturę, nie nazwę. Warto też pamiętać, że samo `map_or` na `Result` wyrzuca błąd do kosza, tak samo jak `unwrap_or`.

Reszta to dwie granice. Pierwsza jest ta sama co w rodzinie `unwrap_or`: `map_or(expensive(), f)` liczy wartość zapasową **zawsze**, a `map_or_else(expensive, f)` dopiero na ścieżce niepowodzenia — w kodzie różni je jedna para nawiasów, co jest bardzo słabym sygnałem jak na realny koszt. Druga to granica wyznaczona przez dwa lintery clippy, które ciągną w przeciwne strony tylko pozornie: `clippy::map_unwrap_or` (pedantic) wpycha cię w `map_or`, gdy wartością zapasową jest **wartość**, a domyślnie włączony `clippy::unnecessary_map_or` wypycha cię z niego, gdy tą wartością jest `false` albo `true` — bo wtedy nie ustawiasz żadnej wartości domyślnej, tylko zadajesz pytanie tak/nie. Napisz je po imieniu: `map_or(false, pred)` to `is_some_and(pred)`, a `map_or(true, pred)` to `is_none_or(pred)`. A gdy któreś ramię ma zrobić coś więcej niż zwrócić wartość — zmienić licznik, wyjść z funkcji przez `?` lub `return` — wracaj do `match`, bo `return` w domknięciu wraca tylko z domknięcia.

**Szukaj po polsku:** wartość domyślna · leniwe wartościowanie · kolejność argumentów · `rust map_or vs unwrap_or` · `clippy unnecessary_map_or is_some_and`
