...
```rust
#[derive(Default)] struct DerivedThreshold(u32);        // 0
impl Default for HouseThreshold { fn default() -> Self { HouseThreshold(50) } }
```

```text
cart total: 40, and the config set no threshold
#[derive(Default)] -> DerivedThreshold(0)  => ships free? true
impl Default (50)  -> HouseThreshold(50)   => ships free? false
```

Same call site, opposite outcome, and the difference is a line of code in another file. `#[derive(Default)]` on a newtype is a claim that **zero is the sensible fallback** — true for a counter or a sum, false for a threshold, a timeout, a retry limit, a page size, a scale maximum.
...
error[E0277]: the trait bound `Threshold: Default` is not satisfied
     |
   4 |     let _ = q.unwrap_or_default();
     |               ^^^^^^^^^^^^^^^^^ the trait `Default` is not implemented for `Threshold`
...
1 | #[derive(Default)]
  |          ^^^^^^^
2 | enum OnConflict { Abort, Overwrite }
  | ------------------------------------ this enum needs a unit variant marked with `#[default]`
help: make this unit variant default by placing `#[default]` on it
  |
2 | enum OnConflict { #[default] Abort, Overwrite }
  |                   ++++++++++
```
...
```text
no file at all         match -> no input file was provided; nothing to read
a file with no rows    match -> a real file that happens to have no rows
no file at all         unwrap_or_default() -> 0 rows, total 0
a file with no rows    unwrap_or_default() -> 0 rows, total 0
```
...
counts.get("dog").copied().unwrap_or_default()     // a word that never appeared: 0
let drained = std::mem::take(&mut pending);        // swap the default IN, move the value OUT
```

A word that never appeared really did occur zero times: here the type's zero **is** the domain's answer, and saying so in one word is an improvement. The pattern generalizes to any accumulation whose identity element is the default — counts, sums, empty strings being built up, `Vec`s being drained.
...
// -> port 8080, retries 3, title "(untitled)"
```
...
**The type's zero is not your domain's zero.** Derive `Default` on a `Response` of three ratings, then call `unwrap_or_default()` on a form that was never returned and compare the result with one that rated everything 0.
...
unwrap_or_default fills in the TYPE's zero:
  returned       -> Response { speed: 5, price: 2, support: 0 }
  never returned -> Response { speed: 0, price: 0, support: 0 }
      The second line is a form nobody filled in, and it is now
      indistinguishable from someone who rated everything 0:
      equal? true

So the summary has to ask before it defaults:
  forms returned: 2 of 3
  of those, all-zero responses: 1
...
──── Step 2: A derived default is the type's zero, not your domain's
  ...
  impl Default (50)  -> HouseThreshold(50)   => ships free? false
      ...
      the sensible fallback' — true for a counter, false for a threshold, a
      timeout, a retry limit, a page size. The third option is the best one
...
──── Step 3: On an enum, the compiler makes you say it out loud
  OnConflict::default()          -> Abort
  configured.unwrap_or_default() -> Abort
    Abort  <- #[default]
    Overwrite
    Rename
...
──── Step 4: Empty is not absent
  no file at all         match -> no input file was provided; nothing to read
  a file with no rows    match -> a real file that happens to have no rows
  a file with 2 rows     match -> 2 rows, total 8
  no file at all         unwrap_or_default() -> 0 rows, total 0
  a file with no rows    unwrap_or_default() -> 0 rows, total 0
...
──── Step 5: Where it is exactly right: zero as an identity
  word counts: {"cat": 2, "mat": 1, "the": 3}
  counts.get("the").copied().unwrap_or_default() -> 3
  counts.get("dog").copied().unwrap_or_default() -> 0
  mem::take(&mut pending) -> "row 3: '4x' is not a number", leaving ""
      A word that never appeared really did occur zero times: here the
...
──── Step 6: Three spellings, and the place Default really earns its keep
  ...
  Config { retries: 3, ..Default::default() }
    -> port 8080, retries 3, title "(untitled)"
...
Sedno strony mieści się w jednym zdaniu: `Default` daje **zero typu, a nie zero twojej dziedziny**. Przy `#[derive(Default)] struct DerivedThreshold(u32)` brak progu w konfiguracji daje `0`, więc koszyk za czterdzieści „przekracza próg darmowej wysyłki”; przy ręcznym `impl Default` zwracającym `HouseThreshold(50)` ten sam koszyk go nie przekracza. To samo miejsce wywołania, przeciwny wynik, a różnicą jest jedna linijka w innym pliku. `derive(Default)` na strukturze opakowującej jest więc twierdzeniem, że zero jest sensowną wartością zapasową — prawdziwym dla licznika czy sumy, fałszywym dla progu, limitu czasu, limitu prób i rozmiaru strony. Jest jeszcze trzecie wyjście, o którym łatwo zapomnieć, bo nie wygląda na wyjście: **nie implementuj `Default` wcale**. Wtedy `unwrap_or_default()` przestaje się kompilować — `E0277`, *the trait bound `Threshold: Default` is not satisfied* — a kompilator wskazuje palcem decyzję, którą się pominęło. Brak implementacji jest barierką ochronną, nie luką, a derywowanie jej „dla porządku” to dokładnie ten ruch, którym barierkę się usuwa. Biblioteka standardowa trzyma się tej samej zasady: `NonZeroU8` nie ma `Default`, bo nie ma zera.

Na wyliczeniu (*enum*) kompilator każe powiedzieć to samo na głos: `#[derive(Default)]` bez `#[default]` na którymś wariancie to `E0665`. Nie ma reguły „pierwszy wariant” ani zera, do którego można by uciec, więc język odmawia zgadywania. Warto zauważyć asymetrię — na strukturze `derive` podejmuje decyzję za każde pole po cichu, na wyliczeniu nie podejmuje żadnej. Atrybut, który trzeba wtedy dopisać, ląduje w definicji typu, czyli tam, gdzie czyta go recenzent kodu, i to jest właściwe miejsce dla ustalenia w rodzaju „przy konflikcie nazw przerwij”. Uwaga dla korzystających ze starszych materiałów: `#[default]` na wariancie działa dopiero od Rusta 1.62, więc dawniejsze poradniki pokazują w tym miejscu ręcznie pisany `impl Default`.

Zostaje różnica między **brakiem a pustką** — po polsku te dwa słowa nie są wymienne, a to właśnie ją kasuje `unwrap_or_default()`. „Nie było pliku wejściowego” oraz „był plik, tylko bez ani jednego wiersza” to dwa różne fakty, a po tym wywołaniu oba są tym samym pustym `Vec`-em, mimo że pierwszy z nich jest zgłoszeniem błędu. Jeśli dla każdego odbiorcy brak i pustka znaczą to samo, trzymaj zwykły `Vec` i wyrzuć `Option` z typu; jeśli znaczą co innego, `unwrap_or_default()` jest tą jednosłowną linijką, która różnicę kasuje. Bywa jednak dokładnie na miejscu: słowo, które nie wystąpiło ani razu, naprawdę wystąpiło zero razy — wtedy zero typu **jest** odpowiedzią dziedziny, podobnie jak w `*counts.entry(word).or_default() += 1` albo w `mem::take`, które wstawia wartość domyślną po to, żeby wyjąć prawdziwą. Najbezpieczniejsze zastosowanie samej cechy `Default` jest zaś zupełnie gdzie indziej — w składni aktualizacji struktury: `Config { retries: 3, ..Default::default() }` bierze wszystkie niewymienione pola z **jednej implementacji, którą można otworzyć i przeczytać**, czyli zapisuje wartości domyślne w jednym miejscu, zamiast podsuwać je w wywołaniu, które o nich nie wspomina.
