...
```bash
rustc --edition 2024 17_Option_and_Result/optional_arguments/examples/optional_arguments.rs -o /tmp/oa && /tmp/oa
```
...
Pytanie, z którym się tu przychodzi, brzmi zwykle „jak zrobić w Ruscie argument domyślny?”, a odpowiedź jest krótka i rozczarowująca: nie da się. Rust nie ma ani argumentów domyślnych, ani przeciążania funkcji, i obie te nieobecności są **celowe** — wartości domyślne źle współpracują z cechami (*traits*) i z wnioskowaniem typów, a przeciążanie sprawia, że na pytanie „którą funkcję właśnie wywołałem?” nie da się odpowiedzieć, czytając kod. Trzeba więc wybrać kształt, a kształtów jest pięć — i `Option<T>` w parametrze jest tylko jednym z nich, zwykle nie najlepszym, bo jego koszt płaci nie autor funkcji, tylko **każde miejsce wywołania**.

Najtańsza odpowiedź jest zarazem najmniej efektowna: napisz dwie funkcje. Tak robi biblioteka standardowa — `Vec::new()` obok `Vec::with_capacity(n)`, `HashMap::new()` obok `HashMap::with_capacity(n)`. Dwie nazwy biją jedną nazwę plus `None`, którego nikt nie umie przeczytać, bo druga nazwa może jeszcze powiedzieć, **po co** istnieje, a `None` nie mówi tego nigdy. Parametr `Option<T>` jest naprawdę na miejscu wtedy, gdy kod wywołujący ma wartość-albo-nie **w czasie wykonania**: `connect(host, config.port, None)` jest w porządku, bo `config.port` faktycznie skądś przyszło jako `Option`. Sygnałem, że wybrałeś źle, jest widok dosłownego `None` w prawie każdym wywołaniu — taki argument nie chce być opcjonalny, on chce po prostu zniknąć. Dochodzi do tego strata, którą najmocniej odczuwają osoby przychodzące z Pythona: Rust nie ma argumentów nazwanych, więc `connect("a.io", None, None)` nie zdradza, **które** pokrętła zostały pominięte, a późniejsza zmiana kolejności parametrów po cichu zmienia znaczenie wszystkich istniejących wywołań.

Najostrzejsza reguła na tej stronie jest zarazem najtańsza do zastosowania: w pozycji argumentu bierz `Option<&T>`, **nigdy** `&Option<T>`. Różnica dotyczy tego, kto płaci. Funkcję `greet(name: Option<&str>)` wywoła każdy — ten, kto trzyma `&str`, ten, kto trzyma `String`, i ten, kto nie ma nic; posiadacz `Option<String>` dochodzi tam jednym `.as_deref()`. Żeby wywołać `greet(name: &Option<String>)`, trzeba już mieć prawdziwy `Option`, na który da się wskazać, więc kod wywołujący z gołym `&str` musi go **zbudować** — zaalokować `String` i owinąć go w `Some` — wyłącznie po to, żeby zadowolić sygnaturę. `&Option<T>` to niemal zawsze sygnatura napisana z perspektywy wnętrza funkcji; konwertuj na granicy przez `.as_ref()` / `.as_deref()` i przyjmuj ten elastyczny kształt.

Przy trzech i więcej pokrętłach oddaj argumentom ich nazwy: struktura opcji z `Default` (`ConnectOpts { timeout: Some(5), ..Default::default() }`) mówi w miejscu wywołania, które dokładnie pokrętło zostało przekręcone, a dołożenie czwartej opcji nie rusza ani jednego istniejącego wywołania — i to ta druga własność decyduje, że jest to rozwiązanie skalujące się. Wzorzec budowniczego (*builder*) to ten sam pomysł z płynnym interfejsem, na etapowe konstruowanie i walidację; zwróć uwagę, że **nie ma w nim `Option`a w ogóle**, bo jego wartości domyślne są zwyczajnymi wartościami, które już trzyma. To jest morał całej strony: `Option` należy się takiemu projektowi tylko wtedy, gdy „nieustawione” znaczy coś innego niż każda konkretna wartość. Zostaje jeszcze `impl Into<Option<T>>`, dzięki któremu `retries(5)` i `retries(None)` działają oba (bo istnieje `impl From<T> for Option<T>`) — poznaj to, żeby rozpoznać w cudzym API, ale sam sięgaj rzadko: osłabia wnioskowanie typów, nie składa się dalej niż do jednego–dwóch argumentów i sygnatura przestaje mówić czytelnikowi, co wolno przekazać.

**Szukaj po polsku:** argumenty domyślne w Ruscie · przeciążanie funkcji · wzorzec budowniczego · `rust default function arguments` · `rust Option<&T> vs &Option<T>` · `rust builder pattern default`
