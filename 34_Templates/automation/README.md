# Automation: either template, one command

**Level:** 201 · working knowledge

**One line:** `lint_profiles.py` starts a project from the [training](../training/README.md) or [production](../production/README.md) template, adds a template's lint and profile tables to a project that already exists, and checks a folder of projects against one. It reads the templates at run time, so the pages, the files and the script cannot disagree.

## Use it

```bash
python3 34_Templates/automation/lint_profiles.py new   ~/RustroverProjects/untitled2
python3 34_Templates/automation/lint_profiles.py new   ~/code/service --profile production
python3 34_Templates/automation/lint_profiles.py apply ~/RustroverProjects/untitled1
python3 34_Templates/automation/lint_profiles.py apply ~/code/service --profile production --force
python3 34_Templates/automation/lint_profiles.py check ~/RustroverProjects/*
```

| Command | What it does |
|---|---|
| `new PATH` | `cargo new`, then every file of the template over it, keeping the package name `cargo new` chose |
| `apply PATH…` | adds the template's `[lints.*]` and `[profile.*]` tables to `Cargo.toml`, its `clippy.toml`, and its `[alias]` table to `.cargo/config.toml`; `[package]`, `[dependencies]` and the toolchain stay the project's own |
| `check PATH…` | compares the same tables with the template, and exits 1 on any difference, on a project that has moved, or when `RUSTFLAGS` is set |

`--profile` picks the template, `training` by default. `apply` also takes `--force`, `--dry-run` and `--no-remap`. The script needs Python 3.11 or later, for `tomllib`; macOS's `/usr/bin/python3` is 3.9, and the script says so instead of failing on an import. The whole script is [at the bottom of this page](#the-script-in-full).

## new

```text title="lint_profiles.py new — real output, the scratch directory shortened to <e2e>"
<e2e>/practice  (training)
...
The package keeps the name `cargo new` gave it — `name = "practice"`, from the folder — and a `target/` or `Cargo.lock` left in the template by building it in place is not copied. In the new project, `cargo run` prints the example's output and `cargo strict` finishes with nothing to report.

## apply, to a project that already exists

A project carrying an older four-line `[lints.rust]` block, an alias of its own, and a remap line pointing at another folder:

```text title="lint_profiles.py apply — real output"
<e2e>/old  (training)
  added   [profile.dev.package."*"] with 1 key  (Cargo.toml)
  added   9 key(s) to [lints.rust]  (Cargo.toml)
  added   [lints.clippy] with 33 keys  (Cargo.toml)
  added   5 key(s)  (clippy.toml)
  added   1 key(s) to [alias]  (.cargo/config.toml)
...
- **A missing table** goes in whole, with its comments; **a missing key** goes at the end of its table.
- **A key already present with a different value** is somebody's decision, so it is kept and reported; `--force` replaces it.
- **The `[lints.*]` tables belong to the profile**, so `--force` also removes a lint the template does not name. Other tables — the project's own alias above, `[package]`, `[dependencies]` — are never touched.
- **Every result is re-parsed** with `tomllib` and compared with the template, and with the original minus what was meant to change, before anything is written.
- **`--dry-run`** prints the same notes and writes nothing; the files hashed identically before and after.

## Switching a project from training to production

Without `--force`, the stricter levels are reported and kept, and so are training's extra lints:

```text title="Abridged — real output; the 30 kept lines naming one lint each are dropped"
<e2e>/old  (production)
  added   [profile.release] with 3 keys  (Cargo.toml)
  kept    5 key(s) in [lints.rust] that production/template/Cargo.toml does not name: unused_variables, unused_imports, unused_mut, dead_code, unit_bindings  (--force removes them)
  added   2 key(s) to [lints.rust]  (Cargo.toml)
  kept    11 key(s) in [lints.clippy] that production/template/Cargo.toml does not name: nursery, integer_division, ref_patterns, if_then_some_else_none, iter_over_hash_type, verbose_file_reads, try_err, empty_structs_with_brackets, unnecessary_self_imports, string_lit_chars_any, undocumented_unsafe_blocks  (--force removes them)
  added   2 key(s) to [lints.clippy]  (Cargo.toml)
  ok      clippy.toml
  added   1 key(s) to [alias]  (.cargo/config.toml)
```

`check --profile production` then fails with `7/37 keys` as the template says. With `--force`, every differing level is replaced and the extras go:

```text title="Abridged — real output; 28 of the 30 forced lines dropped"
<e2e>/old  (production)
  forced  lints.rust.unsafe_code = "forbid"  (was "warn")  (Cargo.toml)
  removed 5 key(s) production/template/Cargo.toml does not name from [lints.rust]: unused_variables, unused_imports, unused_mut, dead_code, unit_bindings  (Cargo.toml)
  forced  lints.clippy.pedantic = { level = "deny", priority = -1 }  (was { level = "warn", priority = -1 })  (Cargo.toml)
  removed 11 key(s) production/template/Cargo.toml does not name from [lints.clippy]: nursery, integer_division, ref_patterns, if_then_some_else_none, iter_over_hash_type, verbose_file_reads, try_err, empty_structs_with_brackets, unnecessary_self_imports, string_lit_chars_any, undocumented_unsafe_blocks  (Cargo.toml)
  ok      clippy.toml
  ok      .cargo/config.toml
```

After that, `check --profile production` reports `37/37`, and `cargo ci` passes the project. Without the removals, training's `unused_variables = "allow"` would have survived the switch into a profile that is supposed to deny everything.

## The remap line

Neither template holds it, because it has to name the project's own directory: `--remap-path-prefix` with an empty old prefix makes diagnostics print a file's full path instead of `src/main.rs` — [explained on the RustRover page](../../05_Tooling/rustrover_setup/README.md). Rustflags interpolate nothing, so the path is spelled out, and moving the folder makes every diagnostic name the old place. `check` compares the two:

```text title="lint_profiles.py check, after mv practice moved — real output"
<e2e>/moved  (training)
  ok      Cargo.toml: 47/47 keys as training/template/Cargo.toml says
  ok      clippy.toml: 5/5 keys as training/template/clippy.toml says
  ok      .cargo/config.toml: 1/1 keys as training/template/.cargo/config.toml says
  FAIL    remap names <e2e>/practice — the project has moved; `apply` rewrites it
```

`apply` then updates the one string. `--no-remap` leaves the line out, and the `.gitignore` entry with it.

## Workspaces

On a workspace root the lint tables are written as `[workspace.lints.rust]` and `[workspace.lints.clippy]` — the name depends on the manifest's shape, and a wrong one is silently never read. A member whose manifest says `[lints] workspace = true` inherits them, and `apply` skips its `Cargo.toml`; `cargo new` inside the workspace writes that line by itself.

## Six traps, each measured

**A Cargo `[profile]` cannot carry lints.** `rustflags` under `[profile.dev]` is refused on stable 1.98:
...
That is why a *profile* here is a set of tables and an alias, not a Cargo profile.
...
`cargo strict` writes its flags inline for that reason.

**`-D warnings` alone does not bring an allowed lint back.** An allowed lint never becomes a warning, so there is nothing to promote: with `unused_variables = "allow"` in the manifest, `-D warnings` on its own left the unused binding silent. `cargo strict` passes `-W unused` first.

**A `RUSTFLAGS` environment variable replaces `build.rustflags`** rather than adding to it. Set, it removed the remap and both of `cargo strict`'s flags from rustc's command line, silently; `check` fails while it is set.

**`#[allow(unused_variables)]` covers one item.** Above `fn main` it silenced `main` and left the next function warning; `#![allow(unused_variables)]`, with the `!`, at the top of `main.rs` covers the file. The manifest table covers the package.

**Two projects with one package name can share a stale result.** With a common `CARGO_TARGET_DIR`, `cargo ci` in a second project named `app` printed `Finished` on code it refuses — [details on the production page](../production/README.md#why-each-part-is-there). `new` keeps the folder's name for exactly this reason.

## In RustRover

- Clippy's levels appear in the editor only with *Settings → Rust → External Linters* set to Clippy — [section 1 of the RustRover page](../../05_Tooling/rustrover_setup/README.md). The rustc half needs nothing.
- `cargo strict` and `cargo ci` run from the IDE's terminal. A *Cargo* run configuration with the command `strict` should work too, since Cargo resolves the alias, but that was not tried in the IDE.
- Keep `-D warnings` out of the linter's *Additional arguments*: every warning then turns red while you type.

## The script in full

<details markdown="1">
<summary><strong>lint_profiles.py</strong></summary>

<!-- file:lint_profiles.py -->
```python title="lint_profiles.py"
#!/usr/bin/env python3
...
- [Training](../training/README.md) and [Production](../production/README.md) — the two templates this script applies
- [Clippy](../clippy/README.md) — what the lint tables it writes are made of
- [Scaffolding a practice tree](../../05_Tooling/scaffolding/README.md) — the workspace answer, where configuration is shared rather than copied, and `rust_scaffold.py adopt`
- [RustRover setup](../../05_Tooling/rustrover_setup/README.md) — Clippy as the linter, and the remap line

---

*Every transcript above is real output from `lint_profiles.py` and cargo 1.98.0 on scratch projects, captured 2026-09-14, with the scratch directory shortened to `<e2e>` and lines dropped where a title says so. They are verified once: tools/run_examples.py runs neither Cargo nor this script. The script block at the bottom is the exception — it is pasted from the file, and checked on every commit.*

## Po polsku

`lint_profiles.py` robi trzy rzeczy. `new` zakłada projekt przez `cargo new` i kopiuje na niego cały szablon — treningowy albo, z `--profile production`, produkcyjny — zostawiając nazwę pakietu, którą nadało `cargo new`. `apply` dokłada do istniejącego projektu tabele `[lints.*]` i `[profile.*]` z `Cargo.toml`, plik `clippy.toml` i alias z `.cargo/config.toml`; `[package]`, `[dependencies]` i toolchain zostają nietknięte. `check` porównuje to samo z szablonem i kończy się kodem 1 przy każdej różnicy, przy przeniesionym projekcie i przy ustawionym `RUSTFLAGS`.

Zasada zmian: brakujące rzeczy są zawsze dopisywane, a wartość, która już jest i się różni, zostaje — to czyjaś decyzja — dopóki nie podasz `--force`. Tabele lintów należą do profilu, więc `--force` usuwa też linty, których szablon nie wymienia; bez tego przy przejściu z treningu na produkcję przetrwałoby `unused_variables = "allow"`. Każdy wynik jest ponownie parsowany przez `tomllib` i porównywany z szablonem, zanim cokolwiek trafi na dysk. Skrypt potrzebuje Pythona 3.11 lub nowszego.

**Szukaj po polsku:** szablon projektu Rust skrypt · `Cargo.toml` `[lints]` · `cargo alias` · `RUSTFLAGS` a `build.rustflags` · `--remap-path-prefix`
