...
- **ABAP.** `move` is the difference between `USING VALUE(iv_x)` and `USING iv_x` in a `FORM`, or between `IMPORTING VALUE(iv_x)` and plain `IMPORTING` on a method — pass by value versus pass by reference, a choice ABAP also makes at the boundary and also for lifetime reasons. Without `VALUE( )` both are passed by reference ([`FORM` parameters ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapform_parameters.htm), [`METHODS` parameters ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapmethods_parameters.htm)). Two things transfer badly. First, a method's `IMPORTING` parameter is by reference *and* read-only by default, so the "borrow" case is familiar, but the borrow checker's exclusivity rule has no counterpart: the parameter is the caller's data object itself, so a change made to that object under another name shows through it — SAP's own guideline has an `IMPORTING` parameter bound to `sy-index` counting up inside the method's `DO` loop ([Using System Fields as Actual Parameters ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abenuse_actual_parameters_guidl.htm)). Second, ABAP's by-value copy is exactly the `Copy` trap above: an internal table passed to `USING VALUE(...)` or `IMPORTING VALUE(...)` and modified inside leaves the caller's table untouched, silently, which is the same bug in the same shape. What Rust adds is that this is only silent for `Copy` types — for a `String` or a `Vec` the compiler stops you from using the original at all, which is the case ABAP cannot detect. The `'static` requirement has no ABAP analogue, because a `CALL FUNCTION ... STARTING NEW TASK` hands its parameters over and takes nothing back on the call itself: no values are accepted through `IMPORTING`, `CHANGING` or `TABLES`, and results return only through `RECEIVE` in a callback routine ([`STARTING NEW TASK` parameters ↗](https://help.sap.com/doc/abapdocu_758_index_htm/7.58/en-US/abapcall_function_starting_para.htm)).
...
**Two places `move` is not optional.** Write a function returning `impl Fn() -> String` that greets a name given to it, and spawn a thread that sums a `Vec` you own. Both fail to compile without `move` — read each error and say, in one sentence each, what the compiler is protecting.

Then the part that catches people: write a `move` closure capturing a `u32` and use that `u32` afterwards. Explain why that compiles, and why the same code with a `String` does not.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:the_move_keyword_kata -->
*[`the_move_keyword_kata.rs`](examples/the_move_keyword_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:the_move_keyword_kata -->
*Verified output of [`the_move_keyword_kata.rs`](examples/the_move_keyword_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. RETURNING A CLOSURE NEEDS `move`
  hello, ada
  hello, ada
  The String lives inside the closure now. That is what let it
  outlive make_greeter, and it is a question about WHERE the
  value lives -- not about what the body does with it.

2. SENDING ONE TO A THREAD NEEDS `move` FOR THE SAME REASON
  sum from the other thread: 14
  The spawned thread may outlive this function, so a borrow of
  `rows` would be unsound and the compiler says so. `move` gives
  the Vec to the closure, and naming `rows` below here would now
  be a use-after-move error.

3. `move` DOES NOT DECIDE THE TRAIT
  with_move() = 20, without() = 20
  Both are `Fn`. `move` copied the u32 into one and borrowed it
  in the other, and neither body mutates or consumes anything --
  so both sit on the top rung. What the BODY does decides the
  trait; `move` decides where the capture lives.

4. AND ON A Copy TYPE IT IS ALMOST INVISIBLE
  n is still usable after the `move` closure: 10
  Because u32 is Copy, `move` copied it rather than moving it --
  which is why `move` on a counter you meant to share is a
  classic silent bug: the closure gets its own copy and your
