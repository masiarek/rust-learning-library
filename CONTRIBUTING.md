...
**There is no `Cargo.toml`, and an IDE wants one.** Nothing here builds with Cargo, so RustRover opens every `.rs` file under *"Project not associated with a Cargo.toml file"* — no inferred types, no go-to-definition, no Run button. [`tools/write_cargo_toml.py`](tools/write_cargo_toml.py) writes a gitignored manifest for your machine, one binary per `.rs` file `git ls-files` shows and at the edition the runner uses, so what the IDE checks is what CI compiles. It is the IDE's view and never the build: an example still has to compile on its own under bare `rustc`, and one example can tell the difference — Run [rustc_without_cargo](15_First_Programs/rustc_without_cargo/README.md) from the IDE and its first step names the package, where the recorded key says *"No Cargo here"*, because `option_env!("CARGO_PKG_NAME")` is answered at compile time by whoever did the compiling. That is the page doing its job; CI still records the key from bare `rustc`, and every other example prints its key byte for byte under Cargo. One kind of file is left out, because a target that cannot build fails a plain `cargo build` — a tracked `.rs` outside `examples/` that rustc refuses, which is what the two refusals under [A stack slot is reused](18_Ownership/a_stack_slot_is_reused/README.md) are for. An untracked one is always listed: a scratch file that does not compile yet is what the IDE is for.

**A lesson that needs Cargo keeps a workspace in `demo/`.** Some things cannot be one file: a procedural macro needs a crate of its own, and `syn`, `quote` and `darling` come from crates.io. Such a lesson gets a Cargo workspace in `<lesson>/demo/` — its own `[workspace]` table, so no manifest above it can claim it, and a committed `Cargo.lock` — and declares the commands whose output the page shows in `demo/cargo_runs.toml`. [`tools/run_cargo_demos.py`](tools/run_cargo_demos.py) runs each with `--locked`, keeps the answer key in `demo/keys/<stem>.out`, and fills `<!-- cargo:stem -->` blocks the way `run_examples.py` fills `output:` ones. A run can be declared `expect = "failure"` with `stream = "stderr"`, which is how a compiler error gets an answer key instead of a hand-pasted transcript. Show the demo's source with `<!-- file:demo/… -->` blocks, never a copied fence. The rules above still hold: stems are unique repo-wide, `--only` goes with `--update`, and CI runs `--check` as its own job, since it needs the network. Every demo shares one target directory, `target/cargo-demos` (`CARGO_DEMOS_TARGET_DIR` moves it), so keep dependency versions and `syn` features the same across demos or each one compiles its own copy. That sharing is also why **every package in every demo needs a name no other demo uses** — `three_kinds_app`, never `app`. Cargo hashes a path package from its name, its path inside the workspace and its dependencies, and then trusts file times, so two identical-looking `app/` packages in two demos can be handed each other's compiled crate without a warning; the tool refuses a duplicate name rather than let that reach an answer key.
...
- **Link both ways, and across languages.** A page's *See also* names its neighbours, and each neighbour gets the return link in the same commit — a one-way link is found only by the reader who already started on the right page. The *If you are coming from another language* section links the matching page in a sibling library when one exists; the sites are `https://masiarek.github.io/<name>-learning-library/`, and a link is checked before it is written.
- **Register the names, not only the folder.** A folder has one name and a reader types another — *vector* for `the_vec`, *static const* for `const_and_static`. A new topic gets a row in [TOPICS.md](TOPICS.md) listing the other names it goes by, and a new type a row in [TYPES.md](TYPES.md). The section README row and `NAV_ORDER` are still needed; they answer *where does it sit*, the index answers *what is it called*.
...
- **A table has a lookup column — link the rows a reader would go and look up, and only those.** The shape of a table says *this cell is the thing, that cell is what to think about it*, so a column of bare code spans is a list of names the reader now has to search for by hand. It is usually the left column, but not always: on [Making a `String`](14_Strings/making_a_string/README.md) the left column is the *source type* and the methods sit on the right, so that is the side to link. Three kinds of row that earn nothing — what nobody looks up (`len()`), a cell that is a snippet rather than an item (`struct Ballot { … }`), and **the left column of a Python or ABAP comparison table**, which is foreign syntax and must stay bare (the Rust column beside it is the linkable one). The test for the rest is whether the destination has something to say that the row does not: `sort_unstable` earns a link because *unstable* is the surprising word and std explains it, `join` because the row claims it is a slice method and the link is the proof. The once-per-page rule still holds — two rows showing the same method with different arguments get one link, on the first. Worked example: the iterator table on [Walking a `String`](14_Strings/walking_a_string/README.md), where eight rows of nine are linked. Adam's rule of thumb, on seeing a table of six unlinked methods: *not all of them, but some.*
...
Five things that follow from that, worth knowing before you add a page:
...
- **A method reference is one sidebar row, not one row per method.** The four per-method references — [`str`](14_Strings/str_methods/README.md), [`String`](14_Strings/string_methods/README.md), [`Vec`](26_Collections/vec_methods/README.md) and [`slice`](26_Collections/slice_methods/README.md) — are 194 pages between them, and naming each one in the sidebar buried the section it lived in: standing on `str` methods, Strings was 83 rows deep and every neighbouring lesson had scrolled off the screen. They are listed in `not_in_nav` in `mkdocs.yml`, so they build, link and get searched exactly as before while the reference's own README does the directory work — grouped by what the methods are *for*, which a sidebar can only do alphabetically. A new method page therefore needs two things the sidebar used to give it free: a row in that README's tables, and a breadcrumb line directly under its H1 linking `../README.md` and `../../README.md`, which is the way back up for a reader who arrived from search.
...
python3 tools/run_cargo_demos.py  # the Cargo demos too (network the first time)
...
**Never chain the push onto the rebase.** `git rebase origin/master; git push origin HEAD:master` reads as one operation and is two, joined by a separator that does not care whether the first one worked. A rebase that stops on a conflict leaves `HEAD` **partway through the series** — the lesson commit replayed, the registration commit not — and the push then publishes exactly the committed-and-unregistered state the paragraph above says to avoid, while the terminal shows a conflict message directly above a successful push. It happened on `splitting_on_nothing`: `str::as_str` took K144 in the forty minutes the page was being written, the kata rows collided, and master carried an unregistered `## Practice` section until the registration commit could be re-derived and pushed a few minutes later. Rebase, look at `git status` and `git log --oneline -3`, then push as its own command. And do not trust an exit code you read through a pipe: `git rebase … | tail -10; echo "EXIT=$?"` prints the status of **`tail`**, so a cheerful `EXIT=0` sat directly under the conflict message and said nothing about the rebase at all.
