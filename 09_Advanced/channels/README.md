...
Kanał (*channel*) przenosi wartość z jednego wątku do drugiego **razem z własnością** — i to jest cały jego dowód bezpieczeństwa. `send` bierze wartość *przez wartość*, więc wątek wysyłający po prostu przestaje ją mieć i nie może jej już tknąć; pilnuje tego borrow checker, a nie żaden mechanizm czasu wykonania. Dlatego przy kanale nie ma po co zakładać blokady: w każdej chwili wartość ma dokładnie jednego właściciela. Skrót `mpsc` to *multi-producer, single-consumer* — wielu nadawców, jeden odbiorca — i widać to w typach: `Sender` implementuje `Clone`, `Receiver` nie. Warto też od razu przyjąć do wiadomości, że **kolejność nadejścia to decyzja planisty**, a nie twoja: jeśli porównujesz wyniki, musisz je posortować.

Najważniejsza pułapka tej strony jest cicha i kosztuje najwięcej czasu: pętla `for x in rx` kończy się dopiero wtedy, gdy zniknie **ostatni** `Sender`. Klony wysłane do wątków znikają razem z nimi, ale oryginalny `tx` w `main` — nie, dopóki go sam nie wypuścisz. Zapomniana linia `drop(tx)` daje program, który zrobił całą robotę i wisi: **bez komunikatu, bez paniki, bez niczego w wyjściu**. I tu jest rzecz, której polskie materiały nie powiedzą wprost — to **nie jest zakleszczenie** (*deadlock*), więc szukanie po haśle „zakleszczenie w Ruscie” nie doprowadzi cię do rozwiązania i żaden detektor zakleszczeń tego nie zgłosi. Kanał po prostu nigdy się nie zamknął. Albo wypuść nadawcę jawnie, albo w ogóle nie wiąż go z nazwą: rozdaj klony i pozwól oryginałowi umrzeć razem z blokiem, który go stworzył.

Stąd druga rzecz warta zapamiętania po polsku: *pusty* i *rozłączony* to **dwie różne odpowiedzi**, a nie dwa słowa na to samo. `Empty` znaczy „w tej chwili nic nie ma”, `Disconnected` znaczy „już nic nie będzie” — i tylko to drugie jest powodem, żeby przestać czekać. `recv()` blokuje do wartości albo do zamknięcia, `try_recv()` odpowiada natychmiast, `recv_timeout(d)` daje `Timeout`, czyli dokładnie to, na co `for x in rx` czekałoby w nieskończoność. Symetrycznie psuje się wysyłka: gdy odbiorcy już nie ma, wartość **wraca do ciebie** w środku `SendError` — bo funkcja, która wzięła własność i nie dowiozła, nie ma prawa jej po cichu zgubić.

Na koniec wybór, który po polsku bywa zamazywany słowem „współdzielenie”: kanał służy do **przekazania** wartości dalej (potok, kolejka zadań, wyniki zebrane z rozgałęzienia), a `Arc<Mutex<T>>` do **współdzielenia jednej** wartości (licznik, cache, rejestr). Pytanie brzmi: czy dane mają w każdej chwili właściciela, czy naprawdę należą do wszystkich naraz. Sięganie po `Mutex` tam, gdzie wystarczyłby kanał, to najprostszy sposób na dorobienie sobie blokady, o której trzeba potem myśleć — a blokada potrafi się zatruć, zakleszczyć i ustawić czytelników w kolejce. Jeśli przychodzisz z Javy albo Pythona, dołóż jeszcze jedno: `BlockingQueue` i `queue.Queue` przekazują *referencję*, więc pytanie „kto to jeszcze trzyma?” zostaje otwarte; w Ruscie kompilator zamyka je za ciebie. A `sync_channel(2)` to nie limit, tylko **przeciwciśnienie** (*backpressure*): kanał bez ograniczenia z szybkim producentem i wolnym konsumentem to wyciek pamięci w dobrym stylu.

**Szukaj po polsku:** kanały w Ruscie · przekazywanie własności między wątkami · `rust mpsc program hangs never exits` · `rust drop sender channel` · `rust sync_channel backpressure`
