...
Cała różnica mieści się w jednej linijce. W Reweighted Range Voting waga karty po *n*-tym mandacie jest funkcją **liczby całkowitej** — więc każdy mianownik, jaki może się w tych wyborach pojawić, da się wypisać, zanim ktokolwiek zagłosuje, wziąć z nich NWW i policzyć wszystko w `i128`. W Allocated Score waga tej rundy jest funkcją **wag z rundy poprzedniej**: mianownik powstaje z sumy wag tych kart, które akurat wylądowały na szczycie skali. Tego zbioru nie da się wyliczyć wcześniej, bo on nie istnieje, dopóki nie ma głosów. Strona nie kończy się jednak na „ta sztuczka tu nie działa” — najciekawsze jest to, **jak** to zawodzi.

Zawodzi tak, że potrzebna szerokość liczby jest własnością **kart do głosowania**, a nie wyborów. Osiem elektoratów identycznych parametrami (900 kart, 18 kandydatów, 12 mandatów), różniących się wyłącznie tym, kto jak zagłosował, potrzebuje mianowników od 27 do 39 bitów — jeden z nich wymaga około czterech tysięcy razy większego zakresu niż drugi. Nie da się więc dobrać typu całkowitego z góry, bo parametry wyborów nie są tym, od czego to zależy; nie da się też zmierzyć raz i zapisać stałej. Dlatego liczenie zwraca wyliczenie `Outcome` (`Exact` albo `Overflowed` z numerem mandatu), a każda operacja na ułamku zwraca `Option` — jedyny uczciwy projekt to taki, który **wykrywa**, zamiast zakładać.

Tu wchodzi rzecz, którą polska szkoła podaje na tacy, a większość implementacji ignoruje: **przepełnia się rachunek pośredni, nie wynik**. Dodawanie „na krzyż”, `a/b + c/d = (ad+cb)/bd`, pada na 16. mandacie z dwudziestu. Sprowadzenie obu ułamków do **NWW mianowników** — czyli dokładnie to, czego uczy się na ułamkach zwykłych w podstawówce — dowozi wszystkie dwadzieścia, a najszerszy mianownik, który trzeba było *zapisać*, ma 68 bitów, czyli połowę pojemności `i128`. Temu pierwszemu rachunkowi wcale nie zabrakło miejsca na odpowiedzi; zabrakło mu miejsca na iloczyn, który za chwilę i tak miał się skrócić. Stąd praktyczny wniosek: `None` z `checked_mul` mówi, że nie weszła **operacja**, a nie że **zadanie** jest za duże — najpierw patrz na drogę, potem na typ. To samo dotyczy porównania, bo `a/b < c/d` też mnoży na krzyż, a wcześniejsze skrócenie wspólnego czynnika przesunęło moment awarii o kilka mandatów.
