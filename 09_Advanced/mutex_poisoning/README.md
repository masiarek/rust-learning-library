...
<!-- source:mutex_poisoning_kata -->
*[`mutex_poisoning_kata.rs`](examples/mutex_poisoning_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:mutex_poisoning_kata -->
*Verified output of [`mutex_poisoning_kata.rs`](examples/mutex_poisoning_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*
...
Przy naprawie liczy się kolejność: `clear_poison()` kasuje **flagę**, a nie dane. Najpierw napraw niezmiennik, dopiero potem gaś ostrzeżenie — odwrotna kolejność zamienia głośny problem w cichy, czyli robi zamianę, na którą nikt świadomie się nie godzi. I pytanie, które warto sobie zadawać zamiast „czy `unwrap`ować blokadę”: **czy panika pomiędzy dwoma moimi zapisami może zostawić te dane mówiące nieprawdę?** Jeśli tak — flaga mówi o twoich danych. Jeśli nie — mówi tylko o cudzym wątku.
