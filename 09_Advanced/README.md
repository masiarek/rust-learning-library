...
| [The right to post is a value](one_account_one_review/README.md) | 301 | Authentication as a type rather than a boolean — a permission that cannot be forged, a `post` that spends it, and the hole move semantics still cannot close |
...
| [Catching a signal](catching_a_signal/README.md) | 301 | There is no `std::signal`, so Ctrl-C ends the process with no unwinding and no `Drop` — and catching it means a libc declaration plus a handler that may not print, allocate or lock |
