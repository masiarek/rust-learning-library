...
- `Ordering` — `Relaxed`, `Acquire`, `Release`, `SeqCst` — as the question *what else is this operation allowed to reorder around?*, with `Relaxed` as fine for a counter nobody synchronises on — and not with `SeqCst` as a "safe default", which [atomic orderings](../atomic_orderings/README.md) argues against
...
- [Atomic orderings](../atomic_orderings/README.md) · [Compare and exchange](../compare_and_exchange/README.md) · [False sharing](../false_sharing/README.md) — the three pages below the atomics (stubs)
