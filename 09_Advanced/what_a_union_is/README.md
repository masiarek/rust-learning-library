...
`union` to unia w dokładnie tym sensie, co w C, i polskie podręczniki C używają tego słowa od zawsze — więc termin jest oswojony, choć w codziennej polszczyźnie „unia” brzmi raczej politycznie. Różnica wobec struktury jest arytmetyczna: rozmiar struktury to suma pól, rozmiar unii to jej **największe** pole zaokrąglone do wyrównania, bo wszystkie pola leżą na tych samych bajtach. Struktura odpowiada na oba pytania naraz; unia odpowiada na dokładnie jedno i nie powie ci, na które. Trzecie słowo z tego zestawu — `struct Unit;` — nie ma z tym nic wspólnego poza podobieństwem liter: to pusta struktura, zero bajtów, zero pól.

Asymetria, którą warto zapamiętać: **zapis do unii jest bezpieczny, odczyt wymaga `unsafe`**. Zapis tylko kładzie bajty w pamięci; odczyt je *interpretuje*, a w unii nie ma niczego, co zapisywałoby, która interpretacja jest w tej chwili żywa — `unsafe` jest twoim podpisem pod tym, że wiesz. I tu częste uproszczenie: to nie unia decyduje, czy odczyt jest niezdefiniowanym zachowaniem (*undefined behaviour*), tylko **typ pola**. Odczytanie bitów `1.0f32` jako `u32` jest w pełni określone, bo każdy 32-bitowy układ bitów to poprawny `u32`; zamień pole na `bool`, a ten sam odczyt to natychmiastowe UB, bo poprawne są wyłącznie `0` i `1`. Dwie reguły dodatkowo zaskakują: pożyczenie jednego pola pożycza **całą** unię (pola się nakładają, więc borrow checker widzi jedno miejsce i drugie `&mut` to `E0499`, podczas gdy w strukturze te same dwie linie są w porządku), a pole unii nie może wymagać sprzątania — `String` jest odrzucany przez `E0740`, bo bez etykiety `drop` nie wiedziałby, czy jest co zwalniać; `ManuallyDrop` to furtka, która przenosi ten obowiązek na ciebie.

Najważniejsze zdanie tej strony brzmi po polsku ciekawiej niż po angielsku, bo polszczyzna ma na to dwa słowa z dwóch różnych tradycji: to, co teoria typów i materiały o programowaniu funkcyjnym nazywają **typem sumarycznym** (albo wariantowym), a podręczniki C nazywają po prostu **unią**, różni się w Ruscie dokładnie jedną rzeczą — etykietą, którą `match` każe ci przeczytać. Wyliczenie (`enum`) to unia z etykietą (*tagged union*); `union` to ta sama nakładka na pamięć, tylko bez niej. Stąd pułapka dla osób przychodzących z Pythona: `typing.Union[int, str]` **nie** jest unią w sensie C, lecz sprawdzanym wyborem, czyli odpowiednikiem rustowego `enum`a — prawdziwym odpowiednikiem unii jest `ctypes.Union`. Praktyczny wniosek: `union` spotkasz w rustowym kodzie prawie wyłącznie na granicy FFI z C, gdzie układ pamięci nie jest twoim wyborem, a do samego przeinterpretowania bitów sięgnij najpierw po std — `f32::to_bits` robi to samo bez ani jednego `unsafe`.

**Szukaj po polsku:** unia w języku C · typ wariantowy · typ sumaryczny · `rust union vs enum` · `rust tagged union` · `E0740`
