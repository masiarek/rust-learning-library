...
One assignment per path is not a mutation, so the binding stays immutable — and the compiler proves it was set before the read. Miss a path and you get `E0381`. The case where this is the *right* tool rather than an `Option` is [initial values](../initial_values/README.md).
...
- [Initial values](../initial_values/README.md) — deferred initialization as the deliberate tool
