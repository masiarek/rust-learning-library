...
An expression computes; a pattern describes a shape. `30` does not match the pattern `8 | 12 | 18`, and there is no way to ask a pattern for the bitwise answer — patterns cannot compute at all. If you want the [bit mask](../bit_flags/README.md), that is a different tool on the other side of the `=>`.
...
Mid-refactor that warning is bookkeeping: the dead arm and the live one give the same answer, so deleting it changes nothing. Worth [reading rather than clearing](../what_a_warning_is_asking/README.md), though, because it is the *same warning* when they disagree.
...
```bash
rustc --edition 2024 01_Foundations/one_arm_many_values/examples/one_arm_many_values.rs -o /tmp/oamv && /tmp/oamv
```
