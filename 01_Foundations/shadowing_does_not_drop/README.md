...
[Shadowing and `unwrap`](../shadowing_and_unwrap/README.md) covers what shadowing is *for*. This page covers what it does to the value underneath, because "the new one hides the old" leaves the interesting question unasked: what happened to the old one?
...
```bash
rustc --edition 2024 01_Foundations/shadowing_does_not_drop/examples/shadowing_does_not_drop.rs -o /tmp/sdnd && /tmp/sdnd
```
