//! A STAR score is not a number — it is an integer 0..=5.
//!
//! `u8` holds 256 values. A STAR ballot cell has 6 legal ones. This program is
//! about the gap between those two facts, and about paying the validation cost
//! exactly once instead of at every call site that ever touches a score.

mod ballot {
    /// A validated STAR score. The field is private *outside this module*, so
...
fn main() {
    rule("Step 1: A bare u8 has no opinion about STAR");
    ...
    println!("      ballot just outvoted ten honest ones, and no type objected.");
    ...
    rule("Step 3: A whole ballot row, accepted or rejected as a unit");
    ...
    println!("      illegal cell means you never hold a half-valid ballot.");
    ...
    println!("      like the numbers they wrap — which is what a runoff needs.");

    rule("Step 5: What actually closes the door is the module boundary");

    println!("  Inside `mod ballot`, `Score(7)` still compiles — privacy is per");
