...
**Four causes, one `None`.** A config line gives a seat count. It can be missing, non-numeric, zero, or larger than the ballot supports. Write the version that returns `Option<u32>`, then the version that keeps the reason, and print both over the same five inputs.
...
<!-- source:none_on_error_kata -->
*[`none_on_error_kata.rs`](examples/none_on_error_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:none_on_error_kata -->
*Verified output of [`none_on_error_kata.rs`](examples/none_on_error_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*
