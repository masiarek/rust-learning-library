...
`bool` being one byte is the ordinary answer — it needs one bit and gets a byte, because a byte is the smallest addressable thing. The two `Option` rows are the interesting ones: wrapping a type in `Option` normally costs a tag, but a `bool` only uses two of its 256 patterns and a `NonZeroU8` never uses `0`, so the compiler stores `None` *in* the value rather than beside it. That is the [niche optimization](../option_as_collection/README.md), and it is the same mechanism that makes [`Option<Box<T>>`](../nullable_pointers/README.md) free.
...
The boundary between the two worlds is `String::from_utf8`, and it hands back a [`Result`](../option_vs_result/README.md) rather than a string — because arbitrary bytes are not necessarily text:
...
- [What is a ballot, in memory?](../representing_a_ballot/README.md) — the layer above: once you have the bytes, which container to put them in
