...
//!
//! The ballots come from the sibling star-voting-library, whose engine accepts
//! five marker characters alongside the digits 0-5. Every marker tabulates as
//! zero. They are not the same thing.
//!
//!   rustc --edition 2024 six_kinds_of_zero.rs -o /tmp/skz && /tmp/skz

/// A score a voter actually wrote — six variants, and no seventh to write.
...
/// One cell of one ballot: either a score, or one of five reasons there isn't one.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Mark {
    Scored(Stars),       // '0'..='5'
    Blank,               // '-'  the box was left empty
    RaceAbstention,      // '~'  the voter skipped this whole race
    CandidateAbstention, // '&'  the voter declined on this candidate
    Spoiled,             // '?'  unreadable, or two bubbles in one row
    SpoiledReissued,     // '%'  spoiled, and a replacement ballot was issued
}
...
impl TryFrom<char> for Mark {
    ...
    fn try_from(c: char) -> Result<Mark, char> {
        Ok(match c {
            '0'..='5' => Mark::Scored(Stars::new(c as u8 - b'0').expect("0-5")),
            '-' => Mark::Blank,
            '~' => Mark::RaceAbstention,
...
impl Mark {
    /// What the tally adds. Five of the six collapse to zero — deliberately.
    ...
    fn label(self) -> &'static str {
        match self {
            ...
            Mark::Blank => "blank",
            Mark::RaceAbstention => "race abstention",
            Mark::CandidateAbstention => "candidate abstention",
            Mark::Spoiled => "spoiled",
            Mark::SpoiledReissued => "spoiled + reissued",
        }
    }

    /// Only paper can produce these. An online form prevents them at input.
    fn paper_only(self) -> bool {
        match self {
            Mark::Scored(_) | Mark::Blank => false,
            Mark::RaceAbstention
            | Mark::CandidateAbstention
            | Mark::Spoiled
            | Mark::SpoiledReissued => true,
        }
    }
}
...
fn main() {
    ...
    let vocabulary = ['0', '-', '~', '&', '?', '%'];
    println!("  char   variant                          tallies as");
    for c in vocabulary {
        let m = Mark::try_from(c).unwrap();
        println!("   {c}     {:<32} {}", m.label(), m.tally_value());
    }
    ...
    println!("      Some(Stars(0)) is the voter who wrote a zero. The five Nones");
    println!("      are five different voters, and Option has one shape for all");
    ...
    println!("      compiler will happily add it to a tally, and 0..=5 no longer");
    println!("      describes the type. You have widened the thing you narrowed.");
    let scores = vec![0u8, 0, 0, 0, 0, 0];
    let is_marker = vec![false, true, true, true, true, true];
    println!("  parallel flags scores {scores:?} is_marker {is_marker:?}");
    println!("      Two Vecs that must be edited together — the desync bug from");
    println!("      the ballot lesson, reintroduced. And it still cannot say WHICH");
    println!("      marker, so a third parallel Vec is coming.");

    banner("Step 4: The enum, and a projection that is lossy on purpose");
    let ballot = parse_row("5,-,3,~,?,%");
    println!("  ballot  5,-,3,~,?,%");
    for (i, m) in ballot.iter().enumerate() {
        println!(
            "    cell {i}  {:<20} -> tallies {}   paper-only: {}",
            m.label(),
            m.tally_value(),
            m.paper_only()
        );
    }
    let total: u32 = ballot.iter().map(|m| m.tally_value() as u32).sum();
    println!("  total = {total}");
    println!("      tally_value() throws five distinctions away, and that is");
    ...
    for channel in [Channel::Paper, Channel::Online] {
        let marks: Vec<Mark> = election
            .iter()
            .filter(|(c, _)| *c == channel)
            .flat_map(|(_, row)| parse_row(row))
            .collect();
        let non_scores = marks.iter().filter(|m| !matches!(m, Mark::Scored(_))).count();
        let paper_only = marks.iter().filter(|m| m.paper_only()).count();
        println!(
            "  {channel:?}: {} cells, {non_scores} non-scores, {paper_only} of them paper-only",
            marks.len()
        );
    }
    println!("      Pooled, that reads \"paper voters are sloppier.\" Split, it reads");
    println!("      \"only paper voters CAN produce four of the six marks.\" Same");
    ...
    println!("      Six score values plus five markers is eleven states, and a byte");
