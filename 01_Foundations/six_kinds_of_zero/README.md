...
[Blank is not zero](../representing_a_ballot/README.md#blank-is-not-zero) already made half this argument: `Vec<Option<Stars>>` remembers that a box was never marked, where `Vec<Stars>` cannot. That is the right type when there are exactly **two** cases.
...
```bash
rustc --edition 2024 01_Foundations/six_kinds_of_zero/examples/six_kinds_of_zero.rs -o /tmp/skz && /tmp/skz
```

## See also

- [What is a ballot, in memory?](../representing_a_ballot/README.md) — the rung below; where "blank is not zero" stops at two cases
- [A score is not a number](../newtype_score/README.md) — the `Stars` enum this page wraps, and why it is an enum
