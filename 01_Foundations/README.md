...
There is also a slow, optional thread running through several of the pages above — [the long way round to a column summary](../ROADMAP.md), which sequences a handful of lessons so each one is the next thing Rust wants to teach. `newtype_score`, `representing_a_record` and `six_kinds_of_zero` are its first rungs. They stand alone as Rust lessons; the tool is the excuse.
