...
`<T>` czyta się najprościej tak: to **lista parametrów typowych**, dokładnie w tym sensie, w jakim `(value: T)` jest listą parametrów wartościowych. Pojawia się tam, gdzie parametr jest *deklarowany* (`struct Container<T>`, `fn largest<T>(…)`), i drugi raz tam, gdzie już zadeklarowany jest *używany* (`Container<u8>`). Zapis `impl<T> Container<T>` wygląda przez to na masło maślane, a nie jest: pierwsze `<T>` wprowadza nazwę na potrzeby bloku, drugie mówi, **dla jakiego typu** ten blok obowiązuje — i właśnie tym różni się od `impl Container<u8>`, całkiem legalnego bloku dotyczącego jednego jedynego pojemnika. Samo `T` to konwencja, nie słowo kluczowe; przy trzech parametrach czytelniejsza bywa pełna nazwa (`Container<Payload>`) — ale identyfikatory w kodzie zostawiaj po angielsku, bo kod czyta się globalnie.

Największa różnica wobec tego, co polski czytelnik zwykle zna z Javy, to brak wymazywania typów (*type erasure*). W Javie `List<Integer>` i `List<String>` to w czasie działania jedna klasa, `T` staje się `Object`, a `List<int>` w ogóle nie istnieje — trzeba opakować. Rust robi coś przeciwnego: **monomorfizuje** (*monomorphization*), czyli kompiluje osobną kopię kodu dla każdego użytego typu, więc `Container<u8>` zajmuje 1 bajt, `Container<i64>` 8, a `Container<[u8; 16]>` — 16. Tyle, ile samo `T`, bez znacznika, bez wskaźnika i bez opakowania. Płaci się za to czasem kompilacji i rozmiarem binarki, nigdy szybkością.

Z monomorfizacji wynika też rzecz, która na początku irytuje: dwa różne wypełnienia to **dwa niespokrewnione typy**. `Vec<Container<u8>>` nie przyjmie `Container<&str>` i dostaniesz `error[E0308]: mismatched types … expected u8, found &str`. To nie jest generyczność narzekająca, tylko generyczność działająca. Jeśli naprawdę potrzebujesz jednej kolekcji trzymającej różne typy naraz, żadna ilość `<T>` tego nie załatwi — od tego jest `dyn Trait`, a wybór między nimi jest pytaniem o twój program, nie o wydajność.

Ostatnia rzecz jest tą, przy której najczęściej odbijają się osoby po C++. Rust sprawdza **samą definicję**, wobec zadeklarowanych ograniczeń (*trait bounds*), zanim ktokolwiek ją wywoła. `T` bez ograniczeń da się przechować, przenieść i wypuścić — i to jest cała lista; nie da się go porównać, wypisać ani dodać, bo nic tego nie obiecało. Dlatego `largest` deklaruje `T: PartialOrd + Copy`, a funkcja bez ograniczenia nie kompiluje się w miejscu, w którym ją piszesz: `error[E0369]: cannot add T to T`, z podpowiedzią `consider restricting type parameter T with trait Add`. W szablonie C++ ten sam błąd wysypałby się dopiero przy tworzeniu instancji, w cudzym nagłówku. Koszt generyczności jest natomiast po stronie czytelnika, nie maszyny — dlatego dobra zasada brzmi: napisz najpierw wersję konkretną, a po `<T>` sięgnij, gdy zabierasz się za pisanie jej po raz drugi.

**Szukaj po polsku:** typy generyczne w Ruscie · parametr typu · monomorfizacja · ograniczenie cechy · `rust impl<T> generic struct` · `rust E0369 cannot add T to T`
