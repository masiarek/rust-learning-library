...
Cała ta strona wisi na jednej różnicy: `to_owned()` **kupuje nowy bufor**, a `clone_into(&mut buf)` **napełnia ten, który już masz**. W pętli po czterech wierszach to cztery przydziały pamięci przeciwko zeru — i nie jest to oszacowanie, tylko pomiar, bo przykład podmienia globalny alokator na liczący. Najważniejsze zdanie jest tu jednak inne i dotyczy słowa, które w polskich materiałach niesie najwięcej nieporozumień: `mut`. Dwie z czterech pętli w zadaniu piszą przez `let mut` i **mimo to alokują pięć razy**. Mutowalność dotyczy **nazwy** — tego, czy wolno ją związać na nowo — a nie bufora leżącego na stercie; bufor wykorzystuje ponownie *metoda*, którą wywołasz, a nie deklaracja zmiennej. `buf = row.to_owned()` zwalnia stary łańcuch i kupuje nowy, choć po lewej stronie stoi `mut`, i to jest cały mechanizm tej strony w jednym zdaniu.

Druga rzecz, o którą potyka się każdy, to kierunek. Metodę wywołuje się **na źródle**, nie na celu: `src.clone_into(&mut dst)` przenosi dane w prawo, a `dst.clone_from(&src)` w lewo — dwie metody robiące to samo, wskazujące w przeciwne strony, w tej samej bibliotece standardowej. To, którą wolno wywołać, jest pytaniem o typy, a nie o gust: `clone_from` wymaga, by obie strony były już tym samym typem posiadającym dane, więc przejście `&str` → `String` albo `&[T]` → `Vec<T>` ma do dyspozycji wyłącznie `clone_into`. Historia, którą strona przytacza, jest pouczająca sama w sobie: zastrzeżenie do tego kierunku zgłoszono w tym samym pull requeście, który metodę wprowadził, w 2017 roku, nigdy na nie nie odpowiedziano, a metoda ustabilizowała się pięć lat później, bo potrzebował jej `Cow::clone_from`. Polskie materiały praktycznie nigdy nie schodzą na ten poziom, a odpowiedź na pytanie „dlaczego to się tak dziwnie nazywa?” prawie zawsze leży w angielskim pull requeście albo w tracking issue, nie w dokumentacji.

Trzy powody, dla których ta optymalizacja **nie** płaci — każdy łatwy do przeoczenia:

- **Cel musi mieć miejsce.** Pusty `String` przy pierwszym `clone_into` zaalokuje dokładnie tak samo jak `to_owned`, a za mały — zrealokuje. Oszczędność zaczyna się dopiero od drugiego wywołania, dlatego bufor wynosi się *przed* pętlę, a nigdy nie tworzy w środku.
- **Bufor pamięta swój największy rozmiar.** Po wierszu 300-bajtowym pojemność zostaje 300 i taka już zostanie. Wymieniasz pamięć na ruch alokatora — nie wygrywasz obu naraz.
- **`#[derive(Clone)]` nie daje z tego nic.** Derive generuje `clone` i tylko `clone`, więc `clone_from` spada na domyślną implementację `*self = source.clone()` i alokuje, choć w celu czekały wolne 64 bajty. Zespół libs-api odrzucił zmianę świadomie w 2022 roku i nie tylko z powodów wydajnościowych: wersja domyślna buduje całą nową wartość i dopiero ją przypisuje, więc panika w połowie zostawia cel nietknięty, podczas gdy wersja pole po polu może go zostawić zaktualizowanego do połowy. Wybierając szybszą ścieżkę, wybierasz też słabszą gwarancję.

Na koniec praktyczna uwaga o clippy, bo to zwykle on przyprowadza ludzi na tę stronę. Lint `assigning_clones` przepisuje `s = src.to_owned()` na `src.clone_into(&mut s)` i **od wersji 1.80 siedzi w grupie `pedantic`**, czyli odzywa się tylko na żądanie; kto pamięta, że krzyczał sam z siebie, pamięta 1.79 albo starszą. Jego własne sformułowanie jest zresztą asekuracyjne — „**może** być nieefektywne” — i słusznie, bo w miejscu, w którym odzywa się najczęściej (dane testowe z jednym nadpisanym polem), przepisanie potrafi zamienić jedną alokację na jedną realokację i nie dać nic. W kodzie testowym oznaczonym `#[cfg(test)]` clippy ten lint w ogóle pomija — decyduje atrybut, nie nazwa modułu, więc moduł nazwany po prostu `tests` dostanie ostrzeżenie jak każdy inny.

**Szukaj po polsku:** ponowne użycie bufora · alokacje na stercie · pojemność łańcucha znaków · `rust clone_from vs clone_into` · `clippy assigning_clones` · `derive Clone clone_from wontfix`
