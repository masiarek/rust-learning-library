...
**And check that the allocation happened at all.** Every figure above keeps the new `String` alive by passing it to `black_box`, and those loops call `__rust_alloc` and `__rust_dealloc` once per iteration in the disassembly. Let the `String` drop unused instead — `let s = "thirteen byte".to_owned(); drop(s);` — and the same loop measures 6 ns on 1.98.0, because the optimizer deletes the allocation: that loop has no allocator call left in it. A counting global allocator does not catch this. It still reported one allocation per iteration for the 6 ns loop, because its counter survived inlining and the `malloc`/`free` pair did not. The disassembly is the check; a counter is not.
...
- [Reading the `Clone for &T` hover](../reading_the_clone_hover/README.md) — the impl `.clone()` on a `&str` lands on, read line by line from the editor's hover
