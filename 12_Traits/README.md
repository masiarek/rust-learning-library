...
| [Extension traits](extension_traits/README.md) | 201 | A trait you declare, implemented for a type std owns: `E0116` and the two escapes its `help:` line names, the orphan rule as a permission rather than an obstacle, the blanket impl that reaches every iterator ever written, and the inherent method that beats yours with no warning of any kind |
