...
| `PhantomData<*const T>` | it merely points at one | covariant (`PhantomData<*mut T>` is the invariant one), owns nothing, and not `Send`/`Sync` |
...
Typ fantomowy (*phantom type*) to parametr typu, którego struktura nigdy nie przechowuje. `Ballot<Star>` i `Ballot<Approval>` mają te same pola, ten sam rozmiar i ten sam kod maszynowy — i mimo to nie są tym samym typem, więc funkcja przyjmująca jeden nie przyjmie drugiego. Nic o tym, „do której metody należy ta karta”, nie jest nigdzie zapisane ani sprawdzane w czasie działania: cała różnica mieszka w kontrolerze typów i znika, zanim program wystartuje. Znacznik zajmuje zero bajtów.

Kompilator prowadzi tu za rękę, ale dwoma komunikatami po kolei, i drugi jest zwykle zaskoczeniem. Najpierw `E0392` — *type parameter `Method` is never used* — z trzema wyjściami w linijce `help:`: usuń parametr, trzymaj w polu coś tego typu, albo powiedz jednym polem, że zostawiasz go świadomie. Dopiero potem przychodzi `E0282`, *type annotations needed*: skoro żadne dane nie zawężają `Method`, to nic go nie zawęzi, więc każda konstrukcja musi mieć adnotację albo turbofisha (`let star: Ballot<Star> = …`, `Ballot::<Star>::new(…)`). To nie jest narzut do obejścia — to jedyne miejsce, w którym wybór znacznika w ogóle jest zapisany. Uwaga praktyczna dla czytających po polsku: starsze materiały cytują `E0392` w innym brzmieniu (bez słowa *type*), więc rozpoznawaj te sytuacje **po kodzie błędu**, a nie po zdaniu — większość polskich tekstów o Ruscie ma dobrych kilka lat.

Największą pułapką jest tu samo polskie słowo. „Fantomowy” podpowiada „pozorny, na niby”, a stąd już blisko do wniosku, że `PhantomData` to atrapa wstawiona po to, żeby uciszyć kompilator. Jest dokładnie odwrotnie: to **deklaracja**, że struktura zachowuje się tak, jakby trzymała `T`. Trzy zapisy składają trzy różne obietnice — `PhantomData<T>` mówi „jestem właścicielem `T`” (drop-check i kowariancja), `PhantomData<fn() -> T>` mówi „tylko produkuję `T`”, a `PhantomData<*const T>` mówi „tylko na niego wskazuję” (brak `Send`/`Sync`). Wariancja jest we wszystkich trzech ta sama — kowariancja względem `T`; niezmienniczy byłby dopiero `PhantomData<*mut T>`. Wszystkie trzy zajmują zero bajtów, więc różnicy nie widać w układzie pamięci — widać ją dopiero w tym, co przepuści borrow checker. Dla zwykłego znacznika, którego nikt nigdy nie będzie posiadał, właściwy jest ten pierwszy.

Zysk jest podwójny. Po pierwsze, `impl Ballot<Star>` istnieje wyłącznie dla jednego znacznika: `max_score` nie robi `match` na przechowywanym wyliczeniu i nie czyta pola z piątką — wartość została wybrana wtedy, gdy wybrano typ. Po drugie, znacznik może zapisywać **stan** wartości (wzorzec *typestate*, bez utrwalonej polskiej nazwy): `Paper<Blank>` po prostu nie ma metody `cast`, a `mark` bierze `self`, więc pustej karty nie da się użyć drugi raz. Warto przy tym robić znaczniki typami pustymi — `enum Star {}` zamiast `struct Star;` — bo typ bez wariantów można tylko *nazwać*, nikt nie stworzy przypadkiem jego wartości. Rachunek za to wszystko jest jednak realny: nie ma kolekcji mieszanej (`vec![star, approval]` się nie kompiluje), nie ma o co zapytać w czasie działania (`ballot.method()` nie istnieje) i przy każdej konstrukcji trzeba pisać adnotację. Stąd zasada: sięgaj po typ fantomowy wtedy, gdy pomieszanie dwóch rodzajów jest **błędem**, a nie przypadkiem do obsłużenia — metry i stopy, dane zwalidowane i surowe, karty z tych wyborów i z tamtych.

**Szukaj po polsku:** typy fantomowe · znacznik typu · typy puste · `rust PhantomData` · `rust E0392 type parameter is never used` · `rust typestate pattern`
