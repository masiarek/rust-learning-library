...
- **Two extension traits, one method name**, both in scope — `E0034: multiple applicable items in scope`, with both candidates named. The fix is the long spelling, `A::shout(&x)`. Real crates collide like this: `Itertools::intersperse` and the standard library's own `intersperse` did, for a whole release cycle.
