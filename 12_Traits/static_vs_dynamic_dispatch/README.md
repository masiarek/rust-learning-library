...
- [Dyn compatibility](../../39_API_Design/dyn_compatibility/README.md) — `E0038` and designing a trait so it can be `dyn` (stub)

## Po polsku

Rozróżnienie, o które tu chodzi, po polsku nazywa się zwykle **wiązaniem statycznym i dynamicznym** — nazwa jest dobra pod warunkiem, że nie przyniesie się ze sobą całej intuicji z C++. `<P: Processor>` to monomorfizacja: kompilator wytwarza osobną kopię funkcji dla każdego konkretnego typu, więc wywołanie jest bezpośrednie i daje się wstawić w miejscu (*inline*), a płaci się rozmiarem wynikowego kodu. `&dyn Processor` to jedna kopia na zawsze, a jej argument jest **grubym wskaźnikiem**: dwa słowa maszynowe, jedno na dane, drugie na tablicę metod wirtualnych. Różnicę wobec C++ widać wprost w wydruku powyżej — tam wskaźnik do tablicy siedzi **wewnątrz obiektu**, tutaj siedzi we wskaźniku, dlatego `Risc` zajmuje 0 bajtów, a `&dyn Processor` szesnaście.

Pytanie, które naprawdę rozstrzyga wybór, nie brzmi „co jest szybsze”, tylko **czy da się nazwać typ**. `Vec<P>` trzyma jeden konkretny `P` i żadne typy generyczne tego nie obejdą; `Vec<Box<dyn Processor>>` działa właśnie dlatego, że wszystkie jego elementy *są* tego samego typu — opakowanym obiektem cechy (*trait object*). Gdy potrzebna jest kolekcja różnorodna, rejestr wtyczek albo wartość wybierana w czasie działania, `dyn` nie jest kompromisem wydajnościowym, tylko jedynym zapisem, który taki program w ogóle wyraża.

Dwie rzeczy, na których polskojęzyczny czytelnik traci przy tej stronie najwięcej czasu:

- **Zmiana nazwy: *object safety* → *dyn compatibility*.** Książki, wpisy i odpowiedzi na Stack Overflow — a tym bardziej nieliczne polskie omówienia, w których spotyka się „bezpieczeństwo obiektowe” — używają starej nazwy, podczas gdy kompilator wypisuje `the trait ... is not dyn compatible` (`E0038`). To jedno i to samo pojęcie, więc szukać trzeba obu haseł naraz.
- **`dyn` nie sprawdza typu w czasie działania.** Bardzo rozpowszechniona nieprawda, także w tekstach angielskich (jeden jest wskazany z nazwiska na końcu tej strony). Obiekt cechy nie niesie żadnego znacznika typu: wskaźnik do tablicy metod wybiera się raz, w miejscu utworzenia `&dyn`, a potem jest już tylko odczyt i skok. Odpowiednika `instanceof` z Javy ani `dynamic_cast` z C++ tu nie ma — identyczność typu w czasie działania jest opcjonalna i nazywa się `std::any::Any`.

Zostaje trzecia droga, którą podział „statycznie albo dynamicznie” skutecznie zasłania: jedno **wyliczenie** (*enum*) z wariantem na każdą implementację i `match` w implementacji cechy. Daje ono różnorodny `Vec` — czyli to, czego typy generyczne nie potrafiły — zachowując bezpośrednie wywołania i wartości trzymane w miejscu, jeden bajt zamiast szesnastu. Kosztem jest **otwartość**: lista wariantów jest zamknięta i mieszka w twoim crate, więc cudzy kod nie może do niej dołączyć, a każda nowa implementacja wymusza poprawkę w każdym `match`u. Wyliczenie wybiera się wtedy, gdy zbiór implementacji jest naprawdę zamknięty — `dyn` wtedy, gdy musi zostać otwarty.

**Szukaj po polsku:** wiązanie statyczne i dynamiczne · monomorfizacja · tablica metod wirtualnych · obiekt cechy · `rust dyn compatibility object safety` · `rust E0038 not dyn compatible`
