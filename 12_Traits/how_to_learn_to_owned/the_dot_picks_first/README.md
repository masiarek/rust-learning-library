...
**Predict the receiver, then take it away from the dot.** With `owner: String`, `r = &owner`, `rr = &r`, `s: &str = "Ada"`, `ss = &s`, `shared = Rc::new(String::from("Ada"))` and `m: &mut String`, write down the type of `r.clone()`, `rr.clone()`, `ss.to_owned()`, `shared.clone()` and `m.clone()` before you run anything. Then print each with `std::any::type_name_of_val`.

Three of the five are not a `String`. Fix each of those three two ways: once by dereferencing to the rung you want, and once with a fully qualified call that names the type, so the dot never searches at all.

<details markdown="1">
<summary><strong>Solution</strong></summary>

<!-- source:the_dot_picks_first_kata -->
*[`the_dot_picks_first_kata.rs`](examples/the_dot_picks_first_kata.rs) in full — pasted here by `tools/run_examples.py` from the file CI compiles and runs.*
...
<!-- output:the_dot_picks_first_kata -->
*Verified output of [`the_dot_picks_first_kata.rs`](examples/the_dot_picks_first_kata.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. Predict, then check
   r.clone()       r: &String     -> alloc::string::String
   rr.clone()      rr: &&String   -> &alloc::string::String
   ss.to_owned()   ss: &&str      -> &str
   shared.clone()  shared: Rc     -> alloc::rc::Rc<alloc::string::String>
   m.clone()       m: &mut String -> alloc::string::String

2. The three that were not a String, fixed by dereferencing to the right rung
   (*rr).clone()        -> alloc::string::String
   (*ss).to_owned()     -> alloc::string::String
   (*shared).clone()    -> alloc::string::String

3. The same three with no search at all: name the type, and the argument is coerced
   <String as Clone>::clone(rr)       -> alloc::string::String
   <str as ToOwned>::to_owned(ss)     -> alloc::string::String
   <String as Clone>::clone(&shared)  -> alloc::string::String
```
<!-- /output -->

The fully qualified calls accept `rr` and `&shared` although neither is a `&String`: once `Self` is named, the argument is an ordinary function argument, and deref coercion shortens it — [name the trait, and the `&` you pass decides](#name-the-trait-and-the-you-pass-decides).
...
- [Reading the `Clone for &T` hover](../../reading_the_clone_hover/README.md), the impl the dot lands on for `.clone()` on a `&str`, as the editor shows it
...
**Kata:** [Predict the receiver, then take it away from the dot](#practice), below.
