...
//! `::tested_describe`, the facade, because that is the one crate a user
//! depends on.
...
pub fn derive_describe(input: TokenStream) -> TokenStream {
    ...
    quote! {
        impl ::tested_describe::Describe for #name {
            fn describe(&self) -> ::std::string::String {
                ::tested_describe::__private::describe_fields(#type_name, &[#((#labels, &self.#idents)),*])
            }
        }
    }
    .into()
