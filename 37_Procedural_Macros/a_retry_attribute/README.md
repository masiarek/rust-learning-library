...
The first attempt moved `message` into `deliver`, and there is nothing left for the second. The labels land on the attribute line because the closure and the loop are tokens the macro wrote, and [`quote!` gives those tokens ↗](https://docs.rs/crate/quote/1.0.47/source/src/lib.rs#169) the call site's span, which for an attribute is the attribute. So `retry` works on functions whose attempts can be repeated: arguments taken by reference, as both functions in `app` take `&Cell`, or cloned inside the body, as the `help` suggests.
