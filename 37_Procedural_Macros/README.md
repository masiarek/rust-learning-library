...
| [Expanding `thiserror`](expanding_thiserror/README.md) *(outline)* | Every line `#[derive(Error)]` writes, read from the expansion |
| [A function, `macro_rules!`, or a procedural macro](function_macro_rules_or_proc_macro/README.md) *(outline)* | When a procedural macro is the right call, and what it costs when it is not |

## 2. The toolkit

| Lesson | What it answers |
|---|---|
| [A proc-macro crate](a_proc_macro_crate/README.md) *(outline)* | Why the macros need a crate of their own, and what that crate may export |
| [Tokens and token streams](tokens_and_token_streams/README.md) *(outline)* | The four kinds of token tree a macro reads and writes |
...
| Lesson | What it answers |
|---|---|
| [Every struct and enum shape](every_struct_and_enum_shape/README.md) *(outline)* | Named, tuple and unit fields, and enums with all three |
| [Absolute paths and hygiene](absolute_paths_and_hygiene/README.md) *(outline)* | Generated code that compiles in a crate with its own `Result` |
| [Errors: from `panic!` to `syn::Error`](errors_from_panic_to_syn_error/README.md) *(outline)* | Three ways to fail, and the one that underlines the right token |
| [Helper attributes by hand](helper_attributes_by_hand/README.md) *(outline)* | Container and field attributes, parsed with `parse_nested_meta` |
| [Helper attributes with `darling`](helper_attributes_with_darling/README.md) *(outline)* | The same attributes, declared as a struct |
| [Generics, lifetimes and `where`](generics_lifetimes_and_where/README.md) *(outline)* | An `impl` that repeats every parameter and bound the type has |

## 4. Function-like macros

| Lesson | What it answers |
|---|---|
| [Why `println!` is a macro](why_println_is_a_macro/README.md) *(outline)* | What no function signature can take, and what `format_args!` checks while compiling |
| [Parsing arbitrary tokens](parsing_arbitrary_tokens/README.md) *(outline)* | Your own syntax between the delimiters, with `syn::parse::Parse` |
| [When `macro_rules!` runs out](when_macro_rules_runs_out/README.md) *(outline)* | The exact point a declarative macro stops, and a procedural one starts to pay |
| [A `routes!` macro](a_routes_macro/README.md) *(outline)* | A small DSL, checked at compile time |

## 5. Attribute macros

| Lesson | What it answers |
|---|---|
| [Parse, tweak, re-emit](parse_tweak_reemit/README.md) *(outline)* | Changing a function and handing all of it back |
| [Re-emit the item on error](re_emit_on_error/README.md) *(outline)* | One error for the user instead of one per call site |
| [A `#[retry]` attribute](a_retry_attribute/README.md) *(outline)* | `#[retry(times = 3, delay_ms = 100)]`, with arguments parsed by `darling` |

## 6. Putting it all together

| Lesson | What it answers |
|---|---|
| [A `StateMachine` derive](a_state_machine_derive/README.md) *(outline)* | Variants, helper attributes, transitions rejected at compile time, identifiers that never clash |
...
- **`macro_rules!` itself.** [Macros](../25_Control_Flow/macros/README.md) is the introduction; this chapter compares declarative macros with procedural ones, and teaches only the second.
