...
**Physical, not abstract** — Knowing a memory rule well enough to draw the bytes behind it, rather than only to state it. `String` owns and `&str` borrows is the abstract form. The physical form is three words and a heap buffer for the owner, and two words pointing into that same buffer for the view, which is enough to predict all five borrow errors a string view can raise. → [Drawing the owner and the view](14_Strings/drawing_the_owner_and_the_view/README.md)
...
**`Borrow<Q>`** — The trait an owned type implements to lend itself out as a `&Q`: `String` as `str`, `Vec<T>` as `[T]`, every `T` as itself. Its docs add a promise no compiler checks — `Eq`, `Ord` and `Hash` agree on both sides — which is what lets `HashMap<String, V>::get` take a `&str`. `AsRef` is the same signature without the promise. → [`Borrow`: look up an owned key with a borrowed one](12_Traits/borrow_trait/README.md)
...
**Returned by value** — The function hands back the value itself, not its address, into room the caller reserved before the call. So the return type needs one size known at compile time, which `str`, `[T]` and `dyn Trait` lack: `-> str` is `E0277`, and `Clone`, whose `clone` returns `Self`, requires `Sized`. rustc 1.98.0 on x86-64 returns a number or a pair such as `&str` in registers and writes larger values through a hidden pointer, LLVM's `sret`. For a `String` only the three-word header comes back; the text stays in its heap buffer. → [Returned by value](18_Ownership/returned_by_value/README.md)

**`sret`** — LLVM's name for the hidden first argument of a function whose return value does not fit in registers: the address of room the caller reserved in its own frame. The callee writes the value straight into it and returns `void`; on x86-64 the address arrives in `rdi` and, per the System V psABI, goes back in `rax`. Nothing is copied on the way out, and the room is not zeroed first. → [Drawing `sret`](18_Ownership/drawing_the_return_slot/README.md)
...
**Polonius** — The next formulation of Rust's borrow checker, as a handful of rules: a borrow (a *loan*) is an error exactly when it is invalidated at a point where some live *origin* — the set of loans a reference may hold — still contains it. Stable `rustc` runs NLL; nightly's `-Zpolonius=next` also accepts programs NLL refuses, such as a conditional return of a borrow. Clang's `-Wlifetime-safety` names it as its model. → [Lifetime safety in Clang](31_C_and_Cpp/lifetime_safety_in_clang/README.md)

**Library hardening** — Bounds checks compiled into a C++ standard library's `operator[]` and friends, switched on with a macro (`_LIBCPP_HARDENING_MODE` in libc++), so an out-of-range index traps instead of reading past the end: Rust's index panic, as a build setting. It cannot reach `unique_ptr<T[]>`, which stores no length. → [Safe Buffers](31_C_and_Cpp/safe_buffers/README.md)
...
**Doctest** — A fenced code block inside a doc comment, which `cargo test` compiles and runs like any other test. It is compiled outside your crate and linked against it, so it sees the public API only — an integration test that is also the documentation. Edition 2024 compiles a crate's doc tests into one merged binary and still runs each in its own process. The reason Rust's examples cannot quietly rot into ones that no longer compile. → [The example that is a test](28_Testing/doc_tests/README.md), [Comments that compile](15_First_Programs/comments_that_compile/README.md)
...
**Format spec** — Everything after the `:` in `{value:>width$.prec$}` — fill, alignment, width, precision, and which trait to print through. A separate small language from the capture before the colon; a trailing `$` is what marks a width or precision as a *name* rather than a literal number. Which trait `{}` and `{:?}` reach for is a different question. → [The format mini-language](14_Strings/the_format_language/README.md), [The braces take a name](15_First_Programs/braces_take_a_name/README.md), [Debug and Display](15_First_Programs/debug_vs_display/README.md)

**Precision (`.N`)** — Digits after the point on a number, and a **maximum length** on a string, counted in `char`s: `{:.3}` of `"hello"` is `hel`. So `{:8}` pads and `{:.8}` cuts, and a dot typed where a width was meant silently truncates a whole column. → [The format mini-language](14_Strings/the_format_language/README.md)
...
**`E0689`** — *can't call method on ambiguous numeric type.* A method called on a number whose type is not settled yet: `24.5.round()`, or `x.pow(2)` after `let x = 1_000_000;`. The compiler needs the type to find the method, so it asks for a suffix or an annotation. Its suggestion is `i32`, which is too narrow for a million squared. → [Listing 2.3, run](10_Resources/rust_in_action/intro_to_numbers/README.md)
...
**Pattern (`str::pattern::Pattern`)** — The trait behind the argument to `contains`, `find`, `split`, `trim_matches`, `replace` and their relatives, which is why a `char`, a `&str`, a list of characters (**any one of** them, not that sequence) and an `FnMut(char) -> bool` are interchangeable there. Nothing to do with the patterns in a `match`. Still unstable, so you can use it on every call and cannot name it in a signature of your own — `P: Pattern` is `E0658` on stable. → [Searching without splitting](14_Strings/searching_a_string/README.md)

**Feature gate (`#![feature(…)]`, `E0658`)** — The door an unstable library or language item sits behind. `error[E0658]: use of unstable library feature` means the method **exists** in the standard library you are already linking against and is simply not promised to you yet — a different situation from a missing import or a typo, and one only nightly opens. Two live examples above and below: `P: Pattern`, and `str::as_str`, which reached stabilization and was **reverted** when the inherent method it added shadowed a trait method the ecosystem relied on. → [`str::as_str`](14_Strings/str_as_str/README.md), [`rustup default nightly`](05_Tooling/nightly/README.md)
...
**`zip`** — Walks two sequences as one sequence of pairs, and **ends when the shorter side ends** — no panic, no warning, no `Result`. Four items against three give three pairs and a silently short answer, which is why an `assert_eq!` on the two lengths belongs above any `zip` whose sides are supposed to match. Its argument is an `IntoIterator`, so `.zip(v)` moves `v` where `.zip(&v)` borrows it. → [`zip` and `enumerate`](24_Iterators/zip_and_enumerate/README.md)

**`enumerate`** — Wraps each item as `(index, item)`, counting in `usize` from zero. It numbers **what reaches it**, not the source: `.filter().enumerate()` gives row numbers in the output, `.enumerate().filter()` gives positions in the original. Both are consecutive and both start at zero, so the wrong one is invisible until someone indexes back into the collection that was filtered. → [`zip` and `enumerate`](24_Iterators/zip_and_enumerate/README.md)
...
**`Cargo.lock`** — The exact versions, sources and checksums a manifest's ranges resolved to, for every crate in the graph including the ones you never named. Every `cargo` command writes it, `build`/`run`/`test` obey it, `cargo install` ignores it unless told `--locked` — and it never reaches the people who depend on your crate, who resolve your *manifest* against their own. Commit it, for libraries too. → [`Cargo.lock`](05_Tooling/cargo_lock/README.md)

**Default (caret) requirement** — What a bare version string in `Cargo.toml` is: `1.2.3` means `>=1.2.3, <2.0.0`, and `^1.2.3` spells the same thing. The component that may not change is the **left-most non-zero** one, so `0.2.3` means `<0.3.0` and `0.8` and `0.9` are as incompatible as `1` and `2`. Tilde (`~`), wildcard (`*`) and exact (`=`) exist for the rare restriction that needs them. → [Adding a dependency](05_Tooling/cargo_dependencies/README.md)

**`cargo install --locked`** — Build a tool against the `Cargo.lock` its author published, rather than re-resolving every dependency to today's newest. Without the flag, `cargo install` is the one Cargo command that ignores a lockfile, so the binary you get was built against versions nobody tested. → [`Cargo.lock`](05_Tooling/cargo_lock/README.md)

**`cargo update --precise`** — Move one package in the lockfile to exactly the version named, up or down — but only *within* the manifest's requirement. Since `cargo add` writes the full current version (`serde = "1.0.229"`, meaning `^1.0.229`), a downgrade by one patch is refused until the manifest is loosened first. → [`Cargo.lock`](05_Tooling/cargo_lock/README.md)

**`cargo tree -d`** — List only the crates present in the build in more than one version, with the chain of who wants each. The answer you want is *"nothing to print"*; anything else is a subtree compiled twice and a type that will one day be "a different" copy of itself. `-i <crate>` inverts the tree to show who depends on it. → [Two versions of one crate](05_Tooling/two_versions_of_one_crate/README.md)

**Vendoring** — Copying every dependency's source into a folder in your repository (`cargo vendor`) and pointing Cargo at it through `.cargo/config.toml`, for builds with no network. The copies are read-only in a two-part sense: an incremental build *ignores* an edit to one, and a clean build *refuses* it by checksum. → [Vendoring, and the `[patch]` table](05_Tooling/vendoring_and_patch/README.md)

**`[patch]`** — The manifest table that substitutes your own copy of a crate — a path or a git checkout — for the registry's, for everyone in the graph who depends on it. Read from the workspace root only, applied only if the copy's version still satisfies the original requirement (else silently ignored), and the successor to the deprecated `[replace]`. → [Vendoring, and the `[patch]` table](05_Tooling/vendoring_and_patch/README.md)
...
**Variance** — Whether a type built from `T` may stand in for one built from a longer- or shorter-lived `T`. `&'a T` is *covariant*: a `&'static str` goes wherever a `&'a str` is wanted, and the compiler shrinks lifetimes for free. `&mut T` is *invariant* in `T`: no substitution either way, because writing through it could store something shorter-lived. That is why `&'a mut Node<'a>` borrows the node for the rest of its life. → [What `&'a T` claims](18_Ownership/what_a_reference_claims/README.md), [Borrowing something forever](18_Ownership/borrowing_forever/README.md)

**Self-referential struct** — A struct with a field that borrows from another field of the same value. Safe Rust can build one in place, but can never move it, return it from a constructor, take `&mut` to it or give it a `Drop`, so the usual fix is a range or index instead of the reference. → [A struct that points into itself](18_Ownership/self_referential_structs/README.md)
