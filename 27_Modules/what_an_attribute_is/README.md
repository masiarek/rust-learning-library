...
Atrybut (*attribute*) to metadane, na które **reaguje kompilator** — a nie kod wykonywany w czasie działania programu. Cała składnia sprowadza się do jednego wykrzyknika: `#[…]` dotyczy elementu **pod sobą**, a `#![…]` dotyczy tego, **w czym samo siedzi** (pliku albo bloku), i musi stać na samym jego początku. Warto od razu wiedzieć, że komentarz dokumentacyjny też jest atrybutem: `///` to lukier składniowy na `#[doc = "…"]`, a `//!` na `#![doc = "…"]`. Stąd bierze się typowe zaskoczenie — źle postawiony `///` daje błąd o atrybucie, a nie o komentarzu.

Polskiemu czytelnikowi atrybut kojarzy się zwykle z **dekoratorem** z Pythona albo **adnotacją** z Javy, i to skojarzenie jest trafne tylko w połowie. Dekorator to funkcja wywoływana w czasie działania, adnotacja Javy to metadane czytane refleksyjnie — a atrybut Rusta potrafi **usunąć element z programu**. `#[cfg(test)]` nie znaczy „pomiń przy uruchomieniu”, tylko „tego modułu nie ma w zwykłej kompilacji”, i dlatego kod testowy nie kosztuje binarki wydaniowej ani bajta. Rozróżnienie do zapamiętania brzmi: **`#[cfg]` usuwa, `cfg!()` wybiera** — to drugie jest wyrażeniem zamienianym na literał `true` albo `false`, więc obie gałęzie i tak muszą przejść kontrolę typów.

Najdroższa pułapka tej strony siedzi w `#[derive(Ord)]`: **porównanie generowane przez `derive` jest leksykograficzne w kolejności deklaracji pól**. Ta sama struktura z polami `name, score` sortuje trzy rekordy do `["Ada", "Ben", "Cara"]`, a z polami `score, name` — do `["Cara", "Ben", "Ada"]`. Przesunięcie pola o jedną linię wygląda w diffie na zmianę czysto kosmetyczną, a po cichu przestawia każdy `sort`, każdą mapę `BTreeMap` i każde `binary_search` po tym typie; jeśli kolejność jest częścią kontraktu, napisz `impl Ord` ręcznie, żeby zmianę widać było tam, gdzie naprawdę zachodzi. Kto zna Pythona, zna to z `@dataclass(order=True)` — identyczna pułapka i identyczne lekarstwo. Dwie mniejsze reguły z tej samej rodziny: `#[derive(Default)]` na wyliczeniu wymaga wariantu oznaczonego `#[default]` (`E0665`), a każde `derive` dokłada ograniczenie na typy generyczne, więc `#[derive(Clone)]` na `Wrapper<T>` daje `impl<T: Clone> Clone for Wrapper<T>` — i stąd `Wrapper<NotClone>` odmawia klonowania z powodu, którego nie ma w twoim kodzie.

Rodzina lintów ma cztery poziomy — `allow`, `warn`, `deny`, `forbid` — które zagnieżdżają się i wygrywa najbardziej wewnętrzny, z jednym wyjątkiem: `forbid` nie daje się poluzować, bo wewnętrzne `allow` jest samo w sobie błędem. Dlatego `#![forbid(unsafe_code)]` w korzeniu crate'a to obietnica warta złożenia, a `#![forbid(warnings)]` to pułapka: w dniu, w którym jedno wygenerowane makro potrąci nowy lint, nikt nie wyciszy go lokalnie. Zasada jest krótka — **`deny` do stylu, `forbid` do własności bezpieczeństwa.**

**Szukaj po polsku:** atrybuty w Ruscie · adnotacje a atrybuty · `rust derive Ord field order` · `rust cfg attribute vs cfg macro` · `rust deny vs forbid lint`
