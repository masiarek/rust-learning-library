...
This is the same program as `let s = "hello".to_string();`, down to the machine code. Build both under one crate name, so no symbol name can differ, and compare the assembly:

```bash
rustc --edition 2024 -O --crate-name demo --emit=asm named.rs -o named.s
rustc --edition 2024 -O --crate-name demo --emit=asm literal.rs -o literal.s
cmp named.s literal.s   # prints nothing: byte-identical
```

Byte-identical on 1.98.0, with `-O` and without it. The name is still there in MIR (`--emit=mir` shows `const main::GREETING`) and gone from the assembly. Both versions allocate a `String` in `.to_string()`, so the `const` makes nothing cheaper at run time.

What the name does change happens at compile time, and all of it follows from one fact: a `const` in a block is an **item**, the same kind of declaration as a `fn` or a `struct` written there, and not a statement.

| | `let s = "hello";` | `const S: &str = "hello";` |
|---|---|---|
| in scope | from its own line to the end of the block | the whole block, lines above it included |
| type | inferred | written out |
| may read | anything in scope | literals, other consts, `const fn` calls |
| outside the block | not in scope | not in scope — `E0425` |

Using a block's `const` above its own line compiles, and reads like a bug. Clippy's [`items_after_statements` ↗](https://rust-lang.github.io/rust-clippy/rust-1.98.0/index.html#items_after_statements) says so on the declaration, *"adding items after statements is confusing, since items exist from the start of the scope"*, but it is in `clippy::pedantic` and off by default.

### What it cannot see

Nothing the surrounding function has: not a `let` above it, and not the function's own type parameters. rustc states the rule in the second refusal, *"a `const` is a separate item from the item that contains it"*.

```rust
fn main() {
    let name = "hello";
    // const GREETING: &str = name;   // E0435
    println!("{name}");
}
```
...
error[E0435]: attempt to use a non-constant value in a constant
 --> e0435.rs:3:28
  |
3 |     const GREETING: &str = name;
  |                            ^^^^ non-constant value
  |
help: consider using `let` instead of `const`
  |
3 -     const GREETING: &str = name;
3 +     let GREETING: &str = name;
  |
```

```rust
fn width<T>() -> usize {
    // const BYTES: usize = size_of::<T>();   // E0401
    const { size_of::<T>() }
}
```
...
help: try using a local `let` binding instead
  |
2 -     const BYTES: usize = size_of::<T>();
2 +     let BYTES: usize = size_of::<T>();
  |
```

A value that only exists at run time wants the `let` rustc suggests. A compile-time value that depends on `T` wants the inline [`const { }` block ↗](https://doc.rust-lang.org/reference/expressions/block-expr.html#const-blocks) on the line under the comment: it is an expression rather than an item, so it sees `T`.

<!-- output:const_in_a_function -->
*Verified output of [`const_in_a_function.rs`](examples/const_in_a_function.rs) — regenerated by `tools/run_examples.py`, never hand-typed.*

```text
1. A named const and the literal it names
   named()   = "hello"
   literal() = "hello"
   Same value and the same machine code: the const is pasted in where
   it is named. Both allocate a String in to_string(); the name makes
   nothing cheaper at run time.

2. In scope for the whole block, above its own line too
   used_above_its_line() = "hello"
   A `let` is in scope from its own line down. A `const` in a block is
   an item, and an item's name covers the block it is declared in.

3. Not in scope outside that block
   sibling_blocks() = (10, 20)
   Two LIMITs in sibling blocks do not collide, and named()'s GREETING
   is not in scope in main: naming it there is E0425.

4. It cannot see the function it sits in
   a `let` binding above it     -> E0435, use `let` instead
   the function's own type T    -> E0401, use `const { }` instead
   width::<u32>() = 4, from `const { size_of::<T>() }`
```
<!-- /output -->
...
- **Python.** Module-level `MAX_SCORE = 5` is a `static` in the address sense — one object, referenced from everywhere — and Python has no equivalent of `const` substitution at all, because everything is a reference. Two habits transfer badly. Python's module constants are *mutable* (nothing stops `module.MAX_SCORE = 9`), where Rust's are enforced; and Python's mutable-default and shared-global bugs are exactly what `static mut` would let you write, which is why Rust makes you spell them with a `Mutex` instead. `functools.cache` on a pure function is the nearest thing to `const fn` evaluation, and it happens at run time. Inside a `def`, `GREETING = "hello"` is an ordinary local variable, and reading it above that line raises `UnboundLocalError` when the function runs; a Rust `const` in a function body is in scope from the top of its block, and its value is settled before the program exists.
...
[constants ↗](https://doc.rust-lang.org/rust-by-example/custom_types/constants.html) in Rust by Example; the Reference's [constant items ↗](https://doc.rust-lang.org/reference/items/constant-items.html) and [static items ↗](https://doc.rust-lang.org/reference/items/static-items.html), plus the edition guide's [static mut references ↗](https://doc.rust-lang.org/edition-guide/rust-2024/static-mut-references.html), which rustc's own note links to. For a `const` in a function body: the Reference's [item scopes ↗](https://doc.rust-lang.org/reference/names/scopes.html#item-scopes) (*"from the start of the block the item statement is in until the end of the block"*) and rustc's explanation of [E0401 ↗](https://doc.rust-lang.org/error_codes/E0401.html).

## Po polsku

Po polsku jedno i drugie nazwie się „stałą globalną” i właśnie ta nazwa myli, bo sugeruje, że różnica dotyczy niezmienności. Nie dotyczy: `const` to **wartość podstawiana w każdym miejscu użycia**, a `static` to **jeden obiekt pod jednym adresem** przez cały czas działania programu. Pytanie rozstrzygające brzmi więc nie „czy to się zmienia”, tylko „czy adres ma znaczenie”. Formalności są wspólne — oba wymagają jawnie wypisanego typu, bo na poziomie elementu nie ma wnioskowania typów, i oba zapisuje się wersalikami w stylu `SCREAMING_SNAKE_CASE`, bo inaczej rustc ostrzega.

Różnicę widać dopiero, gdy się ją zmierzy. `static` **ma** adres — dlatego `&METHOD` jest typu `&'static str`. `const` adresu nie ma, więc `&MAX_SCORE` powoduje promocję wartości (*const promotion*) do anonimowego `static`a, a kompilator może wyemitować jeden taki obiekt albo kilka. Porównanie `&MAX_SCORE` z `&MAX_SCORE` wychodzi w tej kompilacji `true` i nie wolno na tym niczego opierać; podstawianie ujawnia się dopiero przy dwukrotnym `Box::new(SCALE)`, gdzie `std::ptr::eq` daje `false` — dwie kopie, bo dokładnie to znaczy „podstawiana przy każdym użyciu”. Stąd praktyczna reguła: `const` domyślnie, `static` wtedy, gdy adres jest częścią znaczenia (symbol FFI) albo gdy kopiowanie boli — stała tablica 10 000 elementów wymieniona w trzech miejscach to 30 000 elementów w gotowej binarce.

`const fn` jest obietnicą, a nie optymalizacją. Wolno w niej rozgałęziać, robić pętle, indeksować i liczyć; nie wolno alokować, wywołać funkcji nie-`const` ani czytać `static`a — próba kończy się błędem `E0015`, *cannot call non-const function*. Ponieważ ktoś mógł użyć takiej funkcji do zainicjowania stałej, późniejsze odebranie jej `const` psuje cudzy kod i jest zmianą łamiącą API, więc traktuj to słowo jak część publicznego interfejsu. Osobno warto zapamiętać, że `static mut` jest w praktyce zamknięte: w edycji 2024 nawet `&COUNT` wewnątrz bloku `unsafe` odrzuca lint `static_mut_refs` (domyślnie `deny`), a kompilator podpowiada `&raw const COUNT`. Odpowiedzią na zmienny stan globalny — zwłaszcza dzielony między wątkami — jest `static` trzymający `Mutex`, `OnceLock` albo typ atomowy: bez `unsafe` i z regułą współdzielenia wypisaną wprost w typie.

Stała nie musi być globalna. `const GREETING: &str = "hello";` w ciele `main` daje **dokładnie ten sam kod maszynowy** co literał `"hello"` wpisany wprost — asembler obu wersji zbudowanych pod tą samą nazwą crate'a jest identyczny co do bajtu, a `.to_string()` alokuje w obu przypadkach. Nazwa zmienia tylko to, co dzieje się w czasie kompilacji, i wszystko wynika z jednego faktu: stała w bloku jest **elementem** (*item*), tak jak zagnieżdżona `fn` czy `struct`, a nie instrukcją. Dlatego jej nazwa obowiązuje w całym bloku, także powyżej linii z deklaracją (skompiluje się, choć Clippy z grupy `pedantic` zgłasza to jako `items_after_statements`), poza blokiem jej nie ma (`E0425`), i nie widzi niczego z funkcji, w której stoi: ani zmiennej z `let` (`E0435`, kompilator podpowiada `let`), ani parametru typu `T` (`E0401`, *can't use generic parameters from outer item*). Na to drugie jest blok `const { size_of::<T>() }` — to wyrażenie, a nie element, więc `T` widzi.

**Szukaj po polsku:** stałe globalne w Ruscie · stała a zmienna statyczna · stała lokalna w funkcji · `rust const vs static` · `rust const fn E0015` · `rust static_mut_refs 2024` · `rust E0401 generic parameters from outer item`
